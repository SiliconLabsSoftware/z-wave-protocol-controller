...
unsigned char raw_data[]
  = {0x20, 0x01, 0x00, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
...
{
  ZW_TransportService_SendData(&p,
                               raw_data2,
                               sizeof(raw_data2),
                               status_callback);
}
...
int simple_test(void)
{
  ...
  ret = compare_received_datagram(test_complete_datagram,
  ...
  ret = print_failed_if_nonzero(!(current_state == ST_IDLE),
                                "current state has to be ST_IDLE...");
  fail_if_nonzero(ret);
  printf("passed\n");
  return 0;
fail:
...
int test_dont_send_first_frag(void)
{
  ...
  ask_TS_to_receive(
    test_subseq_frag2,
    sizeof(
      test_subseq_frag2)); /* Skip sending first frag and send subseq frag instead */

  ret = memcmp(
    output,
    test_frag_wait_zero_pending,
    sizeof(test_frag_wait_zero_pending)); /* Check if we get fragment wait */
...
int test_frag_wait_fn(void)
{
  ...
  ask_TS_to_receive(test_first_frag1,
                    sizeof(test_first_frag1)); /* Send first frag */

  p.snode = 0xf1;
  ask_TS_to_receive(
    test_subseq_frag2,
    sizeof(
      test_subseq_frag2)); /* Send subseq frag from different source node */
  p.snode = 0xff;
  ...
  ask_TS_to_receive(
    test_subseq_frag2,
    sizeof(test_subseq_frag2)); /* Send correct second subseq frag */
  ask_TS_to_receive(
    test_subseq_frag3,
    sizeof(test_subseq_frag3)); /* Send correct third subseq frag */

  ret = memcmp(
    output,
    test_frag_compl,
    sizeof(test_frag_compl)); /* Check if we recive fragment complete */
...
int test_send_whole_datagram(void)
{
  ...
  ret
    = print_failed_if_nonzero(ret, "tets_send_whole_data_gram first fragment");
  fail_if_nonzero(ret);

  fire_timer_btwn_2_frags(0);
  ret = memcmp(output, test_subseq_frag2, sizeof(test_subseq_frag2));
  ret = print_failed_if_nonzero(
    ret,
    "tets_send_whole_data_gram first subseq fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    ret,
    "tets_send_whole_data_gram second subseq fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(!(global_status == S2_TRANSMIT_COMPLETE_OK),
                                "Did transmissino OK for sending session...");
  fail_if_nonzero(ret);
  ret = print_failed_if_nonzero(
    check_scb_current_dnode(),
    "tets_send_whole_data_gram frag_compl processing");
  fail_if_nonzero(ret);
  ret = print_failed_if_nonzero(!(current_state == ST_IDLE),
                                "current state has to be ST_IDLE...");
  fail_if_nonzero(ret);
...
int test_jakob(void)
{
  ...
  ask_TS_to_receive(
    test_frag_req,
    sizeof(
      test_frag_req)); /* <-- Send fragment request with different session id */
  ret
    = print_failed_if_nonzero(fc_timer_counter,
                              "test_send_frag_req_with_diff_session_id(jakob)");
  fail_if_nonzero(ret);
  ...
  ask_TS_to_receive(
    test_frag_compl,
    sizeof(
      test_frag_compl)); /* <-- Send fragment request with different session id */
...
int test_tie_break(void)
{
  ...
  ask_TS_to_receive(test_first_frag1,
                    sizeof(test_first_frag1)); /* <-- ask to receive*/
  ret = print_failed_if_nonzero(!check_flag_tie_broken(), "Flag tie broken");
  fail_if_nonzero(ret);
  ask_TS_to_receive(test_subseq_frag2,
                    sizeof(test_subseq_frag2)); /* <-- ask to receive*/
  ask_TS_to_receive(test_subseq_frag3,
                    sizeof(test_subseq_frag3)); /* <-- ask to receive*/
                                                //    fire_rx_timer();
  ret = memcmp(output, test_frag_compl, sizeof(test_frag_compl));
...
int test_two_last_fragments(void)
{
  ...
  ret = memcmp(output, test_subseq_frag3, sizeof(test_subseq_frag3));
  ret
    = print_failed_if_nonzero(ret,
                              "test_two_last_fragments second fragment again");
  fail_if_nonzero(ret);

  ret = print_failed_if_nonzero(!check_flag_fc_timer_expired_once(),
                                "fc_timer_expired_flag_check");
  fail_if_nonzero(ret);
  p.snode = 0xfe;
  p.dnode = 0xff;
  ask_TS_to_receive(test_frag_compl, sizeof(test_frag_compl));
...
int test_send_frag_compl_from_diff_session(void)
{
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_send_frag_compl_from_diff_session first fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_send_frag_compl_from_diff_session second fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    fc_timer_counter,
    "test_send_frag_compl_from_diff_session fc_timer");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    check_scb_current_dnode(),
    "test_send_frag_compl_from_diff_session frag_compl processing");
  fail_if_nonzero(ret);
  ret = print_failed_if_nonzero(!(current_state == ST_IDLE),
                                "current state has to be ST_IDLE...");
  fail_if_nonzero(ret);
...
int test_abort_transmission(void)
{
  ...
  ret = print_failed_if_nonzero(
    fc_timer_counter,
    "test_abort_transmission frag_req should be ignored");
...
int test_fc_timer_after_frag_req(void)
{
  ...
  fire_timer_btwn_2_frags(0); /* <-- ask to send second frag */
  ret = memcmp(output, test_subseq_frag2, sizeof(test_subseq_frag2));
  ret = print_failed_if_nonzero(ret,
                                "test_fc_timer_after_frag_req second fragment");
  fail_if_nonzero(ret);
  ...
  fire_fc_timer();
  ret = print_failed_if_nonzero(!(global_status == S2_TRANSMIT_COMPLETE_FAIL),
                                "Did transmissino fail...");
  fail_if_nonzero(ret);
  ret = print_failed_if_nonzero(!(current_state == ST_IDLE),
                                "current state has to be ST_IDLE...");
  fail_if_nonzero(ret);
...
int test_fc_timer_after_frag_compl_of_aborted_transmission(void)
{
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_fc_timer_after_frag_compl_of_aborted_transmission first fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_fc_timer_after_frag_compl_of_aborted_transmission second fragment");
  fail_if_nonzero(ret);
  ...
  ask_TS_to_receive(
    test_first_frag1,
    sizeof(test_first_frag1)); /* <-- ask to receive with session id:10 */
  restore_crc(test_first_frag1, sizeof(test_first_frag1), crc);
  test_first_frag1[3] = backup_byte;
  ...
  fire_fc_timer();
  ret = print_failed_if_nonzero(!(global_status == S2_TRANSMIT_COMPLETE_FAIL),
                                "Did transmissino fail for sending session...");
  fail_if_nonzero(ret);
  ...
  ask_TS_to_receive(
    test_subseq_frag2,
    sizeof(
      test_subseq_frag2)); /* <-- ask to receive subseq frag with session id:10 */
  restore_crc(test_subseq_frag2, sizeof(test_subseq_frag2), crc);
  test_subseq_frag2[3] = backup_byte;
  ...
  ask_TS_to_receive(
    test_subseq_frag3,
    sizeof(
      test_subseq_frag3)); /* <-- ask to receive subseq frag with session id:10 */
  restore_crc(test_subseq_frag3, sizeof(test_subseq_frag3), crc);
  test_subseq_frag3[3] = backup_byte;

  backup_byte
    = test_frag_compl[2]; /* Check if frag compl received for session id:10 */
  test_frag_compl[2] = 0xA0;
  ret                = memcmp(output, test_frag_compl, sizeof(test_frag_compl));
  ret                = print_failed_if_nonzero(ret,
                                "test_fc_timer_after_frag_compl_of_aborted_"
                                               "transmission frag_compl received check");
  test_frag_compl[2] = backup_byte;
  fail_if_nonzero(ret);
...
int test_fc_timer_after_last_frag_twice(void)
{
  ...
  ret = memcmp(output, test_first_frag1, sizeof(test_first_frag1));
  print_failed_if_nonzero(ret,
                          "test_fc_timer_after_last_frag_twice first fragment");

  fire_timer_btwn_2_frags(0); /* <-- ask to send second frag */
  ret = memcmp(output, test_subseq_frag2, sizeof(test_subseq_frag2));
  ret = print_failed_if_nonzero(
    ret,
    "test_fc_timer_after_last_frag_twice second fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_fc_timer_after_last_frag_twice second fragment again");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(check_scb_current_dnode(),
                                "current_dnode is set to 0 check");
  fail_if_nonzero(ret);
  ret = print_failed_if_nonzero(!(current_state == ST_IDLE),
                                "current state has to be ST_IDLE...");
  fail_if_nonzero(ret);
...
int test_frag_wait_for_completed_session(void)
{
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_frag_wait_for_completed_session first fragment");
  fail_if_nonzero(ret);
  ...
  ret = print_failed_if_nonzero(
    ret,
    "test_frag_wait_for_completed_session second fragment");
  fail_if_nonzero(ret);
...
int three_node_test(void)
{
  ...
  ret = memcmp(
    ...
    sizeof(test_frag_wait_two_pending)); /* Check if we get fragment wait */
  ...
  ret = memcmp(output,
               test_frag_wait,
               sizeof(test_frag_wait)); /* Check if we get fragment wait */
 ...
 */
{
  ...
  backup[1] = test_first_frag1[2];
  test_first_frag1[1]
    = COMMAND_FIRST_FRAGMENT | ((DATAGRAM_SIZE_MAX + 1) & 0x700) >> 8;
  test_first_frag1[2] = ((DATAGRAM_SIZE_MAX + 1) & 0xff);
  regenerate_crc(test_first_frag1, sizeof(test_first_frag1), crc);
  ...
  ret = print_failed_if_nonzero(
    ret,
    "sending segment request after too long first segment");
  fail_if_nonzero(ret);
  ...
  ret
    = print_failed_if_nonzero(ret,
                              "correct subsequent without first triggers data");
  fail_if_nonzero(ret);
...
int call_ask_TS_to_receive_with_large_size(void)
{
  ...
  ret = print_failed_if_nonzero(!call_with_large_value,
                                "call_ask_TS_to_receive_with_large_size");
  fail_if_nonzero(ret);
  ret = print_failed_if_nonzero(!(current_state == ST_IDLE),
                                "current state has to be ST_IDLE...");
  fail_if_nonzero(ret);
