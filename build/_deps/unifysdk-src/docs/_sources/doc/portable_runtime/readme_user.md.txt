...
With only one device connected with only ZPC, you can just run the *unify_portable_cli* application. For combinations which require no device, see [EED](#emulated-end-device-experimental-1).
...
The Unify Matter Bridge is an application that makes legacy devices, such as Z-Wave Devices accessible on a Matter fabric. It does so by acting as an IoT Service in a Unify Framework. Please visit [Unify Matter Bridge](https://siliconlabs.github.io/matter/latest/unify/matter-bridge/readme_overview.html) for more details.
