...
sl_status_t zwapi_soft_reset(void)
{
    ...
    sl_status_t status        = zwapi_send_command(FUNC_ID_SERIAL_API_SOFT_RESET, NULL, 0);
...
uint8_t zwapi_get_zw_protocol_status(void)
{
    ...
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_ZW_GET_PROTOCOL_STATUS, NULL, 0, response_buffer, &response_length);
...
uint8_t zwapi_get_zw_library_type(void)
{
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_ZW_TYPE_LIBRARY, NULL, 0, response_buffer, &response_length);
...
uint8_t zwapi_get_zw_library_version(uint8_t *pBuf)
{
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_ZW_GET_VERSION, NULL, 0, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 12)) {
        if (pBuf) {
...
sl_status_t zwapi_set_rf_receive_mode(uint8_t mode)
{
    ...
    sl_status_t send_command_status             = zwapi_send_command_with_response(FUNC_ID_ZW_SET_RF_RECEIVE_MODE, request_buffer, index, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA && response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE) {
        return SL_STATUS_OK;
    }
...
rf_power_level_t zwapi_set_rf_power_level(rf_power_level_t powerLevel) /* IN Powerlevel to set */
{
    uint8_t response_length                     = 0;
    uint8_t index                               = 0;
    uint8_t request_buffer[REQUEST_BUFFER_SIZE] = {0};
    ...
    sl_status_t send_command_status             = zwapi_send_command_with_response(FUNC_ID_ZW_RF_POWER_LEVEL_SET, request_buffer, index, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA) {
        return response_buffer[IDX_DATA];
    }
    return NORMAL_POWER;
...
rf_power_level_t zwapi_get_rf_power_level(void)
{
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_ZW_RF_POWER_LEVEL_GET, NULL, 0, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA) {
        return response_buffer[IDX_DATA];
    }
    return NORMAL_POWER;
...
{
    if (!zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) || !zwapi_support_setup_func(SERIAL_API_SETUP_CMD_TX_STATUS_REPORT)) {
    ...
    sl_status_t frame_status = zwapi_session_send_frame_with_response(FUNC_ID_SERIAL_API_SETUP, request_buffer, sizeof(request_buffer), response_buffer, &response_length);
    if (frame_status != SL_STATUS_OK) {
        sl_log_warning(LOG_TAG, "Failed to configure tx status report for send_data\n");
    }
...
{
    if (!zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) || !zwapi_support_setup_func(SERIAL_API_SETUP_CMD_MAX_LR_TX_PWR_SET)) {
        return SL_STATUS_NOT_SUPPORTED;
    }
    uint8_t response_length                     = 0;
    uint8_t index                               = 0;
    ...
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == SERIAL_API_SETUP_CMD_MAX_LR_TX_PWR_SET && response_buffer[IDX_DATA + 1] == ZW_COMMAND_RETURN_VALUE_TRUE) {
        return SL_STATUS_OK;
    }
    return SL_STATUS_FAIL;
}
...
int16_t zwapi_get_max_lr_tx_power_level(void)
{
    ...
    if (zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) && zwapi_support_setup_func(SERIAL_API_SETUP_CMD_MAX_LR_TX_PWR_GET)) {
        uint8_t response_length                     = 0;
        uint8_t index                               = 0;
        uint8_t request_buffer[REQUEST_BUFFER_SIZE] = {0};
        uint8_t response_buffer[FRAME_LENGTH_MAX]   = {0};
...
tx_power_level_t zwapi_get_tx_power_level(void)
{
    ...
    if (zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) && zwapi_support_setup_func(SERIAL_API_SETUP_CMD_TX_POWERLEVEL_GET)) {
        uint8_t response_length                     = 0;
        uint8_t index                               = 0;
        uint8_t request_buffer[REQUEST_BUFFER_SIZE] = {0};
        uint8_t response_buffer[FRAME_LENGTH_MAX]   = {0};
...
{
    if (!zwapi_support_command_func(FUNC_ID_ENABLE_RADIO_PTI) || !zwapi_support_command_func(FUNC_ID_GET_RADIO_PTI)) {
        return false;
    }
    return true;
}
...
bool zwapi_is_pti_enabled()
{
    ...
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_GET_RADIO_PTI, NULL, 0, response_buffer, &response_length);
    if ((send_command_status == SL_STATUS_OK) && (response_length == (IDX_DATA + 1)) && (response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE)) {
        return true;
    }
    return false;
}
...
sl_status_t zwapi_set_radio_pti(bool state)
{
    ...
    sl_status_t send_command_status             = zwapi_send_command_with_response(FUNC_ID_ENABLE_RADIO_PTI, request_buffer, index, response_buffer, &response_length);
    if ((send_command_status == SL_STATUS_OK) && (response_length == (IDX_DATA + 1)) && (response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE)) {
        return SL_STATUS_OK;
    }
    return SL_STATUS_FAIL;
...
{
    if (!zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) || !zwapi_support_setup_func(SERIAL_API_SETUP_CMD_RF_REGION_SET)) {
        return SL_STATUS_NOT_SUPPORTED;
    }
    ...
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == SERIAL_API_SETUP_CMD_RF_REGION_SET && response_buffer[IDX_DATA + 1] == ZW_COMMAND_RETURN_VALUE_TRUE) {
        return SL_STATUS_OK;
    }
...
{
    if (!zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) || !zwapi_support_setup_func(SERIAL_API_SETUP_CMD_MAXIMUM_PAYLOAD_SIZE_GET)) {
        return SL_STATUS_NOT_SUPPORTED;
    }
    uint8_t response_length                     = 0;
    uint8_t index                               = 0;
    ...
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == SERIAL_API_SETUP_CMD_MAXIMUM_PAYLOAD_SIZE_GET) {
...
{
    if (!zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) || !zwapi_support_setup_func(SERIAL_API_SETUP_CMD_LR_MAXIMUM_PAYLOAD_SIZE_GET)) {
        return SL_STATUS_NOT_SUPPORTED;
    }
    uint8_t response_length                     = 0;
    uint8_t index                               = 0;
    ...
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == SERIAL_API_SETUP_CMD_LR_MAXIMUM_PAYLOAD_SIZE_GET) {
        *maximum_size = response_buffer[IDX_DATA + 1];
        return SL_STATUS_OK;
    }
    return SL_STATUS_FAIL;
...
sl_status_t zwapi_set_node_id_basetype(zwave_node_id_basetype_t basetype)
{
    ...
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == SERIAL_API_SETUP_CMD_NODEID_BASETYPE_SET && response_buffer[IDX_DATA + 1] == ZW_COMMAND_RETURN_VALUE_TRUE) {
...
sl_status_t zwapi_send_node_information(zwave_node_id_t destNode, uint8_t txOptions, void (*completedFunc)(uint8_t))
{
    ...
    return zwapi_send_command(FUNC_ID_ZW_SEND_NODE_INFORMATION, request_buffer, index);
}
...
sl_status_t zwapi_send_test_frame(zwave_node_id_t destination_node_id, rf_power_level_t power_level, void (*callback_function)(uint8_t tx_status))
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_SEND_TEST_FRAME, request_buffer, index, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA && response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE) {
        return SL_STATUS_OK;
    }
...
sl_status_t zwapi_get_random_word(uint8_t *random_buffer, uint8_t number_of_random_bytes)
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_GET_RANDOM, request_buffer, index, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE) {
...
sl_status_t zwapi_get_random_byte(uint8_t *user_byte)
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_RANDOM, NULL, 0, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA) {
...
/* TODO - Consolidate zwapi_explore_request_inclusion() and setlearnMode - Let protocol handle */
...
sl_status_t zwapi_explore_request_inclusion()
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_EXPLORE_REQUEST_INCLUSION, NULL, 0, response_buffer, &response_length);
...
/* TODO - Consolidate zwapi_explore_request_exclusion() and setlearnMode - Let protocol handle */
...
sl_status_t zwapi_explore_request_exclusion()
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_EXPLORE_REQUEST_EXCLUSION, NULL, 0, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA && response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE) {
        return SL_STATUS_OK;
    }
...
sl_status_t zwapi_get_background_rssi(uint8_t *rssi_values, uint8_t *values_length)
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_GET_BACKGROUND_RSSI, NULL, 0, response_buffer, &number_of_channels);
...
sl_status_t zwapi_set_application_node_information(uint8_t listening, node_type_t node_type, const uint8_t *nodeParm, uint8_t parmLength)
{
    ...
    return zwapi_send_command(FUNC_ID_SERIAL_API_APPL_NODE_INFORMATION, request_buffer, index);
}
...
sl_status_t zwapi_get_full_node_list(zwave_nodemask_t node_list)
{
    ...
    sl_status_t extended_node_list_status = zwapi_get_long_range_nodes(&length, node_list);

    // if 1 of them returns OK, then we return OK
    // since there are some nodes added to the mask.
    if (node_list_status == SL_STATUS_OK || extended_node_list_status == SL_STATUS_OK) {
        return SL_STATUS_OK;
    }
    return SL_STATUS_FAIL;
}
...
sl_status_t zwapi_get_long_range_nodes(uint16_t *number_of_bytes, zwave_nodemask_t lr_node_list)
{
    ...
    *number_of_bytes           = 0;
    ...
    while (more_nodes == 1) {
        ...
        command_status = zwapi_send_command_with_response(FUNC_ID_SERIAL_API_GET_LR_NODES, request_buffer, index, response_buffer, &response_length);
            ...
            *number_of_bytes += received_nodelist_length;
            ...
            for (int i = 0; i < received_nodelist_length; i++) {
                if ((i + (start_offset * 128)) > sizeof(zwave_nodemask_t)) {
                    sl_log_warning(LOG_TAG, "Z-Wave API get LR node list index of bound\n");
                    return SL_STATUS_FAIL;
                }
...
sl_status_t zwapi_get_long_range_channel(uint8_t *lr_channel)
{
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_GET_LR_CHANNEL, NULL, 0, response_buffer, &response_length);
    if (send_command_status == SL_STATUS_OK && response_length > IDX_DATA) {
...
sl_status_t zwapi_set_long_range_channel(uint8_t lr_channel)
{
    ...
    return zwapi_send_command_with_response(FUNC_ID_SET_LR_CHANNEL, request_buffer, index, response_buffer, &response_length);
}
...
bool zwapi_supports_long_range()
{
     ...
     */
    if (!zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP) || !zwapi_support_setup_func(SERIAL_API_SETUP_CMD_NODEID_BASETYPE_SET)) {
        return false;
    }
    return true;
}
...
{
    if (zwapi_support_command_func(FUNC_ID_ZW_TRANSFER_PROTOCOL_CC) && zwapi_support_command_func(FUNC_ID_ZW_ENABLE_NODE_NLS) && zwapi_support_command_func(FUNC_ID_ZW_GET_NODE_NLS_STATE) && zwapi_support_command_func(FUNC_ID_ZW_REQUEST_PROTOCOL_CC_ENCRYPTION)
