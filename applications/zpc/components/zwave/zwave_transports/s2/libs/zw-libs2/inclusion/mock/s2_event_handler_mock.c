...
#define MOCK_CALL_COMPARE_INPUT_STRUCT_S2(P_MOCK, ARGUMENT, P_ACTUAL)    \
  do {                                                                   \
    MOCK_CALL_COMPARE_STRUCT_MEMBER_UINT16(P_MOCK,                       \
                                           ARGUMENT,                     \
                                           P_ACTUAL,                     \
                                           zwave_event_t,                \
                                           evt.s2_event.peer.l_node);    \
    MOCK_CALL_COMPARE_STRUCT_MEMBER_UINT16(P_MOCK,                       \
                                           ARGUMENT,                     \
                                           P_ACTUAL,                     \
                                           zwave_event_t,                \
                                           evt.s2_event.peer.r_node);    \
    MOCK_CALL_COMPARE_STRUCT_MEMBER_UINT8(P_MOCK,                        \
                                          ARGUMENT,                      \
                                          P_ACTUAL,                      \
                                          zwave_event_t,                 \
                                          evt.s2_event.peer.tx_options); \
  } while (0)
...
void s2_event_handler(zwave_event_t *p_actual_evt)
{
  ...
  if (p_mock->compare_rule_arg[0] == COMPARE_STRICT) {
    TEST_ASSERT_EQUAL_UINT32(
      ((zwave_event_t *)p_mock->expect_arg[0].pointer)->event_type,
      p_actual_evt->event_type);
    ...
    switch (((zwave_event_t *)p_mock->expect_arg[0].pointer)->event_type) {
      case S2_NODE_INCLUSION_KEX_REPORT_EVENT:
        MOCK_CALL_COMPARE_STRUCT_MEMBER(
          p_mock,
          ARG0,
          p_actual_evt,
          zwave_event_t,
          evt.s2_event.s2_data.kex_report.security_keys,
          UINT8);
        MOCK_CALL_COMPARE_STRUCT_MEMBER(p_mock,
                                        ARG0,
                                        p_actual_evt,
                                        zwave_event_t,
                                        evt.s2_event.s2_data.kex_report.csa,
                                        UINT8);
        break;

      case S2_NODE_INCLUSION_PUBLIC_KEY_CHALLENGE_EVENT:
        MOCK_CALL_COMPARE_STRUCT_ARRAY_UINT8(
          p_mock,
          ARG0,
          p_actual_evt,
          zwave_event_t,
          evt.s2_event.s2_data.challenge_req.public_key,
          evt.s2_event.s2_data.challenge_req.length);
        MOCK_CALL_COMPARE_STRUCT_MEMBER(
          p_mock,
          ARG0,
          p_actual_evt,
          zwave_event_t,
          evt.s2_event.s2_data.challenge_req.granted_keys,
          UINT8);
        MOCK_CALL_COMPARE_STRUCT_MEMBER(
          p_mock,
          ARG0,
      ...
      case S2_NODE_INCLUSION_COMPLETE_EVENT:
        MOCK_CALL_COMPARE_STRUCT_MEMBER(
          ...
          p_actual_evt,
          zwave_event_t,
          evt.s2_event.s2_data.inclusion_complete.exchanged_keys,
          UINT8);
        break;
      ...
      case S2_NODE_INCLUSION_FAILED_EVENT:
        MOCK_CALL_COMPARE_STRUCT_MEMBER(
          ...
          p_actual_evt,
          zwave_event_t,
          evt.s2_event.s2_data.inclusion_fail.kex_fail_type,
          UINT8);
        break;
      ...
      default:
        TEST_FAIL_MESSAGE(
          "Unsupported event type expected on this mock in " TO_STR_MACRO(
            __FILE__) ":" TO_STR_MACRO(__LINE__));
        break;
    }
  }
}
