...
/**
 * @brief Clear exhausted Get retry state under @p node and rescan.
 *
 * For each attribute in the subtree that still needs a Get but previously
 * reached max retries (give-up), clears the pending Get entry so the Get can
 * be attempted again. Used when a new communication window opens (e.g. Wake Up
 * Notification during an unfinished interview).
 *
 * @param node Root attribute (typically a NodeID node) to walk.
 * @returns Number of Get resolutions restarted.
 */
...
/**
 * @brief Asks the resolver to re-arm a group node's Set resolution once the
 *        in-flight resolution completes.
 *
 * Intended to be called when a Set rule is already in flight on @p node and a
 * new request would otherwise be silently dropped: the in-flight completion
 * callback unconditionally clears the group node's desired/reported, wiping
 * any state set by the new request. After the in-flight resolution finishes,
 * the resolver restores `desired = retry_count` and triggers a scan, so the
 * Set rule is re-executed using the latest sub-attribute desired values.
 *
 * @param node         Group attribute node whose desired should be restored.
 * @param retry_count  Value to restore on `desired` (same semantics as
 *                     @c group_resolution_options::retry_count).
 */
