...
// multicast session.
constexpr uint16_t MAXIMUM_MULTICAST_PAYLOAD_LENGTH = ZWAVE_TX_SAFE_LOWEST_MAX_PAYLOAD;

// Some defines used for Tx QoS
constexpr uint32_t DEFAULT_QOS               = ZWAVE_TX_QOS_RECOMMENDED_NODE_INTERVIEW_PRIORITY;
...
// Function prototypes
///////////////////////////////////////////////////////////////////////////////
static void on_zwave_group_send_data_complete(uint8_t status, const zwapi_tx_report_t *tx_info, void *user);

static void on_supervision_group_send_data_complete(uint8_t status, const zwapi_tx_report_t *tx_info, void *user);
...
static void on_settable_attribute_update(attribute_store_node_t node, attribute_store_change_t change);
...
void zpc_attribute_resolver_group_state_log()
{
    ...
    for (auto it = multicast_candidates.begin(); it != multicast_candidates.end(); ++it) {
        stream << *it;
        stream << " ";
    }
    stream << "]\n";
    ...
    for (auto it = multicast_pools.begin(); it != multicast_pools.end(); ++it) {
        ...
        for (auto node = it->node_list.begin(); node != it->node_list.end(); ++node) {
            stream << *node;
            stream << " ";
        }
        stream << "]\n";
...
static sl_status_t assign_multicast_pool(attribute_store_node_t node)
{
    ...
    if (is_node_in_resolution_list(node)) {
        sl_log_debug(LOG_TAG, "Send group skip in resolution list: node=%d", node);
        return SL_STATUS_OK;
    }
    ...
    zwave_controller_encapsulation_scheme_t encapsulation = zwave_tx_scheme_get_node_highest_security_class(node_id);

    uint16_t payload_length                           = 0;
    std::array<uint8_t, ZWAVE_MAX_FRAME_SIZE> payload = {0};
    auto resolution_function                          = attribute_resolver::set_function(attribute_store_get_node_type(node));
    ...
    if (SL_STATUS_OK != resolution_function(node, payload.data(), &payload_length)) {
        return SL_STATUS_FAIL;
    }

    bool use_supervision = !attribute_resolver_is_no_supervision_type(attribute_store_get_node_type(node)) && zwave_command_class_supervision_want_supervision_frame(node_id, endpoint_id);
    ...
    if (payload_length == 0 || payload_length > MAXIMUM_MULTICAST_PAYLOAD_LENGTH) {
    ...
    for (auto it = multicast_pools.begin(); it != multicast_pools.end(); ++it) {
        ...
        if (zwave_tx_is_group_locked(it->group_id)) {
            continue;
        }
        ...
        for (auto existing_node: it->node_list) {
            ...
            attribute_store_network_helper_get_node_id_from_node(existing_node, &existing_node_id);
 ...
 *                  be enqueued.
 */
static void zpc_attribute_resolver_send_singlecast_follow_ups(zwave_multicast_group_id_t group_id)
{
    ...
    for (auto it = multicast_pools.begin(); it != multicast_pools.end(); ++it) {
        ...
        for (auto node = it->node_list.begin(); node != it->node_list.end(); ++node) {
            ...
            if (it->use_supervision) {
                send_status = zwave_command_class_supervision_send_data(&info, it->payload_length, it->payload.data(), &options, on_supervision_group_send_data_complete, reinterpret_cast<void *>(user), &tx_session_id);
                if (send_status != SL_STATUS_OK) {
                    ...
                    on_supervision_group_send_data_complete(SUPERVISION_REPORT_FAIL, nullptr, reinterpret_cast<void *>(user));
                }
            } else {
                send_status = zwave_tx_send_data(&info, it->payload_length, it->payload.data(), &options, on_zwave_group_send_data_complete, reinterpret_cast<void *>(user), &tx_session_id);
                if (send_status != SL_STATUS_OK) {
                    // Make a send fail callback if we could not do it. It will
                    // force the resolver to resolve the attribute from scratch
                    on_resolver_zwave_send_data_complete(TRANSMIT_COMPLETE_FAIL, nullptr, reinterpret_cast<void *>(user));
                }
            }
            if (send_status == SL_STATUS_OK) {
 ...
 *
 * @param initiating_node  The node that the ZPC attribute resolver is trying
 ...
 */
static sl_status_t zpc_attribute_resolver_pools_send(attribute_store_node_t initiating_node)
{
    ...
    for (auto it = multicast_pools.begin(); it != multicast_pools.end(); ++it) {
        ...
        zwave_tx_scheme_get_node_tx_options(DEFAULT_QOS + MULTICAST_QOS_BOOST, 0, 0, &tx_options);

        sl_status_t send_status = SL_STATUS_OK;
        if (it->use_supervision) {
            send_status = zwave_command_class_supervision_send_data(&connection_info,
                                                                    it->payload_length,
                                                                    it->payload.data(),
                                                                    &tx_options,
        ...
        if (it->node_list.contains(initiating_node)) {
...
sl_status_t zpc_attribute_resolver_send_group(attribute_store_node_t node)
{
    ...
    if (!multicast_candidates.contains(node)) {
    ...
    multicast_pools.erase(std::remove_if(multicast_pools.begin(), multicast_pools.end(), [](multicast_session_t &s) { return s.group_id == ZWAVE_TX_INVALID_GROUP; }), multicast_pools.end());
...
static void on_new_set_rule_registered(attribute_store_type_t type)
{
    ...
    attribute_store_register_callback_by_type_and_state(on_settable_attribute_update, type, DESIRED_ATTRIBUTE);
}
...
static void on_settable_attribute_update(attribute_store_node_t node, attribute_store_change_t change)
{
    ...
    if (attribute_store_is_value_defined(node, DESIRED_ATTRIBUTE) && !attribute_store_is_value_matched(node)) {
