...
void get_association_list(association_bytes source, association_set &target)
{
    ...
    for (auto byte: source) {
        ...
        if (association.type == NODE_ID) {
            ...
            if (is_association_valid(association)) {
 ...
 *
 */
static void remove_node_id_from_association_set(zwave_node_id_t node_id, association_set &set)
{
 ...
 * @return std::vector<attribute>
 */
static std::vector<attribute> group_content_attribute_list(std::vector<attribute> endpoint_id_nodes)
{
    ...
    for (auto endpoint_node: endpoint_id_nodes) {
        for (attribute group_id_node: endpoint_node.children(ATTRIBUTE(GROUP_ID))) {
            attribute group_content_node(group_id_node.child_by_type(ATTRIBUTE(GROUP_CONTENT)));
...
{
    attribute_store_node_t endpoint_node = zwave_command_class_get_endpoint_id_node(node_id, endpoint_id);
    attribute_store_node_t group_id_node = attribute_store_get_node_child_by_value(endpoint_node, ATTRIBUTE(GROUP_ID), REPORTED_ATTRIBUTE, (uint8_t *)&group_id, sizeof(group_id), 0);
...
attribute_store_node_t get_group_content_node(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    attribute_store_node_t endpoint_node      = zwave_command_class_get_endpoint_id_node(node_id, endpoint_id);
    attribute_store_node_t group_id_node      = attribute_store_get_node_child_by_value(endpoint_node, ATTRIBUTE(GROUP_ID), REPORTED_ATTRIBUTE, (uint8_t *)&group_id, sizeof(group_id), 0);
    attribute_store_node_t group_content_node = attribute_store_get_first_child_by_type(group_id_node, ATTRIBUTE(GROUP_CONTENT));
...
{
    attribute_store_node_t endpoint_node            = zwave_command_class_get_endpoint_id_node(node_id, endpoint_id);
    attribute_store_node_t supported_groupings_node = attribute_store_get_first_child_by_type(endpoint_node, ATTRIBUTE(SUPPORTED_GROUPINGS));

    association_group_count_t number_of_groups = 0;
    attribute_store_get_reported(supported_groupings_node, &number_of_groups, sizeof(number_of_groups));
...
association_group_capacity_t get_group_capacity(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    attribute_store_node_t endpoint_node       = zwave_command_class_get_endpoint_id_node(node_id, endpoint_id);
    attribute_store_node_t group_id_node       = attribute_store_get_node_child_by_value(endpoint_node, ATTRIBUTE(GROUP_ID), REPORTED_ATTRIBUTE, (uint8_t *)&group_id, sizeof(group_id), 0);
    attribute_store_node_t group_capacity_node = attribute_store_get_first_child_by_type(group_id_node, ATTRIBUTE(MAX_NODES_SUPPORTED));

    association_group_capacity_t group_capacity = 0;
    attribute_store_get_reported(group_capacity_node, &group_capacity, sizeof(group_capacity));
...
sl_status_t add_desired_association(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id, association_t association)
{
    try {
        ...
        association_bytes bytestream = group.get<association_bytes>(DESIRED_OR_REPORTED_ATTRIBUTE);
        ...
        if (get_group_capacity(node_id, endpoint_id, group_id) < associations.size()) {
        ...
        sl_log_warning(LOG_TAG, "Failed to Add desired association: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
...
sl_status_t add_reported_association(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id, association_t association)
{
    try {
        ...
        attribute group(get_group_content_node(node_id, endpoint_id, group_id));
        association_bytes bytestream = group.get<association_bytes>(REPORTED_ATTRIBUTE);
        ...
        sl_log_warning(LOG_TAG, "Failed to Add Reported association: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
    ...
    try {
        ...
        if (get_group_capacity(node_id, endpoint_id, group_id) < associations.size()) {
            ...
            for (auto association: associations) {
                ...
                if ((association.node_id == new_association.node_id) && (association.type == new_association.type) && (association.endpoint_id == new_association.endpoint_id)) {
                    continue;
                }
...
sl_status_t remove_desired_association(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id, association_t association)
{
    try {
        ...
        sl_log_warning(LOG_TAG, "Failed to Remove desired association: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
...
sl_status_t remove_reported_association(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id, association_t association)
{
    try {
        ...
        sl_log_warning(LOG_TAG, "Failed to Remove reported association: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
...
sl_status_t remove_all_desired_associations(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    try {
        ...
        sl_log_warning(LOG_TAG, "Failed to Remove All Desired associations: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
...
sl_status_t remove_all_reported_associations(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    try {
        ...
        sl_log_warning(LOG_TAG, "Failed to Remove All Reported associations: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
...
sl_status_t remove_all_reported_node_id_associations(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    try {
        ...
        sl_log_warning(LOG_TAG, "Failed to Remove all NodeID reported associations: %s\n", err.what());
        return SL_STATUS_FAIL;
    }
}
...
bool is_reported_group_full(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    try {
        ...
        return associations.size() >= get_group_capacity(node_id, endpoint_id, group_id);

    } catch (const std::exception &err) {
        sl_log_warning(LOG_TAG, "Failed to evaluate if reported association group is full: %s\n", err.what());
        return false;
    }
}
...
bool is_desired_group_full(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id)
{
    try {
        // Read the desired assocation group bytestream from the attribute store
        attribute group(get_group_content_node(node_id, endpoint_id, group_id));
        association_bytes bytestream = group.get<association_bytes>(DESIRED_ATTRIBUTE);
...
bool is_association_in_group(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_group_id_t group_id, association_t association)
{
    try {
        ...
        sl_log_warning(LOG_TAG, "Failed to evaluate if an association is in a group: %s\n", err.what());
        return false;
    }
}
...
{
    for (auto group_content_node: group_content_attribute_list(endpoint_id_attribute_list())) {
        try {
            // Read the Desired assocation group bytestream from the attribute store
            association_bytes bytestream = group_content_node.get<association_bytes>(DESIRED_OR_REPORTED_ATTRIBUTE);
            ...
            sl_log_warning(LOG_TAG, "Failed to update association group content: %s\n", err.what());
        }
    }
}
...
{
    association_group_count_t group_count = get_number_of_groups(node_id, endpoint_id);

    for (association_group_id_t current_group_id = 1; current_group_id <= group_count; current_group_id++) {
        add_desired_association(node_id, endpoint_id, current_group_id, association);
    }
}
...
void remove_desired_association_in_all_groups(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id, association_t association)
{
    ...
    for (association_group_id_t current_group_id = 1; current_group_id <= group_count; current_group_id++) {
        remove_desired_association(node_id, endpoint_id, current_group_id, association);
    }
}
...
void remove_desired_node_id_from_all_associations(zwave_node_id_t node_id_to_remove, zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    ...
    for (association_group_id_t current_group_id = 1; current_group_id <= group_count; current_group_id++) {
        try {
            attribute group(get_group_content_node(node_id, endpoint_id, current_group_id));
            association_bytes bytestream = group.get<association_bytes>(DESIRED_OR_REPORTED_ATTRIBUTE);
