...
typedef struct {
        ...
        ///
        /// Invoked synchronously when a Z-Wave frame is received, before it is
        /// dispatched to any transport or application layer. Subscribers that
        /// need to make payload-aware decisions (for example to distinguish
        /// S2 transport-internal frames from application-level replies for
        /// reply matching) may inspect `frame_data`/`frame_length`. For NIFs
        /// and other non-application events where no raw Z-Wave payload is
        /// available, `frame_data` is NULL and `frame_length` is 0.
        ///
        /// @param node_id         Z-Wave NodeID that sent us a frame.
        /// @param frame_data      Z-Wave frame as received from the controller
        ///                        or NULL if not available for this event.
        /// @param frame_length    Length of `frame_data` in bytes, or 0 if
        ///                        `frame_data` is NULL.
        ///
        void (*on_rx_frame_received)(zwave_node_id_t node_id, const uint8_t *frame_data, uint16_t frame_length);
...
/// Mark the reset chain as completed so observers can publish a "ready"
/// signal once the Z-Wave Controller has entered the new network.
/// Runs after the Z-Wave API set-default step.
#define ZWAVE_CONTROLLER_FACTORY_RESET_REPORT_STEP_PRIORITY (ZWAVE_CONTROLLER_MAXIMUM_CALLBACK_SUBSCRIBERS - 1)
 ...
 * component and/or trigger an asynchronous event using the threading system.
 *
