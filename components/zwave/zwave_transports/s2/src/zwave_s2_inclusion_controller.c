...
{
    uint8_t schemes = mp_context->buf[SECURITY_2_KEX_REP_SCHEME_POS];

    if (schemes & (uint8_t)~SECURITY_2_SCHEME_SUPPORT_MASK) {
        inclusion_failed_frame_send(KEX_FAIL_KEX_SCHEME, NON_SECURE);
        return KEX_FAIL;
    }

    mp_context->scheme_support = schemes;
...
{
    uint8_t curves = mp_context->buf[SECURITY_2_KEX_REP_CURVE_POS];

    if (curves & (uint8_t)~KEX_REPORT_CURVE_MASK) {
        inclusion_failed_frame_send(KEX_FAIL_KEX_CURVES, NON_SECURE);
        return KEX_FAIL;
    }

    mp_context->curve_support = curves;
...
static uint8_t validate_requested_key(void)
{
    ...
    if ((0 == (req_key & (req_key - 1))) && (req_key & mp_context->key_granted))  // key_granted is already in LR format because it was converted in s2_inclusion_key_grant()
    {
...
{
    // Test that only one bit is set and that the bit corresponds to a granted key.
    if ((mp_context->scheme_support == mp_context->buf[SECURITY_2_KEX_SET_SCHEME_POS]) && (mp_context->curve_support == mp_context->buf[SECURITY_2_KEX_SET_CURVE_POS]) && (mp_context->key_granted == mp_context->buf[SECURITY_2_KEX_SET_KEYS_POS])
...
static void s2_kex_rep_recv(void)
{
    ...
    memcpy(mp_context->kex_report_echo_payload, &mp_context->buf[SECURITY_2_KEX_REP_ECHO_POS], sizeof(mp_context->kex_report_echo_payload));
...
static void s2_send_pub_key_a(void)
{
    ...
    memcpy(s2_event->evt.s2_event.s2_data.challenge_req.public_key, &mp_context->buf[SECURITY_2_PUB_KEY_KEY_POS], sizeof(public_key_t));
    ...
    if ((mp_context->key_granted & (SECURITY_2_SECURITY_2_CLASS_1 | SECURITY_2_SECURITY_2_CLASS_2)) && (mp_context->inclusion_mode == INCLUSION_MODE_SSA)) {
    ...
    memcpy(mp_context->public_key, &mp_context->buf[SECURITY_2_PUB_KEY_KEY_POS], sizeof(public_key_t));
    ...
    if ((mp_context->inclusion_mode == INCLUSION_MODE_CSA) && (mp_context->key_granted & ~(KEY_CLASS_S0 | KEY_CLASS_S2_UNAUTHENTICATED))) {
...
static void s2_send_echo_kex_report(void)
{
    ...
    mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_ECHO_POS]   = mp_context->kex_report_echo_payload[0] | SECURITY_2_ECHO_ON;
    mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_SCHEME_POS] = mp_context->kex_report_echo_payload[1];
    mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_CURVE_POS]  = mp_context->kex_report_echo_payload[2];
    mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_KEYS_POS]   = mp_context->kex_report_echo_payload[3];
...
static void s2_send_net_key_report(void)
{
    ...
    uint8_t keyslot_to_read = translate_keyslot_to_read_for_LR(mp_context->u.inclusion_buf[SECURITY_2_NET_KEY_REP_GRANT_KEY_POS], mp_context->inclusion_peer.r_node);
...
void s2_inclusion_including_start(struct S2 *p_context, const s2_connection_t *p_peer)
{
    ...
    if (mp_context->inclusion_state == S2_INC_IDLE) {
        mp_context->inclusion_peer            = *p_peer;
        mp_context->inclusion_peer.tx_options = 0;
