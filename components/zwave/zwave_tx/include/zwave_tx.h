...
#ifndef ZWAVE_TX_QUEUE_MAX_FRAMES_PER_NODE
// Maximum number of singlecast frames (including S2 transport children) that
// a single remote NodeID may hold in the TX queue simultaneously.  Capping
// per-node occupancy prevents a slow or misbehaving node from monopolising
// all queue slots and starving every other node.
#define ZWAVE_TX_QUEUE_MAX_FRAMES_PER_NODE (ZWAVE_TX_QUEUE_BUFFER_SIZE / 8)
#endif
