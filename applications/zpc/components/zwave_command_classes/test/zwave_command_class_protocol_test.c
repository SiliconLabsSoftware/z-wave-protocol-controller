  ...
  zwave_command_handler_t new_command_class_handler, int cmock_num_calls)
{
  ...
  TEST_ASSERT_TRUE(ZWAVE_CMD_CLASS_PROTOCOL == protocol_handler.command_class
                   || ZWAVE_CMD_CLASS_PROTOCOL_LR
                        == protocol_handler.command_class);
...
void test_zwave_command_class_protocol_handler(void)
{
  ...
  zwapi_transfer_protocol_cc_ExpectAndReturn(
    2,
    ZWAVE_CONTROLLER_S2_AUTHENTICATED_KEY,
    5,
...
void test_zwave_s2_on_protocol_cc_encryption_request_happy_case()
{
  ...
  uint8_t protocol_metadata_length
    = sizeof(protocol_metadata) / sizeof(protocol_metadata[0]);
  ...
  zwave_tx_scheme_get_node_tx_options_Expect(ZWAVE_TX_QOS_MAX_PRIORITY,
                                             1,
                                             5000,
                                             NULL);
  ...
  uint16_t supervision_frame_size
    = 4
      + payload_length;  // sizeof(frame) - SUPERVISION_ENCAPSULATED_COMMAND_MAXIMUM_SIZE
  tx_options.number_of_responses
    += 1;  // supervision get expects 1 frame in response
