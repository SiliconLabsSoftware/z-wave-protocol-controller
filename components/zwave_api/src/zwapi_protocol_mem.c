...
sl_status_t zwapi_nvr_get_value(uint8_t offset, uint8_t bLength, uint8_t *pNVRValue)
{
    ...
    sl_status_t send_command_status             = zwapi_send_command_with_response(FUNC_ID_NVR_GET_VALUE, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_memory_get_ids(zwave_home_id_t *home_id, zwave_node_id_t *node_id)
{
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_send_command_with_response(FUNC_ID_MEMORY_GET_ID, NULL, 0, response_buffer, &response_length);
    ...
    if (send_command_status == SL_STATUS_OK && response_length > (IDX_DATA + 4)) {
        ...
        memcpy(&received_home_id, &response_buffer[IDX_DATA], sizeof(received_home_id));
...
sl_status_t zwapi_memory_get_byte(uint16_t offset, uint8_t *byte_value)
{
    ...
    sl_status_t send_command_status             = zwapi_send_command_with_response(FUNC_ID_MEMORY_GET_BYTE, request_buffer, index, response_buffer, &response_length);
...
uint8_t zwapi_memory_put_byte(uint16_t offset, uint8_t bData)
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_MEMORY_PUT_BYTE, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_memory_get_buffer(uint16_t offset, uint8_t *buf, uint8_t length)
{
    if (zwapi_support_command_func(FUNC_ID_MEMORY_GET_BUFFER)) {
        ...
        sl_status_t send_frame_status = zwapi_session_send_frame_with_response(FUNC_ID_MEMORY_GET_BUFFER, request_buffer, index, response_buffer, &response_length);
...
uint8_t zwapi_memory_put_buffer(uint16_t offset, const uint8_t *buf, uint16_t length, void (*callback_function)(void))
{
    ...
    if (zwapi_support_command_func(FUNC_ID_MEMORY_PUT_BUFFER) && !function_buffer_too_small) {
        uint8_t func_id;
        uint8_t response_length                     = 0;
        uint8_t index                               = 0;
        uint8_t request_buffer[REQUEST_BUFFER_SIZE] = {0};
        ...
        func_id                 = (callback_function == NULL ? 0 : 3);  // FIXME: WHy 3 here ? I think it should only be non-zero
        request_buffer[index++] = (offset) >> 8;
        request_buffer[index++] = (offset) & 0xFF;
        ...
        // We already know that the command is supported here, so we call
        // zwapi_session_send_frame_with_response() instead of zwapi_send_command_with_response()
        sl_status_t send_frame_status = zwapi_session_send_frame_with_response(FUNC_ID_MEMORY_PUT_BUFFER, request_buffer, index, response_buffer, &response_length);
    ...
    }  // Else resort to copy byte-by-byte
    for (uint16_t i = 0; i < length; i++) {
...
uint8_t zwapi_memory_put_buffer_blocking(uint16_t offset, uint8_t *buffer, uint16_t length, uint16_t timeout)
{
    ...
    uint8_t return_value = zwapi_memory_put_buffer(offset, buffer, length, zwapi_memory_put_buffer_builtin_callback);

    while (!zwapi_is_timestamp_elapsed(&callback_timeout) && zwapi_memory_put_buffer_builtin_callback_called == false) {
    }
