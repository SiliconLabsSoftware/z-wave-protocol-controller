...
static bool create_path(std::vector<BranchNode> *branch_data, attribute branch_root)
{
    ...
    if (branch_root) {
        if (branch_root.is_valid()) {
            ...
            std::cout << "Error, unable to locate root node 0x" << std::hex << branch_root;
            return false;
        }
    } else {
    ...
    for (std::vector<BranchNode>::iterator vec_it = branch_data_begin; vec_it != branch_data_end; vec_it++) {
        ...
        if (vec_it->is_parent) {
            node = parent.child_by_type_and_value<uint8_t>(vec_it->node_type, vec_it->node_value);
            if (!node.is_valid()) {
...
static bool validate_path(std::vector<BranchNode> *branch_data, attribute branch_root)
{
    attribute branch_parent, parent, node;
    ...
    for (std::vector<BranchNode>::iterator vec_it = branch_data_begin; vec_it != branch_data_end; vec_it++) {
        ...
        if (vec_it->is_parent) {
            node = parent.child_by_type_and_value<uint8_t>(vec_it->node_type, vec_it->node_value);
            if (!node.is_valid()) {
                std::cout << " :Error, unable to locate node 0x" << std::hex << vec_it->node_type;
            ...
            if (!node.is_valid()) {
                std::cout << " :Error, unable to locate node 0x" << std::hex << vec_it->node_type << std::endl;
                return false;
            }
        }
        vec_it->attr = node;
...
static bool parse_branch_descriptor(std::string &branch_descriptor, std::vector<BranchNode> *branch_data)
{
    // Split branch_descriptor to tokens, separated by '.' and add to vector
    const std::regex re_separator {"\\."};
    std::vector<std::string> node_list {std::sregex_token_iterator(branch_descriptor.cbegin(), branch_descriptor.cend(), re_separator, -1), std::sregex_token_iterator()};

    BranchNode branch_node = {};

    // Iterate over node_list to identify value in []
    for (std::vector<std::string>::iterator it = std::begin(node_list); it != std::end(node_list); ++it) {
        std::string node = *it;
        ...
        if (open_bracket == std::string::npos && close_bracket == std::string::npos) {
            ...
            if (open_bracket != std::string::npos && close_bracket != std::string::npos) {
                std::string s_node_value = node.substr(open_bracket + 1, close_bracket - open_bracket - 1);
