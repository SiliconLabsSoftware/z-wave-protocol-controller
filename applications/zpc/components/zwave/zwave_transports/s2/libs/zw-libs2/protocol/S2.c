...
#pragma GCC diagnostic ignored \
  "-Wunused-function"  //TODO: Remove after merging in complete multicast support
#endif
...
static void S2_set_peer(struct S2 *p_context,
...
static void S2_send_nonce_report(struct S2 *p_context,
                                 const s2_connection_t *conn,
                                 uint8_t flags);
static int S2_is_peernode(struct S2 *p_context, const s2_connection_t *peer);
#ifdef ZW_CONTROLLER
static void S2_send_nls_state_set(struct S2 *p_context,
                                  s2_connection_t *con,
                                  bool nls_active);
...
static void S2_send_nls_node_list_get(struct S2 *p_context,
                                      s2_connection_t *con,
                                      bool request);
static void S2_prepare_nls_node_list_report(uint8_t *buf,
                                            bool is_last_node,
                                            uint16_t node_id,
                                            uint8_t granted_keys,
                                            uint8_t nls_state);
static void S2_send_nls_node_list_report(struct S2 *p_context,
                                         s2_connection_t *con,
                                         bool is_last_node,
                                         uint16_t node_id,
                                         uint8_t granted_keys,
                                         uint8_t nls_state);
#endif /* ZW_CONTROLLER */
static void S2_send_nls_state_report(struct S2 *p_context,
                                     s2_connection_t *con);
static void S2_command_handler(struct S2 *p_context,
                               s2_connection_t *src,
                               uint8_t *cmd,
                               uint16_t cmd_length);
...
static decrypt_return_code_t S2_decrypt_msg(struct S2 *p_context,
                                            s2_connection_t *conn,
                                            uint8_t *msg,
                                            uint16_t msg_len,
                                            uint8_t **plain_text,
                                            uint16_t *plain_text_len);
static struct SPAN *find_span_by_node(struct S2 *p_context,
                                      const s2_connection_t *con);
static int S2_make_aad(struct S2 *p_context,
                       node_t sender,
                       node_t receiver,
                       uint8_t *msg,
                       uint16_t hdr_len,
 ...
 */
static uint8_t S2_send_data_all_cast(struct S2 *p_context,
                                     const s2_connection_t *con,
                                     const uint8_t *buf,
                                     uint16_t len,
                                     event_t ev);
 ...
 * we use a new entry.
 */
static struct MPAN *find_mpan_by_group_id(struct S2 *p_context,
                                          node_t owner_id,
                                          uint8_t group_id,
                                          uint8_t create_new)
{
  ...
  for (i = 0; i < MPAN_TABLE_SIZE; i++) {
    if ((ctxt->mpan_table[i].state != MPAN_NOT_USED)
        ...
        && ((1 << ctxt->mpan_table[i].class_id) & ctxt->loaded_keys)) {
  ...
  ctxt->mpan_table[i].group_id = group_id;
  ctxt->mpan_table[i].owner_id = owner_id;
  ctxt->mpan_table[i].class_id
    = ctxt->peer.class_id;  //Here we assume that peer is set...
...
static int S2_span_ok(struct S2 *p_context, const s2_connection_t *con)
{
  ...
  if (span) {
    return ((span->state == SPAN_NEGOTIATED)
...
void S2_encrypt_and_send(struct S2 *p_context)
{
  ...
  uint16_t aad_len;
  uint8_t
    ei_sender[RANDLEN];  //Note we are actually only using the first 16 bytes
  ...
  if (span->state == SPAN_SOS_REMOTE_NONCE) {
    ...
    next_nonce_instantiate(&span->d.rng,
                           ei_sender,
                           ei_receiver,
                           ctxt->sg[ctxt->peer.class_id].nonce_key);

    span->class_id = ctxt->peer.class_id;
    span->state
      = SPAN_NEGOTIATED;  //TODO is it better to set this on send_data complete?
  ...
  if ((ctxt->peer.tx_options
    ...
    if ((ctxt->peer.tx_options & S2_TXOPTION_FIRST_SINGLECAST_FOLLOWUP)
        && ctxt->retry == 2) {
  ...
  aad_len
    = (uint16_t)S2_make_aad(ctxt,
                            ctxt->peer.l_node,
                            ctxt->peer.r_node,
                            msg,
                            hdr_len,
                            ctxt->length + shdr_len + hdr_len + AUTH_TAG_LEN,
                            aad,
                            sizeof(aad));
...
#else
  msg_len
    = (uint16_t)CCM_encrypt_and_auth(ctxt->sg[ctxt->peer.class_id].enc_key,
                                     nonce,
                                     aad,
                                     aad_len,
                                     ciphertext,
...
void S2_encrypt_and_send_multi(struct S2 *p_context)
{
  ...
  aad_len = (uint16_t)S2_make_aad(ctxt,
                                  ctxt->peer.l_node,
                                  ctxt->peer.r_node,
                                  msg,
                                  hdr_len,
                                  ctxt->length + hdr_len + AUTH_TAG_LEN,
                                  aad,
                                  sizeof(aad));
  ...
  /* Import key into secure vault */
  zw_wrap_aes_key_secure_vault(&key_id,
                               ctxt->sg[ctxt->mpan->class_id].mpan_key,
                               ZW_PSA_ALG_ECB_NO_PAD);
  zw_psa_aes_ecb_encrypt(key_id, ctxt->mpan->inner_state, nonce);
  /* Remove key from vault */
  zw_psa_destroy_key(key_id);
#else
  AES128_ECB_encrypt(ctxt->mpan->inner_state,
                     ctxt->sg[ctxt->mpan->class_id].mpan_key,
                     nonce);
#endif
  ...
  if (ctxt->is_keys_restored == false) {
    /* Import key into secure vault */
    zw_wrap_aes_key_secure_vault(&key_id,
                                 ctxt->sg[ctxt->mpan->class_id].enc_key,
                                 ZW_PSA_ALG_CCM);
  } else {
    /* Use secure vault for encryption using PSA APIs */
    key_id = convert_keyclass_to_derived_key_id(
      convert_key_slot_to_keyid(ctxt->mpan->class_id),
      ZWAVE_KEY_TYPE_SINGLE_CAST);
  }
  zw_psa_aead_encrypt_ccm(key_id,
                          nonce,
                          ZWAVE_PSA_AES_NONCE_LENGTH,
                          aad,
                          aad_len,
                          ...
                          ctxt->length,
                          ciphertext,
                          ctxt->length + ZWAVE_PSA_AES_MAC_LENGTH,
                          &out_len);
  msg_len = out_len;
...
#else
  msg_len
    = (uint16_t)CCM_encrypt_and_auth(ctxt->sg[ctxt->mpan->class_id].enc_key,
                                     nonce,
                                     aad,
                                     aad_len,
                                     ciphertext,
...
void S2_send_frame_done_notify(struct S2 *p_context,
...
static uint8_t S2_is_mos(struct S2 *p_context, node_t node_id, uint8_t clear)
{
  ...
  uint8_t i;
  for (i = 0; i < MPAN_TABLE_SIZE; i++) {
    if ((ctxt->mpan_table[i].owner_id == node_id)
        && (ctxt->mpan_table[i].state == MPAN_MOS)) {
      if (clear) {
        ctxt->mpan_table[i].state = MPAN_NOT_USED;
      }
      return 1;
                                 ...
                                 uint8_t flags)
{
  ...
  S2_send_frame_no_cb(
    ...
    conn,
    nonce_report,
    nonce_report[3] & SECURITY_2_NONCE_REPORT_PROPERTIES1_SOS_BIT_MASK ? 20
                                                                       : 4);
}
  ...
  S2_register_nonce(struct S2 *p_context, const uint8_t *buf, uint16_t len)
{
  ...
  if (len >= (4 + 16)
      && (buf[3] & SECURITY_2_NONCE_REPORT_PROPERTIES1_SOS_BIT_MASK)) {
                       ...
                       uint16_t max_size)
{
  ...
  if ((LOWEST_LONG_RANGE_NODE_ID <= sender)
      || (LOWEST_LONG_RANGE_NODE_ID <= receiver)) {
                                            ...
                                            uint16_t *plain_text_len)
{
  ...
  *plain_text_len = 0;
    ...
    do {
      ...
      switch (ext_data[1] & S2_MSG_EXTHDR_TYPE_MASK) {
        case S2_MSG_EXTHDR_TYPE_SN:
          ...
          if (span && span->state == SPAN_SOS_LOCAL_NONCE
              && (ext_len
                  == (2 + sizeof(span->d.r_nonce)))) { /*Save the nonces */
            ...
            next_nonce_instantiate(&span->d.rng,
                                   s_nonce,
                                   r_nonce,
                                   ctxt->sg[span->class_id].nonce_key);
          }
          break;
        case S2_MSG_EXTHDR_TYPE_MGRP:
          ...
          mpan = find_mpan_by_group_id(
            ctxt,
            conn->r_node,
            ext_data[2],
            (conn->rx_options & S2_RXOPTION_MULTICAST) == 0);
          break;
        ...
        default:
          if (ext_data[1]
              & S2_MSG_EXTHDR_CRITICAL_FLAG) {  //Unsupported critical option
            goto parse_fail;
          }
      }
  ...
  if (span) {
    ...
    for (i = 0; i < N_SEC_CLASS; i++) {
      ...
      if (ctxt->loaded_keys & (1 << span->class_id)) {
        ...
        if (ctxt->is_keys_restored == false) {
          /* Import key into secure vault */
          zw_wrap_aes_key_secure_vault(&ccm_key_id,
                                       ctxt->sg[span->class_id].enc_key,
                                       ZW_PSA_ALG_CCM);
        } else {
          /* Use secure vault for encryption using PSA APIs */
          ccm_key_id = convert_keyclass_to_derived_key_id(
            convert_key_slot_to_keyid(span->class_id),
            ZWAVE_KEY_TYPE_SINGLE_CAST);
        }
        status
          = zw_psa_aead_decrypt_ccm(ccm_key_id,
                                    nonce,
                                    ZWAVE_PSA_AES_NONCE_LENGTH,
                                    aad,
                                    aad_len,
                                    ...
                                    ciphertext_len,
                                    ciphertext,
                                    ciphertext_len + ZWAVE_PSA_AES_MAC_LENGTH,
                                    &out_len);
...
#else
        decrypt_len = CCM_decrypt_and_auth(ctxt->sg[span->class_id].enc_key,
                                           nonce,
                                           aad,
                                           aad_len,
                                           ciphertext,
        ...
        if (decrypt_len) {
          ...
          if (
            mpan) {  //This means that a MGRP extension was included in the message
    ...
    zw_wrap_aes_key_secure_vault(&key_id,
                                 ctxt->sg[mpan->class_id].mpan_key,
                                 ZW_PSA_ALG_ECB_NO_PAD);
    /* Import key into secure vault */
    ...
    zw_psa_destroy_key(key_id);
#else
    AES128_ECB_encrypt(mpan->inner_state,
                       ctxt->sg[mpan->class_id].mpan_key,
                       nonce);
#endif
    next_mpan_state(mpan);
    ...
    /* Import key into secure vault */
    zw_wrap_aes_key_secure_vault(&key_id,
                                 ctxt->sg[mpan->class_id].enc_key,
                                 ZW_PSA_ALG_CCM);
    /* Use secure vault for decryption using PSA APIs */
    status = zw_psa_aead_decrypt_ccm(key_id,
                                     nonce,
                                     ZWAVE_PSA_AES_NONCE_LENGTH,
                                     aad,
                                     aad_len,
    ...
    zw_psa_destroy_key(key_id);
#else
    decrypt_len = CCM_decrypt_and_auth(ctxt->sg[mpan->class_id].enc_key,
                                       nonce,
                                       aad,
                                       aad_len,
                                       ciphertext,
...
auth_fail:
  ...
  if ((conn->rx_options & S2_RXOPTION_MULTICAST) == 0) {
    S2_send_nonce_report(
      ctxt,
      conn,
      mpan ? (SECURITY_2_NONCE_REPORT_PROPERTIES1_SOS_BIT_MASK
 ...
 *
 */
uint8_t S2_send_data(struct S2 *p_context,
                     s2_connection_t *dst,
                     const uint8_t *buf,
                     uint16_t len)
{
...
uint8_t S2_network_key_update(struct S2 *p_context,
                              uint32_t key_id,
                              ...
                              __attribute__((unused)) bool make_keys_persist_se)
{
  ...
  if (temp_key_expand) {
    tempkey_expand(key_id,
                   net_key,
                   ctxt->sg[class_id].enc_key,
                   ctxt->sg[class_id].nonce_key,
                   ctxt->sg[class_id].mpan_key);
  } else {
    networkkey_expand(key_id,
                      net_key,
                      ctxt->sg[class_id].enc_key,
                      ctxt->sg[class_id].nonce_key,
                      ctxt->sg[class_id].mpan_key);
    ...
    if (make_keys_persist_se) {
      uint32_t ccm_key_id = convert_keyclass_to_derived_key_id(
        convert_key_slot_to_keyid(class_id),
        ZWAVE_KEY_TYPE_SINGLE_CAST);
      assert((ccm_key_id >= ZWAVE_PSA_KEY_ID_MIN)
             && (ccm_key_id <= ZWAVE_PSA_KEY_ID_MAX));
      zw_wrap_aes_key_secure_vault(&ccm_key_id,
                                   ctxt->sg[class_id].enc_key,
                                   ZW_PSA_ALG_CCM);
...
void S2_application_command_handler(struct S2 *p_context,
                                    s2_connection_t *src,
                                    uint8_t *buf,
                                    uint16_t len)
{
  ...
  switch (buf[1]) {
    case SECURITY_2_NONCE_GET:
      if ((src->rx_options & S2_RXOPTION_MULTICAST) != S2_RXOPTION_MULTICAST
          && ((len >= SECURITY_2_NONCE_GET_LENGTH)
              && S2_verify_seq(ctxt, src, buf[2]))) {
        S2_send_nonce_report(ctxt,
                             src,
                             SECURITY_2_NONCE_REPORT_PROPERTIES1_SOS_BIT_MASK);
      }
      break;
  ...
  if (cmd[0] == COMMAND_CLASS_SECURITY_2
    ...
    switch (cmd[1]) {
      case SECURITY_2_COMMANDS_SUPPORTED_GET_V2:
        ...
        ctxt->u.commands_sup_report_buf[1]
          = SECURITY_2_COMMANDS_SUPPORTED_REPORT;

        S2_get_commands_supported(src->l_node,
                                  src->class_id,
                                  &classes,
                                  &n_commands_supported);

        if (n_commands_supported + 2
            > sizeof(ctxt->u.commands_sup_report_buf)) {
          return;
        }
        memcpy(&ctxt->u.commands_sup_report_buf[2],
               classes,
               n_commands_supported);
        /*TODO If ctxt->fsm is busy the report is not going to be sent*/
        S2_send_data(ctxt,
                     src,
                     ctxt->u.commands_sup_report_buf,
                     n_commands_supported + 2);
        break;
      ...
      case NLS_STATE_REPORT_V2:
        if (cmd_length == SECURITY_2_V2_NLS_STATE_REPORT_LENGTH) {
          bool capability = (cmd[SECURITY_2_V2_NLS_STATE_REPORT_BITFIELD_POS]
                             & SECURITY_2_V2_NLS_STATE_REPORT_CAPABILITY_FIELD)
                              ? true
                              : false;
          bool state      = (cmd[SECURITY_2_V2_NLS_STATE_REPORT_BITFIELD_POS]
                        & SECURITY_2_V2_NLS_STATE_REPORT_STATE_FIELD)
                              ? true
                              : false;

          if (state) {
            ctxt->nls_state
              = 1;  // There is at least one an NLS-enabled node in the network
          }
          S2_save_nls_state();

          S2_notify_nls_state_report(src->r_node,
                                     src->class_id,
                                     capability,
                                     state);
        }
      ...
      case NLS_NODE_LIST_GET_V2:
        if (cmd_length == SECURITY_2_V2_NLS_NODE_LIST_GET_LENGTH) {
          ...
          uint8_t retval;
          bool request = (cmd[SECURITY_2_V2_NLS_NODE_LIST_GET_REQUEST_POS] == 1)
                           ? true
                           : false;

          retval = S2_get_nls_node_list(src->r_node,
                                        request,
                                        &is_last_node,
                                        &node_id,
                                        &granted_keys,
          ...
          if (nls_state && retval == 0) {
            if (S2_is_send_data_busy(ctxt)) {
              ...
              S2_send_nls_node_list_report(ctxt,
                                           src,
                                           is_last_node,
                                           node_id,
                                           granted_keys,
          ...
          bool is_last_node
            = (cmd[SECURITY_2_V2_NLS_NODE_LIST_REPORT_LAST_NODE_POS]
               & SECURITY_2_V2_NLS_NODE_LIST_REPORT_LAST_NODE_FIELD)
                ...
                : false;
          bool nls_state
            = (cmd[SECURITY_2_V2_NLS_NODE_LIST_REPORT_NLS_STATE_POS] != 0)
                ? true
                : false;

          int8_t retval = S2_notify_nls_node_list_report(
            src->r_node,
            (uint16_t)(cmd[SECURITY_2_V2_NLS_NODE_LIST_REPORT_NODE_ID_MSB_POS]
                         << 8
                       | cmd
                         [SECURITY_2_V2_NLS_NODE_LIST_REPORT_NODE_ID_LSB_POS]),
...
static void S2_post_send_done_event(struct S2 *p_context, s2_tx_status_t status)
{
  ...
  s2_inclusion_send_done(ctxt,
                         (status == S2_TRANSMIT_COMPLETE_OK)
                           || (status == S2_TRANSMIT_COMPLETE_VERIFIED));
  S2_send_done_event(ctxt, status);
}

static void emit_S2_synchronization_event(sos_event_reason_t reason,
                                          event_data_t *d)
{
  S2_resynchronization_event(d->con->r_node,
                             reason,
                             d->d.buf.buffer[2],
                             d->con->l_node);
}
...
void S2_fsm_post_event(struct S2 *p_context, event_t e, event_data_t *d)
{
  ...
  switch (ctxt->fsm) {
    ...
    case IDLE:
      ...
      } else if (e == GOT_NONCE_GET && (d->d.buf.len >= 3)
                 && S2_verify_seq(ctxt, d->con, d->d.buf.buffer[2])) {
        S2_send_nonce_report(ctxt,
                             d->con,
                             SECURITY_2_NONCE_REPORT_PROPERTIES1_SOS_BIT_MASK);
    ...
    case WAIT_NONCE_RAPORT:
      if ((e == SEND_DONE) && (d->d.tx.status == S2_TRANSMIT_COMPLETE_NO_ACK)) {
        ...
        S2_post_send_done_event(
          ctxt,
          d->d.tx
            .status);  // forward status to upper layer, S2_TRANSMIT_COMPLETE_FAIL happens only on timeout
      } else if ((e == SEND_DONE)
                 && (d->d.tx.status == S2_TRANSMIT_COMPLETE_OK)) {
        S2_set_timeout(
          ctxt,
          d->d.tx.time);  //Just shorten timer but stay in this state
        ...
        if (S2_register_nonce(ctxt, d->d.buf.buffer, d->d.buf.len)
            & SECURITY_2_NONCE_REPORT_PROPERTIES1_SOS_BIT_MASK) {
          goto send_msg_state_enter;
        }
      } else if ((e == GOT_NONCE_REPORT) && !S2_is_peernode(ctxt, d->con)) {
    ...
    case SENDING_MSG:
      if (e == SEND_DONE) {
        ...
        S2_post_send_done_event(ctxt, d->d.tx.status);
#ifdef ZW_CONTROLLER
        if (ctxt->delayed_transmission_flags.send_nls_node_list_get
            && ctxt->nls_state) {
        ...
        if (ctxt->delayed_transmission_flags.send_nls_node_list_report
            && ctxt->nls_state) {
    ...
    else if (e == GOT_BAD_ENC_MSG && S2_is_peernode(ctxt, d->con))
    {
      ...
      S2_post_send_done_event(ctxt, S2_TRANSMIT_COMPLETE_FAIL);
        ...
        ctxt->fsm
          = IDLE;  //The frame seems to be handled but we don't know for sure
...
uint8_t S2_send_data_multicast(struct S2 *p_context,
                               const s2_connection_t *con,
                               const uint8_t *buf,
                               uint16_t len)
{
                                     ...
                                     event_t ev)
{
  ...
  if (len == 0 || len > WORKBUF_SIZE || buf == 0
      || S2_is_send_data_busy(ctxt)) {
    return 0;
  }
  ...
  if (ev == SEND_MULTICAST) {
    ...
    struct MPAN *new_mpan
      = find_mpan_by_group_id(ctxt, 0, con->mgrp_group_id, 0);
    if (new_mpan) {
      ctxt->mpan = new_mpan;
    }
...
static void S2_send_nls_state_report(struct S2 *p_context, s2_connection_t *con)
{
  ...
  nls_bitfield
    = SECURITY_2_V2_NLS_STATE_REPORT_CAPABILITY_FIELD
      | (ctxt->nls_state
           ? SECURITY_2_V2_NLS_STATE_REPORT_STATE_FIELD
           : 0);  // A node sending this frame will always support NLS
                                            ...
                                            uint8_t nls_state)
{
  ...
  buf[SECURITY_2_V2_NLS_NODE_LIST_REPORT_NODE_ID_MSB_POS]
    = (node_id >> 8) & 0xFF;
  buf[SECURITY_2_V2_NLS_NODE_LIST_REPORT_NODE_ID_LSB_POS]
    = (node_id >> 0) & 0xFF;
