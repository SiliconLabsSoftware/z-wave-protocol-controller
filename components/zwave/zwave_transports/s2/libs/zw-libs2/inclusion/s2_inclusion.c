    ...
    for (uint8_t i = 0; i < table_length; i++) {
        if (mp_context->inclusion_state == table[i].state || S2_INC_STATE_ANY == table[i].state) {
            if ((event == table[i].event) || (S2_EVT_ANY == table[i].event)) {
                ...
                if ((S2_INC_STATE_ANY != table[i].new_state) && (mp_context->inclusion_state != table[i].new_state)) {
...
{
    if ((m_schemes == mp_context->buf[SECURITY_2_KEX_REP_SCHEME_POS]) && (m_curves == mp_context->buf[SECURITY_2_KEX_REP_CURVE_POS]) && (m_keys == mp_context->buf[SECURITY_2_KEX_REP_KEYS_POS])
...
static void s2_inclusion_post_event_internal(struct S2 *p_context)
{
    ...
    if ((ELEM_COUNT(m_frame_rules) < event_rule_index) || (mp_context->inclusion_state == S2_INC_IDLE)) {
    ...
    if ((ECHO_SUPPORT == m_frame_rules[event_rule_index][ENCRYPTION_SUPPORT_INDEX]) && (mp_context->buf[SECURITY_2_KEX_SET_ECHO_POS] & SECURITY_2_ECHO_ON)) {
    ...
    if (NETWORK_KEY_SECURE == crypt_rule) {
        ...
        if ((DUAL_RULE_SUPPORT != m_frame_rules[event_rule_index][ENCRYPTION_SUPPORT_INDEX]) || (mp_context->inclusion_peer.class_id != m_frame_rules[event_rule_index][ENCRYPTION_RULE_INDEX_B])) {
            inclusion_failed_frame_send(KEX_FAIL_AUTH, NON_SECURE);
            process_event(S2_INCLUSION_ERROR_SENT);
            return;
        }
...
uint8_t s2_inclusion_init(uint8_t schemes, uint8_t curves, uint8_t keys_to_request)
{
    ...
    CHECK_AND_FAIL(((~SECURITY_2_KEY_MASK & keys_to_request) || ((SECURITY_2_KEY_MASK & keys_to_request) == 0)), KEX_FAIL_KEX_KEY);
...
static void s2_send_pub_key_b(void)
{
    ...
    if ((mp_context->inclusion_mode == INCLUSION_MODE_SSA) && (mp_context->key_granted & ~(KEY_CLASS_S0 | KEY_CLASS_S2_UNAUTHENTICATED))) {
...
static void s2_pub_key_a_recv(void)
{
    ...
    if ((mp_context->key_granted & (SECURITY_2_SECURITY_2_CLASS_1 | SECURITY_2_SECURITY_2_CLASS_2)) && (mp_context->inclusion_mode == INCLUSION_MODE_CSA)) {
...
static void s2_send_net_key_get(void)
{
    ...
    if ((KEX_REPORT == mp_context->buf[SECURITY_2_COMMAND_POS]) && (validate_echo_kex_report() != 0)) {
        process_event(S2_INCLUSION_ERROR_SENT);
        return;
    }
    ...
    if ((SECURITY_2_TRANSFER_END == mp_context->buf[SECURITY_2_COMMAND_POS]) && (SECURITY_2_KEY_VERIFIED != (mp_context->buf[SECURITY_2_TRANSFER_END_FLAGS_POS] & (SECURITY_2_KEY_REQ_COMPLETE | SECURITY_2_KEY_VERIFIED)))) {
...
void s2_inclusion_send_frame(void)
{
    if (0 == S2_send_frame(mp_context, &mp_context->inclusion_peer, mp_context->u.inclusion_buf, mp_context->inclusion_buf_length)) {
...
void s2_inclusion_send_data(void)
{
    if (0 == S2_send_data(mp_context, &mp_context->inclusion_peer, mp_context->u.inclusion_buf, mp_context->inclusion_buf_length)) {
        m_retry_counter = QUEUE_FULL;
    } else {
        m_retry_counter--;
    }
