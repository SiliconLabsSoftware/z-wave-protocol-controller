 ...
 */
#define STORE_ATTRIBUTE(node) datastore_store_attribute(node->id, node->type, node->parent_node->id, node->reported_value.data(), node->reported_value.size(), node->desired_value.data(), node->desired_value.size())

/**
 * @brief Saves the root node in the datastore.
 */
#define STORE_ROOT_ATTRIBUTE(root_node) datastore_store_attribute(root_node->id, root_node->type, ATTRIBUTE_STORE_NULL_ID, root_node->reported_value.data(), root_node->reported_value.size(), root_node->desired_value.data(), root_node->desired_value.size())
