...
static pthread_mutex_t s2_transport_mutex;
static pthread_once_t s2_transport_mutex_once = PTHREAD_ONCE_INIT;

static void s2_transport_mutex_init_once(void)
{
    pthread_mutexattr_t attr;
    pthread_mutexattr_init(&attr);
    pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE);
    pthread_mutex_init(&s2_transport_mutex, &attr);
    pthread_mutexattr_destroy(&attr);
}

void zwave_s2_transport_lock(void)
{
    pthread_once(&s2_transport_mutex_once, s2_transport_mutex_init_once);
    pthread_mutex_lock(&s2_transport_mutex);
}

void zwave_s2_transport_unlock(void)
{
    pthread_mutex_unlock(&s2_transport_mutex);
}

bool zwave_s2_transport_is_busy(void)
{
    bool busy = false;

    zwave_s2_transport_lock();
    // Active application S2 send session only. Do not use S2_is_busy() here:
    // inclusion bootstrap also sets that, which blocks TX backoff preemption and
    // strands inclusion egress frames until their discard timer expires.
    busy = (state.s2_send_callback != NULL);
...
void zwave_s2_transport_set_inclusion_in_progress(bool active)
{
    ...
    if (active && state.s2_send_callback != NULL) {
        ...
        } while (S2_is_send_data_busy(s2_ctx));
        state.abort_in_progress = false;
    }
    if (active) {
...
static bool s2_is_inclusion_bootstrap_egress(void)
{
    return state.inclusion_in_progress;
}
...
static zwave_s2_keyset_t get_s2_keyset_from_node(zwave_node_id_t node_id)
{
    ...
    zwave_protocol_t node_protocol = zwave_controller_storage_inclusion_protocol(node_id);
    switch (node_protocol) {
        ...
        case PROTOCOL_ZWAVE_LONG_RANGE:
            ...
            break;
        default:
            sl_log_error(LOG_TAG, "Unknown keyset for NodeID %d. S2 transmission will fail.", node_id);
...
static zwave_s2_keyset_t get_s2_keyset_from_group(zwave_multicast_group_id_t group_id)
{
    ...
    // No node in the mask ?
    sl_log_warning(LOG_TAG, "Unknown keyset for GroupID %d. S2 transmission will fail.", group_id);
    return UNKNOWN_KEYSET;
}
...
static void send_frame_callback(uint8_t status, const zwapi_tx_report_t *tx_info, void *user)
{
    ...
    S2_send_frame_done_notify(s2_ctx, status == TRANSMIT_COMPLETE_OK ? S2_TRANSMIT_COMPLETE_OK : S2_TRANSMIT_COMPLETE_NO_ACK, tx_time);
    zwave_s2_transport_unlock();
}
...
void S2_get_commands_supported(node_t lnode, uint8_t class_id, const uint8_t **cmdClasses, uint8_t *length)
{
    ...
    uint8_t assigned_keys = zwave_s2_keystore_get_assigned_keys() & SECURITY_2_KEY_MASK & ~SECURITY_2_SECURITY_0_NETWORK_KEY;
    sl_log_info(LOG_TAG,
...
void S2_send_done_event(struct S2 *ctxt, s2_tx_status_t status)
{
    ...
    const uint8_t s2zw_codes[]                 = {TRANSMIT_COMPLETE_OK, TRANSMIT_COMPLETE_NO_ACK, TRANSMIT_COMPLETE_FAIL, 0, 0, TRANSMIT_COMPLETE_VERIFIED};
    on_zwave_tx_send_data_complete_t cb_save   = NULL;
    void *user_save                            = NULL;
    zwapi_tx_report_t tx_status_for_completion = {};
    ...
    if (cb_save) {
        cb_save(s2zw_codes[status], &tx_status_for_completion, user_save);
    }
}
...
uint8_t S2_send_frame(struct S2 *ctxt, const s2_connection_t *conn, uint8_t *buf, uint16_t len)
{
    ...
    if (len > COMMAND_INDEX && buf[COMMAND_INDEX] == SECURITY_2_NONCE_GET) {
        options.number_of_responses = 1;
    } else if ((len > S2_NONCE_REPORT_MOS_SOS_INDEX) && (buf[COMMAND_INDEX] == SECURITY_2_NONCE_REPORT) && (buf[S2_NONCE_REPORT_MOS_SOS_INDEX] & S2_NONCE_REPORT_MOS_SOS_MASK)) {
        // This SOS/MOS NONCE_REPORT is a nonce re-sync that must
        // reach the peer inside its 250 ms window, even while another
        // S2 session is paused in BACKOFF_CURRENT_SESSION_ID. We keep
        // number_of_responses at 0 because the peer's follow-up
        // MESSAGE_ENCAPSULATION is the reply to the paused session,
        // not to this frame.
        //
        // To skip the back-off, the TX process requires the frame to
        // be standalone (valid_parent_session_id = false) and to
        // enable the bypass (ignore_incoming_frames_back_off = true).
        // When an application send is in flight, `state` carries
        // valid_parent_session_id = true, so we override it here.
        // Without the override the frame stays blocked and gets
        // dropped when its discard_timeout_ms expires.
        options.number_of_responses                       = 0;
        options.transport.ignore_incoming_frames_back_off = true;
        options.transport.valid_parent_session_id         = false;
        options.transport.parent_session_id               = NULL;
    } else if (s2_is_inclusion_bootstrap_egress()) {
        // Inclusion bootstrap replies must not wait out a prior frame's
        // BACKOFF_CURRENT_SESSION_ID or they miss their discard window.
        options.transport.ignore_incoming_frames_back_off = true;
        options.transport.valid_parent_session_id         = false;
        options.transport.parent_session_id               = NULL;
        options.discard_timeout_ms                        = 0;
    ...
    } else if (state.abort_in_progress || (state.s2_send_callback != NULL && !state.inclusion_in_progress)) {
        user = (void *)&app_frame_generation_tags[state.abort_generation];
    }
...
uint8_t S2_send_frame_no_cb(struct S2 *ctxt, const s2_connection_t *conn, uint8_t *buf, uint16_t len)
{
    ...
    options.qos_priority = ZWAVE_TX_QOS_RECOMMENDED_TIMING_CRITICAL_PRIORITY + (10 * ZWAVE_TX_RECOMMENDED_QOS_GAP);
    ...
    return SL_STATUS_OK == zwave_tx_send_data(&info, len, buf, &options, NULL, 0, 0);
}
...
void S2_notify_nls_state_report(node_t srcNode, uint8_t class_id, bool nls_capability, bool nls_state)
{
    ...
    if (zwave_store_nls_support((zwave_node_id_t)srcNode, nls_capability, REPORTED_ATTRIBUTE) || zwave_store_nls_state((zwave_node_id_t)srcNode, nls_state, REPORTED_ATTRIBUTE)) {
...
{
    for (uint16_t i = (node_nls_context.next_sent_node_id + 1); i <= (node_nls_context.node_list_length * NLS_ENABLED_NODES_PER_BYTE); i++) {
...
{
    for (uint16_t i = 1; i <= (node_nls_context.node_list_length * NLS_ENABLED_NODES_PER_BYTE); i++) {
        uint8_t nls_enabled = ZW_IS_NODE_IN_MASK(i, node_nls_context.node_list);
        if (nls_enabled) {
...
int8_t S2_get_nls_node_list(node_t srcNode, bool request, bool *is_last_node, uint16_t *node_id, uint8_t *granted_keys, bool *nls_state)
{
    ...
    *nls_state         = false;
    ...
    sl_log_debug(LOG_TAG, "NLS Node List Get received from node %d, request: %d", srcNode, request);

    if (request == false) {
        memset(&node_nls_context, 0, sizeof(s2_node_nls_context_t));
        status = zwapi_get_nls_nodes(&node_nls_context.node_list_length, node_nls_context.node_list);
        if (status != SL_STATUS_OK) {
    ...
    if (node_nls_context.nls_enabled_node_sent_cnt == node_nls_context.nls_enabled_node_cnt) {
        sl_log_warning(LOG_TAG, "All NLS enabled nodes are already sent");
        return -1;
    }

    if (node_nls_context.nls_enabled_node_sent_cnt == node_nls_context.nls_enabled_node_cnt - 1) {
        *is_last_node = true;
    }
    ...
    status                = zwave_get_node_granted_keys(node_nls_context.next_sent_node_id, &keyset);
    if (status != SL_STATUS_OK) {
        sl_log_error(LOG_TAG, "Error getting granted keys of the node %d", node_nls_context.next_sent_node_id);
        return -1;
    }
...
int8_t S2_notify_nls_node_list_report(node_t srcNode, uint16_t id_of_node, uint8_t keys_node_bitmask, bool nls_state)
{
    ...
    sl_log_debug(LOG_TAG, "NLS Node List Report received from the node %d, NLS state: %d", srcNode, nls_state);

    if (nls_state) {
        status = zwapi_enable_node_nls(id_of_node);
        if (SL_STATUS_OK != status) {
            sl_log_error(LOG_TAG, "Error saving NLS state of the node %d in the controller NVM", id_of_node);
            return -1;
        }

        if (zwave_store_nls_support((zwave_node_id_t)id_of_node, true, REPORTED_ATTRIBUTE) || zwave_store_nls_state((zwave_node_id_t)id_of_node, true, REPORTED_ATTRIBUTE)) {
            sl_log_error(LOG_TAG, "Error setting NLS attributes of the node %d", id_of_node);
            return -1;
        }

        status = zwave_set_node_granted_keys((zwave_node_id_t)id_of_node, &keys_node_bitmask);
        if (SL_STATUS_OK != status) {
            sl_log_error(LOG_TAG, "Error setting granted keys of the node %d", id_of_node);
            return -1;
        }
...
sl_status_t zwave_s2_send_data(const zwave_controller_connection_info_t *connection, uint16_t data_length, const uint8_t *cmd_data, const zwave_tx_options_t *tx_options, const on_zwave_tx_send_data_complete_t on_send_complete, void *user, zwave_tx_session_id_t parent_session_id)
{
    ...
    if (connection->remote.is_multicast == false && !tx_options->skip_s2_verify_delivery) {
    ...
    if (data_length > ZWAVE_MAX_FRAME_SIZE) {
        sl_log_error(LOG_TAG, "Too large frame requested for transmitting. Discarding\n");
        return SL_STATUS_WOULD_OVERFLOW;
    }

    zwave_s2_transport_lock();

    if (s2_application_send_is_blocked(tx_options)) {
        sl_log_debug(LOG_TAG, "zwave_s2_send_data: application send blocked (S2 busy or session active); returning BUSY");
        zwave_s2_transport_unlock();
    ...
    if (tx_options->transport.is_protocol_frame == true) {
        ...
        memcpy(state.protocol_metadata.data, protocol_metadata->data, state.protocol_metadata.data_length);
    ...
    if (connection->remote.is_multicast == false) {
        ...
        zwave_s2_keyset_t keyset = get_s2_keyset_from_node(connection->remote.node_id);
        if (keyset == UNKNOWN_KEYSET) {
            // No usable keyset for this NodeID. Returning BUSY would make
            // zwave_tx_process defer the frame waiting for a transport
            // completion callback that will never fire (libS2 never starts a
            // transmission for an UNKNOWN_KEYSET). Fail fast instead so the
            // resolver can fail the rule immediately.
        ...
        if (tx_options->transport.group_id != ZWAVE_TX_INVALID_GROUP) {
            ...
            if (S2_send_data_singlecast_follow_up_with_keyset(s2_ctx, &s2_connection, keyset, state.last_frame_data, state.last_frame_data_length)) {
                result = SL_STATUS_OK;
                goto done;
            } else {
                state.s2_send_callback = cb_save;
        ...
        if (S2_send_data_singlecast_with_keyset(s2_ctx, &s2_connection, keyset, state.last_frame_data, state.last_frame_data_length)) {
            result = SL_STATUS_OK;
            goto done;
        } else {
            state.s2_send_callback = cb_save;
        ...
        zwave_s2_keyset_t keyset = get_s2_keyset_from_group(connection->remote.multicast_group);
        if (keyset == UNKNOWN_KEYSET) {
            state.s2_send_callback = cb_save;
            state.s2_send_user     = user_save;
            result                 = SL_STATUS_FAIL;
        ...
        if (S2_send_data_multicast_with_keyset(s2_ctx, &s2_connection, keyset, state.last_frame_data, state.last_frame_data_length)) {
            result = SL_STATUS_OK;
            goto done;
        } else {
            state.s2_send_callback = cb_save;
...
sl_status_t zwave_s2_on_frame_received(const zwave_controller_connection_info_t *connection_info, const zwave_rx_receive_options_t *rx_options, const uint8_t *frame_data, uint16_t frame_length)
{
    ...
    s2_connection.rx_options    = rx_options->status_flags & (RECEIVE_STATUS_TYPE_BROAD | RECEIVE_STATUS_TYPE_MULTI) ? S2_RXOPTION_MULTICAST : 0;
    ...
    S2_application_command_handler(s2_ctx, &s2_connection, frame_buffer, frame_length);
    zwave_s2_transport_unlock();
    return SL_STATUS_OK;
}
...
void zwave_s2_transport_init()
{
    ...
    state.current_transmission_type = SINGLECAST;
    state.inclusion_in_progress     = false;
    zwave_s2_transport_unlock();
}
...
sl_status_t zwave_s2_abort_send_data(zwave_tx_session_id_t session_id)
{
    ...
    void *user_save                                        = NULL;
    zwapi_tx_report_t tx_status_for_completion             = {};
    clock_time_t transmit_start_time                       = 0;
    zwave_s2_current_transmission_type_t transmission_type = SINGLECAST;

    zwave_s2_transport_lock();
    if (state.parent_session_id == session_id) {
        sl_log_debug(LOG_TAG, "Aborting S2 send session for frame id=%p", session_id);
        state.parent_session_id       = NULL;
        state.valid_parent_session_id = false;
        cb_save                       = state.s2_send_callback;
        user_save                     = state.s2_send_user;
        ...
        state.s2_send_user            = NULL;
        tx_status_for_completion      = state.s2_send_tx_status;
        transmit_start_time           = state.transmit_start_time;
        transmission_type             = state.current_transmission_type;
        status                        = SL_STATUS_OK;

        uint16_t tx_time = (uint16_t)(clock_time() - transmit_start_time);
        if (transmission_type == SINGLECAST) {
            tx_time += NONCE_REP_TIME;
        }
        S2_send_frame_done_notify(s2_ctx, S2_TRANSMIT_COMPLETE_NO_ACK, tx_time);
    }
    zwave_s2_transport_unlock();
