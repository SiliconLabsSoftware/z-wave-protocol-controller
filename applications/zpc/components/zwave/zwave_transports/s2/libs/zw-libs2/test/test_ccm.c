...
void test_verify_handful_payload_lengths(void)
{
  ...
  int lengths[6] = {
    ...
    54,
    1279,
    1280};  // Makes no sense to test for length 0 since the compare function will compare zero characters.
  ...
  for (i = 0; i < 6; i++) {
    ...
    memcpy(
      ciphertext,
      text_to_encrypt,
      text_to_encrypt_len);  //in-place encryption, so pass ciphertext which is of size "text_to_encrypt_len + t"
    ret = CCM_encrypt_and_auth(key,
                               nonce,
                               aad,
                               aad_len,
                               ciphertext,
    ...
    printf(
      "verify_handful_payload_lengths TEST PASSSED for plaintext length: %d\n",
      lengths[i]);
...
void ignore_test_verify_all_payload_lengths(void)
{
  ...
  for (i = 1; i < 0xffff; i++) {
    ...
    UNITY_TEST_ASSERT_EQUAL_UINT8_ARRAY(text_to_encrypt_bkup,
                                        ciphertext,
                                        plaintext_len,
                                        __LINE__,
                                        "");
...
void test_example1(void)
{
  ...
  uint8_t aad[16]       = {0x00,
                           ...
                           0x0e,
                           0x0f};
  uint8_t text_to_encrypt[16]            = {0x20,
                                            0x21,
                                            0x22,
                                            0x23,
                                            0x24,
                                            ...
                                            0x2e,
                                            0x2f};
  const uint8_t text_to_encrypt_bkup[16] = {0x20,
                                            0x21,
                                            0x22,
                                            0x23,
                                            0x24,
...
#define CIPHERTEXT_LEN_DEF (TEXT_TO_ENCRYPT_LEN_DEF + T_DEF)
  ...
  int ret;
  uint8_t nist_cipher_and_auth_tag[16 + 6]
  ...
  printf("Refer to "
         ...
         "SP800-38C_updated-July20_2007.pdf page 17 Example 2\n");
  ret = CCM_encrypt_and_auth(key,
                             nonce,
                             aad,
                             aad_len,
  ...
  UNITY_TEST_ASSERT_EQUAL_UINT32(
    sizeof(nist_cipher_and_auth_tag),
    ciphertext_len,
    __LINE__,
    "Ciphertext length does not match with NIST document.");

  UNITY_TEST_ASSERT_EQUAL_UINT8_ARRAY(
    nist_cipher_and_auth_tag,
    ciphertext,
    ciphertext_len,
    __LINE__,
    "Ciphertext does not match with NIST document.");
...
void test_big_aad_example(void)
{
  ...
  uint32_t ciphertext_len;
  int ret;
  uint8_t nist_cipher_and_auth_tag[32 + 14]
    = {0x69, 0x91, 0x5d, 0xad, 0x1e, 0x84, 0xc6, 0x37, 0x6a, 0x68, 0xc2, 0x96,
       0x7e, 0x4d, 0xab, 0x61, 0x5a, 0xe0, 0xfd, 0x1f, 0xae, 0xc4, 0x4c, 0xc4,
       0x84, 0x82, 0x85, 0x29, 0x46, 0x3c, 0xcf, 0x72, 0xb4, 0xac, 0x6b, 0xec,
       0x93, 0xe8, 0x59, 0x8e, 0x7f, 0x0d, 0xad, 0xbc, 0xea, 0x5b};
...
void test_handful_aad_lengths(void)
{
  ...
  for (i = 0; i < sizeof(lengths) / sizeof(*lengths); i++) {
    ...
    plaintext_len = CCM_decrypt_and_auth(key,
                                         nonce,
                                         aad,
                                         aad_len,
                                         ciphertext,
...
#ifdef NOT_USED
static int test_all_aad_lengths(void)
{
  uint8_t key[16]   = {0x40,
                       0x41,
                       0x42,
...
static int test_aad_len_smaller_than_12(void)
{
  uint8_t key[16]   = {0x0,
                       0x0,
                       0x0,
  ...
  uint8_t text_to_encrypt[24]
    = {0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x57, 0x6f, 0x72, 0x6c, 0x64, 0x20, 0x53,
       0x65, 0x63, 0x6f, 0x6e, 0x64, 0x20, 0x46, 0x72, 0x61, 0x6d, 0x65, 0x00};
...
int main(int argc, char *argv[])
{
  if ((argc == 2) && (strncmp(argv[1], "-h", 2) == 0)) {
    printf("./test_ccm -s for verifying all payload lengths and aad lengths "
           "and standard tests\n");
