...
static zwapi_protocol_version_information_t protocol_version;
...
{
    if ((func_id == FUNC_ID_SERIAL_API_GET_CAPABILITIES) || (chip.supported_bitmask[(func_id - 1) >> 3] & (1 << ((func_id - 1) & 0x7)))) {
        return true;
    }
    return false;
}
...
{
    int file_descriptor = zwapi_session_init(connection_params);
    if (file_descriptor <= 0) {
        return SL_STATUS_FAIL;
    }
    if (connection_fd != NULL) {
        *connection_fd = file_descriptor;
    }
    ...
    sl_status_t refresh_status = zwapi_refresh_protocol_version();

    if (refresh_status == SL_STATUS_OK) {
        refresh_status = zwapi_refresh_capabilities();
    }
...
int zwapi_reconnect(void)
{
    return zwapi_session_restart();
}

sl_status_t zwapi_log_to_file_enable(const char *filename)
{
    return zwapi_connection_log_to_file_enable(filename);
}

sl_status_t zwapi_log_to_file_disable()
{
    return zwapi_connection_log_to_file_disable();
}
...
sl_status_t zwapi_refresh_protocol_version(void)
{
    uint8_t response_length                   = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_session_send_frame_with_response(FUNC_ID_ZW_GET_PROTOCOL_VERSION, NULL, 0, response_buffer, &response_length);

    if (send_command_status == SL_STATUS_OK) {
        if ((response_length - IDX_DATA) >= 1) {
            protocol_version.type = response_buffer[IDX_DATA];
        }
        if ((response_length - IDX_DATA) >= 4) {
            protocol_version.revision_version = response_buffer[IDX_DATA + 3];
            protocol_version.minor_version    = response_buffer[IDX_DATA + 2];
            protocol_version.major_version    = response_buffer[IDX_DATA + 1];
        }

        if ((response_length - IDX_DATA) >= 6) {
            protocol_version.build_number = response_buffer[IDX_DATA + 5];
            protocol_version.build_number += response_buffer[IDX_DATA + 4] << 8;
        }
        if ((response_length - IDX_DATA) >= (6 + sizeof(protocol_version.git_commit))) {
            memcpy(&protocol_version.git_commit, &response_buffer[IDX_DATA + 6], sizeof(protocol_version.git_commit));
        }
    }
...
sl_status_t zwapi_refresh_capabilities(void)
{
    ...
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    uint8_t response_length                   = 0;
    sl_status_t capabilities_status           = zwapi_session_send_frame_with_response(FUNC_ID_SERIAL_API_GET_CAPABILITIES, NULL, 0, response_buffer, &response_length);

    if (capabilities_status != SL_STATUS_OK) {
        sl_log_error(LOG_TAG, "Failed to fetch capabilities from the Z-Wave module\n");
    ...
    if (response_length > (IDX_DATA + 7) && (response_length <= FRAME_LENGTH_MAX)) {
        ...
        if (length > sizeof(chip.supported_bitmask)) {
            sl_log_warning(LOG_TAG, "zwapi_refresh_capabilities: clamping supported_bitmask response from %d to %d\n", length, sizeof(chip.supported_bitmask));
            length = sizeof(chip.supported_bitmask);
        }
    ...
    if (zwapi_support_command_func(FUNC_ID_SERIAL_API_SETUP)) {
        ...
        sl_status_t setup_status = zwapi_session_send_frame_with_response(FUNC_ID_SERIAL_API_SETUP, request_buffer, index, response_buffer, &response_length);
        if (setup_status != SL_STATUS_OK) {
            sl_log_error(LOG_TAG, "Failed to fetch supported setup commands from the Z-Wave module\n");
            return setup_status;
        }

        if (response_length > (IDX_DATA + 1) && response_buffer[IDX_DATA] == ZW_COMMAND_RETURN_VALUE_TRUE) {
            uint8_t current_index = IDX_DATA + 1;
            memcpy(chip.supported_setup_bitmask, &(response_buffer[current_index]), response_length - (current_index - 1));
        } else {
            return SL_STATUS_FAIL;
        }
    }
    ...
    if (chip.library_type < sizeof(zw_lib_names) / sizeof(char *)) {
        ...
        sl_log_info(LOG_TAG, "Z-Wave library type    : %s\n", zw_lib_names[chip.library_type]);
    }

    // Update our local cache of the chip type/version.
    sl_status_t init_data_status = zwapi_get_init_data(NULL, &chip.capability_flag, NULL, chip.node_list, &chip.chip_type, &chip.chip_revision);
...
{
    uint8_t response_length                   = 0;
    uint8_t node_list_length                  = 0;
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    sl_status_t send_command_status           = zwapi_session_send_frame_with_response(FUNC_ID_SERIAL_API_GET_INIT_DATA, NULL, 0, response_buffer, &response_length);
...
sl_status_t zwapi_get_node_list(zwave_nodemask_t node_list)
{
    ...
    sl_status_t command_status = zwapi_get_init_data(NULL, NULL, NULL, chip.node_list, NULL, NULL);
...
sl_status_t zwapi_get_protocol_version(zwapi_protocol_version_information_t *protocol)
{
    ...
    memcpy(protocol, &protocol_version, sizeof(protocol_version));
    return SL_STATUS_OK;
}
...
sl_status_t zwapi_send_command(uint8_t command, const uint8_t *payload_buffer, uint8_t payload_buffer_length)
{
    ...
    return zwapi_session_send_frame(command, payload_buffer, payload_buffer_length);
}
...
sl_status_t zwapi_send_command_with_response(uint8_t command, const uint8_t *payload_buffer, uint8_t payload_buffer_length, uint8_t *response_buffer, uint8_t *response_length)
{
    ...
    return zwapi_session_send_frame_with_response(command, payload_buffer, payload_buffer_length, response_buffer, response_length);
}
...
sl_status_t zwapi_send_command_no_ack(uint8_t command, const uint8_t *payload_buffer, uint8_t payload_buffer_length)
{
    ...
    return zwapi_session_send_frame_no_ack(command, payload_buffer, payload_buffer_length);
}
...
zwave_lr_channel_t zwapi_get_zwave_lr_channel()
{
    uint8_t response_buffer[FRAME_LENGTH_MAX] = {0};
    uint8_t response_length                   = 0;
    sl_status_t command_status                = zwapi_session_send_frame_with_response(FUNC_ID_GET_LR_CHANNEL, NULL, 0, response_buffer, &response_length);
...
sl_status_t zwapi_set_zwave_lr_channel(zwave_lr_channel_t new_channel)
{
    ...
    return zwapi_send_command(FUNC_ID_SET_LR_CHANNEL, request_buffer, sizeof(request_buffer));
}
