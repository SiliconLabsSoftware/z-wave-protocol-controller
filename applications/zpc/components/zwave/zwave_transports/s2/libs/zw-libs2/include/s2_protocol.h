...
struct SPAN {
  ...
  security_class_t
    class_id;  //The id of the security group in which this span is negotiated.
  span_state_t state;
};

struct MPAN {
  node_t
    owner_id;  //this is the node id of the node maintaining mcast group members
  ...
  enum { MPAN_NOT_USED, MPAN_SET, MPAN_MOS } state;  //State of this entry
};
...
struct S2 {
  ...
  uint8_t
    key_granted;  // The granted keys bitmask. Including the LR bits if bootstrapping a controller only.
  ...
  uint8_t
    key_exchange;  // Single bit rolling across bitmask of all keys to keep track of which keys have been exchanged so far.
  uint8_t
    key_requested;  // A single bit identifying the key that has been requested most recently in a Network Key Get frame.
  uint8_t
    kex_report_keys;  // The unmodified keys from the incoming Kex Report. Used for echoing back. No filtering for LR etc.
 ...
 *
 */
uint8_t S2_network_key_update(struct S2 *ctxt,
                              uint32_t key_id,
                              security_class_t class_id,
