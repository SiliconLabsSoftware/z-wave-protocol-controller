...
#define ZWAVE_API_STARTED_ATTEMPT_TIMEOUT_MS 5000
#define ZWAVE_API_STARTED_MAX_ATTEMPTS       3
...
static void zwave_rx_print_chip_data(zwapi_chip_data_t chip_data)
{
    ...
    sl_log_info(LOG_TAG, "Z-Wave API module revision: %d\n", chip_data.chip_revision);
    sl_log_info(LOG_TAG, "Z-Wave API module software version: %d.%d\n", chip_data.version_major, chip_data.version_minor);
    sl_log_info(LOG_TAG, "Z-Wave API module Manufacturer ID 0x%04X\n", chip_data.manufacturer_id);
    sl_log_info(LOG_TAG, "Z-Wave API module Product type 0x%04X\n", chip_data.product_type);
    sl_log_info(LOG_TAG, "Z-Wave API module Product ID 0x%04X\n", chip_data.product_id);
    sl_log_info(LOG_TAG, "Z-Wave API module supported setup mask 0x%02X\n", chip_data.supported_setup_bitmask);
...
static void zwave_rx_print_protocol_version(zwapi_protocol_version_information_t zwapi_version)
{
    ...
    sl_log_info(LOG_TAG, "Z-Wave API protocol version: %d.%02d.%02d\n", zwapi_version.major_version, zwapi_version.minor_version, zwapi_version.revision_version);
    sl_log_info(LOG_TAG, "Z-Wave API protocol build number: %d\n", zwapi_version.build_number);

    char git_commit_string[(GIT_COMMIT_HASH_SIZE * 2) + 1] = {0};
    uint16_t index                                         = 0;
    for (uint8_t i = 0; i < GIT_COMMIT_HASH_SIZE; i++) {
        int written = snprintf(git_commit_string + index, sizeof(git_commit_string) - index, "%x", zwapi_version.git_commit[i]);
...
sl_status_t zwave_rx_init(const zwapi_connection_params_t *connection_params, int *connection_fd, int8_t normal_tx_power_dbm, int8_t measured_0dBm_power, int16_t max_lr_tx_power_dbm, zwave_controller_region_t region)
{
    ...
    zwave_rx_zwapi_callbacks.protocol_cc_encryption_request     = zwave_rx_protocol_cc_encryption_request;
    zwave_rx_zwapi_callbacks.application_controller_update      = zwave_rx_application_controller_update;
    ...
    if (chip_data.library_type != ZWAVE_LIBRARY_TYPE_CONTROLLER_STATIC && chip_data.library_type != ZWAVE_LIBRARY_TYPE_CONTROLLER && chip_data.library_type != ZWAVE_LIBRARY_TYPE_CONTROLLER_BRIDGE) {
        sl_log_error(LOG_TAG, "Z-Wave module library is not a controller.\n");
        return SL_STATUS_FAIL;
    }

    // Configure the RF transmit power
    tx_power_level_t zwave_tx_powerlevel = {.normal = normal_tx_power_dbm, .measured0dBm = measured_0dBm_power};
    command_status                       = zwapi_set_tx_power_level(zwave_tx_powerlevel);
    if (command_status != SL_STATUS_OK && command_status != SL_STATUS_NOT_SUPPORTED) {
        sl_log_warning(LOG_TAG, "Z-Wave module power setting could not be applied. Command status: %d.\n", command_status);
    ...
    if (region != zwapi_get_rf_region()) {
        ...
        if (command_status == SL_STATUS_NOT_SUPPORTED) {
            ...
            sl_log_error(LOG_TAG, "Could not set the Z-Wave RF Region %d. Command status: %d\n", region, command_status);
            sl_log_warning(LOG_TAG, "Z-Wave API is running with RF Region %d.\n", zwapi_get_rf_region());
    ...
    if (IS_RF_REGION_LR(zwapi_get_rf_region())) {
        ...
        if (command_status != SL_STATUS_OK && command_status != SL_STATUS_NOT_SUPPORTED) {
            ...
            sl_log_debug(LOG_TAG, "Success setting Z-Wave module Max Long Range transmit power: %d", max_lr_tx_power_dbm);
            sl_log_info(LOG_TAG, "Applying soft reset of the Z-Wave Module\n");
            soft_reset_needed = true;
        }

        sl_log_debug(LOG_TAG, "Current Z-Wave module Max Long Range transmit power in dbm: %d", zwapi_get_max_lr_tx_power_level());
    }

    if (true == soft_reset_needed) {
        zwapi_soft_reset();
        sl_status_t wait_status = zwave_rx_wait_for_zwave_api_to_be_ready();
        if (wait_status != SL_STATUS_OK) {
            return wait_status;
        }
    }
    ...
    if (command_status != SL_STATUS_OK) {
        sl_log_info(LOG_TAG, "Z-Wave Module runs on 8-bits NodeID basetype setting\n");
    } else {
        sl_log_info(LOG_TAG, "Z-Wave Module runs on 16-bits NodeID basetype setting\n");
    }

    return SL_STATUS_OK;
}

sl_status_t zwave_rx_wait_for_zwave_api_to_be_ready()
{
    for (int attempt = 0; attempt < ZWAVE_API_STARTED_MAX_ATTEMPTS; attempt++) {
        if (attempt > 0) {
            sl_log_warning(LOG_TAG,
                           "Timed out waiting for Z-Wave API started (attempt %d/%d). "
                           "Reconnecting serial and retrying.\n",
                           attempt,
                           ZWAVE_API_STARTED_MAX_ATTEMPTS);
            zwapi_reconnect();
            zwapi_soft_reset();
        }
        ...
        deadline.tv_sec += ZWAVE_API_STARTED_ATTEMPT_TIMEOUT_MS / 1000;

        while (zwapi_is_awaiting_zwave_api_started()) {
            zwapi_poll();
            struct timespec now;
        ...
        if (!zwapi_is_awaiting_zwave_api_started()) {
            return SL_STATUS_OK;
        }
    }

    sl_log_critical(LOG_TAG,
                    "Z-Wave module did not send SERIAL_API_STARTED after %d attempts. "
                    "Check the serial connection and module firmware.\n",
                    ZWAVE_API_STARTED_MAX_ATTEMPTS);
    return SL_STATUS_FAIL;
}
