...
static unsigned int s0_is_nonce_blocked(const uint8_t src, const uint8_t dst, const uint8_t *nonce)
{
    for (unsigned int i = 0; i < NONCE_BLOCK_LIST_SIZE; i++) {
        if (nonce_block[i].in_use && (0 == memcmp(nonce_block[i].nonce, nonce, 8)) && (nonce_block[i].src == src) && (nonce_block[i].dst == dst)) {
            return 1;
        }
    }
    return 0;
...
struct timer_handle_t nonce_timer = {NULL};

/**
 * Register a new nonce from sent from src to dst
 */
static uint8_t register_nonce(uint8_t src, uint8_t dst, uint8_t reply_nonce, const uint8_t nonce[8])
{
    if (reply_nonce) {
        /*Only one reply nonce is allowed*/
        for (uint8_t i = 0; i < NONCE_TABLE_SIZE; i++) {
            if (nonce_table[i].reply_nonce && nonce_table[i].timeout > 0 && nonce_table[i].src == src && nonce_table[i].dst == dst) {
                sl_log_debug(LOG_TAG, "Reply nonce overwritten %d->%d slot:%d\n", src, dst, i);
 ...
 */
static uint8_t get_s0_nonce(uint8_t src, uint8_t dst, uint8_t ri, uint8_t nonce[RECEIVERS_NONCE_SIZE], uint8_t any_nonce)
{
    for (uint8_t i = 0; i < NONCE_TABLE_SIZE; i++) {
        if ((nonce_table[i].timeout > 0) && (nonce_table[i].src == src) && (nonce_table[i].dst == dst) && (any_nonce || (nonce_table[i].nonce[0] == ri))) {
...
static void nonce_clear(uint8_t src, uint8_t dst)
{
    ...
    for (uint8_t i = 0; i < NONCE_TABLE_SIZE; i++) {
        if ((nonce_table[i].timeout) && (nonce_table[i].src == src) && (nonce_table[i].dst == dst)) {
            ...
            sl_log_debug(LOG_TAG, "%d -> %d nonce cleared slot: %d\n", nonce_table[i].src, nonce_table[i].dst, i);
        }
    }
}
...
static void nonce_timer_timeout(void *data)
{
    ...
    for (uint8_t i = 0; i < NONCE_TABLE_SIZE; i++) {
        if (nonce_table[i].timeout > 0) {
            ...
            if (nonce_table[i].timeout == 0) {
                sl_log_debug(LOG_TAG, "%d -> %d nonce timed out slot: %d\n", nonce_table[i].src, nonce_table[i].dst, i);
            } else {
                restartTimer = true;
            }
        }
    ...
    if (restartTimer) {
        timer_set(&nonce_timer, 1000, nonce_timer_timeout, 0);
    }
}

static void restart_s0_timer()
{
    if (timer_expired(&nonce_timer)) {
        timer_set(&nonce_timer, 1000, nonce_timer_timeout, 0);
    }
}
...
static void s0_timeout(void *user)
{
    ...
    sl_log_warning(LOG_TAG, "Security0 transmit timeout state: %s\n", s0_state_name(s->state));
...
static sec_tx_session_t *get_tx_session_by_node(uint8_t snode, uint8_t dnode)
{
    for (uint8_t i = 0; i < NUM_TX_SESSIONS; i++) {
        if (tx_sessions[i].conn_info.remote.node_id == dnode && tx_sessions[i].conn_info.local.node_id == snode) {
...
/**
 * Get the maximum MAC-layer payload that can be delivered to a node.
 *
 * Delegates to zwave_tx_scheme_get_max_payload() so that region-dependent
 * capacity (e.g. 150 bytes for KR/JP R3 vs 46 bytes for R1/R2) is honoured.
 * S0 subtracts S0_ENCAP_HEADER_LEN from this value when computing how many
 * application bytes fit per encrypted frame.
 */
static uint8_t get_node_max_frame_size(zwave_node_id_t node_id)
{
    return (uint8_t)zwave_tx_scheme_get_max_payload(node_id);
}
...
static uint8_t s0_encrypt_message(sec_tx_session_t *s, uint8_t pass2)
{
    ...
    } while (get_s0_nonce(s->conn_info.remote.node_id, s->conn_info.local.node_id, iv[0], tmp, false));
    ...
    if (!get_s0_nonce(s->conn_info.remote.node_id, s->conn_info.local.node_id, 0, iv + 8, true)) {
        sl_log_warning(LOG_TAG, "error: Nonce for NodeIDs %u -> %u is not found\n", s->conn_info.remote.node_id, s->conn_info.local.node_id);
        return 0;
    ...
    /*Register my nonce */
    register_nonce(s->conn_info.local.node_id, s->conn_info.remote.node_id, true, iv);
    ...
    /*
     * The second frame (pass2=1) is always the terminal frame of a two-frame
     * S0 sequence. It MUST use SECURITY_MESSAGE_ENCAPSULATION so the receiver
     * does not expect a further nonce exchange (spec: last frame MAY use
     * SECURITY_MESSAGE_ENCAPSULATION). Using NONCE_GET here would cause the
     * device to issue a spurious Nonce Report that no session can match.
     */
    if (pass2) {
        auth->sh = SECURITY_MESSAGE_ENCAPSULATION;
    } else {
    ...
    /*
     * S0 sequencing is limited to two frames. If bytes remain after the
     * second frame the payload exceeds the two-frame capacity and cannot
     * be delivered — fail now rather than silently truncate.
     */
    if (pass2 && s->data_len > 0) {
        sl_log_error(LOG_TAG,
                     "S0 payload too large for two-frame sequence: %u bytes remain "
                     "after ENC_MSG2 for NodeIDs %u -> %u\n",
                     s->data_len,
...
static void tx_session_state_set(sec_tx_session_t *s, tx_state_t state)
{
    ...
    switch (s->state) {
        case NONCE_GET:
            ...
            options.number_of_responses = 1;
            if (zwave_tx_send_data(&c2, sizeof(nonce_get), nonce_get, &options, callback, s, 0) != SL_STATUS_OK) {
                s->tx_code = TRANSMIT_COMPLETE_FAIL;
                tx_session_state_set(s, TX_DONE);
            } else {
                 ...
                 */
                timer_set(&s->timer, NONCE_REQUEST_TIMER, s0_timeout, s);
            }
            break;
        case NONCE_GET_SENT:
        ...
        case ENC_MSG:
            ...
            // When more data remains the device will reply with SECURITY_NONCE_GET to
            // obtain the sender's nonce for ENC_MSG2, so count that as one response.
            // When this is the last frame no application-level reply is expected.
            options.number_of_responses = (s->data_len > 0) ? 1 : 0;
            if ((len == 0) || (zwave_tx_send_data(&c2, len, s->crypted_msg, &options, callback, s, 0) != SL_STATUS_OK)) {
                s->tx_code = TRANSMIT_COMPLETE_FAIL;
                tx_session_state_set(s, TX_DONE);
            } else {
        ...
        case ENC_MSG2:
            ...
            options.number_of_responses = 0;  // Last frame: no reply expected.
            if ((len == 0) || (zwave_tx_send_data(&c2, len, s->crypted_msg, &options, callback, s, 0)) != SL_STATUS_OK) {
                s->tx_code = TRANSMIT_COMPLETE_FAIL;
                tx_session_state_set(s, TX_DONE);
            } else {
        ...
        case ENC_MSG2_SENT:
            if (s->data_len > 0) {
                sl_log_error(LOG_TAG,
                             "S0 session completed with %u undelivered bytes; "
                             "payload exceeded two-frame S0 capacity\n",
                             s->data_len);
                s->tx_code = TRANSMIT_COMPLETE_FAIL;
            }
            tx_session_state_set(s, TX_DONE);
        ...
        case TX_DONE:
            ...
            break;
        default:
            sl_log_error(LOG_TAG, "Unknown state %d. Considering session failed.\n", state);
            s->tx_code = TRANSMIT_COMPLETE_FAIL;
            tx_session_state_set(s, TX_DONE);
            break;
    }
...
sl_status_t zwave_s0_send_data(const zwave_controller_connection_info_t *conn_info, uint16_t data_length_2_byte, const uint8_t *cmd_data, const zwave_tx_options_t *tx_options, const on_zwave_tx_send_data_complete_t on_send_complete, void *user, zwave_tx_session_id_t parent_session_id)
{
    ...
    s = get_tx_session_by_node(conn_info->local.node_id, conn_info->remote.node_id);
    if (s) {
        sl_log_error(LOG_TAG, "Already have one tx session from NodeID %d to %d\n", conn_info->local.node_id, conn_info->remote.node_id);
...
static void s0_register_nonce(uint8_t src, uint8_t dst, const uint8_t *nonce)
{
    ...
    if (s0_is_nonce_blocked(src, dst, nonce)) {
        sl_log_warning(LOG_TAG, "Ignoring duplicate nonce for NodeIDs %d -> %d\n", src, dst);
        return;
    }
...
static void s0_send_nonce(const zwave_controller_connection_info_t *conn_info)
{
    ...
    } while (get_s0_nonce(conn_info->local.node_id, conn_info->remote.node_id, nonce[0], tmp, false));
    ...
    c2.encapsulation = 0;
    if (zwave_tx_send_data(&c2, sizeof(nonce_res), (uint8_t *)nonce_res, &options, 0, 0, 0) == SL_STATUS_OK) {
        register_nonce(conn_info->local.node_id, conn_info->remote.node_id, false, nonce);
    } else {
        sl_log_error(LOG_TAG, "Failure in sending nonce report from %d -> %d\n", conn_info->local.node_id, conn_info->remote.node_id);
    }
}
...
static uint8_t s0_decrypt_message(const zwave_controller_connection_info_t *connection_info, const zwave_rx_receive_options_t *rx_options, const uint8_t *encrypted_frame, uint16_t encrypted_frame_len, uint8_t *decrypted_frame, uint16_t decrypted_frame_len)
{
    ...
    /*Build the IV*/
    memcpy(iv, frame_data + offsetof(ZW_SECURITY_MESSAGE_ENCAPSULATION_1BYTE_FRAME, initializationVectorByte1), S0_IV_SIZE);
    ...
    nonce_clear(connection_info->local.node_id, connection_info->remote.node_id);

    /*TODO don't create sessions for single fragment frames*/
    s = get_rx_session_by_nodes(connection_info->remote.node_id, connection_info->local.node_id);
    if (!s) {
        s = new_rx_session(connection_info->remote.node_id, connection_info->local.node_id);
        if (s) {
            s->state = RX_INIT;
        } else {
    ...
    enc_payload = frame_data + offsetof(ZW_SECURITY_MESSAGE_ENCAPSULATION_1BYTE_FRAME, properties1);
    ...
    memcpy((uint8_t *)auth + sizeof(auth_data_t), enc_payload, auth->payloadLength);
    ...
    if (flags & SECURITY_MESSAGE_ENCAPSULATION_PROPERTIES1_SEQUENCED_BIT_MASK) {
        ...
        if ((s->state != RX_ENC1) || (flags & 0xF) != s->seq_nr) {
            sl_log_error(LOG_TAG, "State is %d, received sequence number is %u while we expected %u\n", (int)s->state, flags & 0xF, s->seq_nr);
            goto state_error;
        } else {
            s->state = RX_ENC2;
        }
    ...
    switch (frame_data[COMMAND_INDEX]) {
        case SECURITY_SCHEME_REPORT:
            if (frame_length >= sizeof(ZW_SECURITY_SCHEME_REPORT_FRAME)) {
                s0_scheme_report_received(frame_data[offsetof(ZW_SECURITY_SCHEME_REPORT_FRAME, supportedSecuritySchemes)], c->remote.node_id);
            }
            break;
        ...
        case SECURITY_NONCE_REPORT:
            if ((c->encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_NONE) && (frame_length >= sizeof(ZW_SECURITY_NONCE_REPORT_FRAME))) {
                s0_register_nonce(c->remote.node_id, c->local.node_id, &frame_data[offsetof(ZW_SECURITY_NONCE_REPORT_FRAME, nonceByte1)]);
            }
            break;
        case SECURITY_COMMANDS_SUPPORTED_GET:
            ...
            zwave_controller_storage_get_node_granted_keys(zwave_network_management_get_node_id(), &supporting_node_keys);
            ...
            sl_log_debug(LOG_TAG, "supporting_node_scheme = %d\n", supporting_node_scheme);
            if (supporting_node_scheme == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_0) {
                ...
                // never be S0
                // ZPC-794: Implement if ZPC has S0 as highest key.
            ...
            zwave_tx_options_t options                         = {};
            const uint8_t empty_s0_command_supported_report[3] = {COMMAND_CLASS_SECURITY, SECURITY_COMMANDS_SUPPORTED_REPORT, 0x00};
            zwave_tx_send_data(&c2, sizeof(empty_s0_command_supported_report), empty_s0_command_supported_report, &options, 0, 0, 0);
            break;
        case SECURITY_MESSAGE_ENCAPSULATION_NONCE_GET:
        case SECURITY_MESSAGE_ENCAPSULATION:

            len = s0_decrypt_message(c, rx_options, encrypted_frame_data, frame_length, decrypted_frame, sizeof(decrypted_frame));
            if (len) {
                zwave_controller_connection_info_t c2;
                memcpy(&c2, c, sizeof(zwave_controller_connection_info_t));
                c2.encapsulation = ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_0;
                zwave_controller_on_frame_received(&c2, rx_options, decrypted_frame, len);
            } else {
                return SL_STATUS_FAIL;
            }
            break;
        default:
            sl_log_debug(LOG_TAG, "S0 Command: 0x%02X not supported\n", frame_data[COMMAND_INDEX]);
            return (SL_STATUS_NOT_FOUND);
    }
    return SL_STATUS_OK;
}
...
{
    return s0_application_command_handler(c, rx_options, encrypted_frame, encrypted_frame_len);
}

void free_all_tx_sessions()
{
    for (uint8_t i = 0; i < NUM_TX_SESSIONS; i++) {
        timer_stop(&tx_sessions[i].timer);
...
{
    timer_stop(&nonce_timer);
    timer_set(&nonce_timer, CLOCK_SECOND, nonce_timer_timeout, 0);
}
...
sl_status_t zwave_s0_on_abort_send_data(zwave_tx_session_id_t session_id)
{
    ...
    for (uint8_t i = 0; i < NUM_TX_SESSIONS; i++) {
        if (tx_sessions[i].session_id == session_id) {
            callback(TRANSMIT_COMPLETE_FAIL, tx_sessions[i].tx_ext_status, &tx_sessions[i]);
            return SL_STATUS_OK;
        }
    }
    return SL_STATUS_NOT_FOUND;
