...
///@{
typedef enum {
    ZWAVE_LIBRARY_TYPE_CONTROLLER_STATIC = 0x01,  ///< aka ZW_LIB_CONTROLLER_STATIC
    ...
    ZWAVE_LIBRARY_TYPE_CONTROLLER_BRIDGE = 0x07,  ///< aka ZW_LIB_CONTROLLER_BRIDGE
...
///@{
typedef enum {
    ZWAVE_LONG_RANGE_CHANNEL_NOT_SUPPORTED = 0x00,  ///< Z-Wave LR is not supported
 ...
 *
 * @param connection_params is a pointer to a zwapi_connection_params_t structure
 * containing the connection parameters.
 * @param connection_fd pointer where the connection file descriptor will be written
...
/**
 * @brief Restart the connection to the Z-Wave module.
 *
 * Closes and reopens the serial/IP connection. Useful after a soft reset
 * when the module may have re-enumerated (e.g. USB CDC-ACM reconnect).
 *
 * @returns File descriptor for the connection, greater than 0 on success.
 */
...
/**
 * @brief Refresh the protocol version from the Z-Wave module.
 *
 * @returns SL_STATUS_OK if the refresh operation was successful.
 * @returns SL_STATUS_FAIL otherwise
 */
 ...
 *
 * @param protocol pointer to the zwapi_protocol_version_information_t structure to copy the protocol version information to.
