  ...
  const uint8_t session_id)
{
  ...
  zwave_tx_scheme_get_node_connection_info(destination_node_id,
                                           0,
                                           &connection_info);
  ...
  uint16_t frame_length)
{
  ...
  sl_log_info(LOG_TAG,
              "Protocol command received from NodeID %d:%d",
              connection->remote.node_id,
              connection->remote.endpoint_id);

  sl_status_t status = zwapi_transfer_protocol_cc(
    connection->remote.node_id,
...
sl_status_t zwave_command_class_protocol_init()
{
  ...
  handler_protocol.support_handler
  ...
  if (SL_STATUS_OK
    ...
    sl_log_error(LOG_TAG,
                 "Failed to register callbacks for Protocol Command Class");
    return SL_STATUS_FAIL;
  }

  if (SL_STATUS_OK
      != zwave_command_handler_register_handler(handler_protocol)) {
    sl_log_error(LOG_TAG, "Failed to register Protocol Command Class");
    return SL_STATUS_FAIL;
  }

  zwave_command_handler_t handler_protocol_lr = {0};
  handler_protocol_lr.support_handler
    = &zwave_command_class_protocol_support_handler;
  ...
  if (SL_STATUS_OK
      != zwave_command_handler_register_handler(handler_protocol_lr)) {
    sl_log_error(LOG_TAG, "Failed to register Protocol Command Class");
    return SL_STATUS_FAIL;
  }
