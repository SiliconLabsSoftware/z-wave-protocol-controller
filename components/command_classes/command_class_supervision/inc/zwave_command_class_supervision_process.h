...
/**
 * @brief Restarts the supervision timer
 */
void zwave_command_class_supervision_restart_timer();

/**
 * @brief Sets the next session ID
 *
 * @param session_id The next session ID to set
 */
void zwave_command_class_supervision_set_next_session_id(uint8_t session_id);

/**
 * @brief Closes all supervision sessions targeting a deleted node, firing
 *        their callbacks with SUPERVISION_REPORT_FAIL immediately.
 *
 * @param node_id The deleted node ID.
 */
...
{
    class supervision_handler;
}

/**
 * @brief Gets the global supervision handler instance
 *
 * @return Pointer to the supervision handler instance, or nullptr if not initialized
 */
zwave_component::supervision_handler *zwave_command_class_supervision_get_handler();
}
