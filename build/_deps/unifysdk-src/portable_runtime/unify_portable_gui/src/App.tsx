...
import { AppState, Frequency, Status, BuildStatus, DeviceTypeList } from './app-types';
...
class App extends Component<{}, AppState> {
  ...
  isNextDisabled = () => {
    return (this.state.Status === Status.Protocol
      && !this.state.ProtocolList["Z-Wave"].Enabled
      && !this.state.Emulator['EED'].Enabled)
      || (this.state.Status === Status.Configuration && this.state.ProtocolList["Z-Wave"].Enabled && this.state.ProtocolList["Z-Wave"].Device == "")
      || (this.state.Status === Status.Configuration && this.state.Emulator["EED"].Enabled && this.state.Emulator["EED"].DeviceType == "")
  ...
  onNextClick = () => {
    switch (this.state.Status) {
      ...
      case Status.Configuration:
        ...
        invoke('validate', { serviceList: serviceList.join(','), protocolList: protocolList, region: this.state.ProtocolList['Z-Wave'].RF,
                ...
                if (item.Enabled){
                  return (<div className='config-item' key={index}>
                    ...
                    <div className='config-device'>
                      ...
                      <Form.Control as="select" name="DeviceType" onChange={this.onDeviceTypeChange.bind(this, emulator)} value={item.DeviceType}>
                        <option value="">Select a device type</option>
                        {Object.keys(DeviceTypeList).map((dt: any, index: number) => {
                          return <option key={index} value={DeviceTypeList[dt as keyof typeof DeviceTypeList]}>{dt}</option>
                        })}
                      </Form.Control>
                    </div>
                  </div>)
