...
/**
 * @brief Check if the humidity control mode is compatible with the current version
 * 
 * @param supported_humidity_control_mode_type Tested humidity control mode
 * @param current_version Current Z-Wave command class version
 * 
 * @returns true if supported_humidity_control_mode_type is compatible with current_version; false otherwise 
  ...
  zwave_cc_version_t current_version)
{
  ...
  switch (current_version) {
    case 1:
      compatibility = (supported_humidity_control_mode_type
                       <= HUMIDITY_CONTROL_MODE_SET_MODE_DEHUMIDIFY);
      break;
    case 2:
      compatibility = (supported_humidity_control_mode_type
                       <= HUMIDITY_CONTROL_MODE_SET_MODE_AUTO_V2);
      break;
    default:
      compatibility = false;
  }
  ...
  attribute_store_node_t endpoint_id_node)
{
  ...
  switch (humidity_control_mode_type) {
    case HUMIDITY_CONTROL_MODE_SET_MODE_HUMIDIFY:
      is_supported
        = supported_types & HUMIDITY_CONTROL_MODE_SET_MODE_HUMIDIFY_BITMASK;
      break;
    case HUMIDITY_CONTROL_MODE_SET_MODE_DEHUMIDIFY:
      is_supported
        = supported_types & HUMIDITY_CONTROL_MODE_SET_MODE_DEHUMIDIFY_BITMASK;
      break;
    case HUMIDITY_CONTROL_MODE_SET_MODE_AUTO_V2:
      is_supported
        = supported_types & HUMIDITY_CONTROL_MODE_SET_MODE_AUTO_BITMASK;
      break;
    default:
      is_supported = false;
  }
