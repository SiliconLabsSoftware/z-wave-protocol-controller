...
static uint8_t m_test_public_key_b[]
  = {0xde, 0x9e, 0xdb, 0x7d, 0x7b, 0x7d, 0xc1, 0xb4, 0xd3, 0x5b, 0x61,
     0xc2, 0xec, 0xe4, 0x35, 0x37, 0x3f, 0x83, 0x43, 0xc8, 0x5b, 0x78,
     0x67, 0x4d, 0xad, 0xfc, 0x7e, 0x14, 0x6f, 0x88, 0x2b, 0x4f};

// Obfuscated Public key B where first bytes are masked out as used for DSK input.
static uint8_t m_test_obfuscated_public_key_b[]
  = {0x00, 0x00, 0xdb, 0x7d, 0x7b, 0x7d, 0xc1, 0xb4, 0xd3, 0x5b, 0x61,
     0xc2, 0xec, 0xe4, 0x35, 0x37, 0x3f, 0x83, 0x43, 0xc8, 0x5b, 0x78,
     0x67, 0x4d, 0xad, 0xfc, 0x7e, 0x14, 0x6f, 0x88, 0x2b, 0x4f};
...
static uint8_t m_test_network_key_s2_class_0[] = {0x11,
                                                  0x77,
                                                  0x66,
                                                  0x55,
                                                  0x44,
...
static uint8_t m_test_network_key_s2_class_1[] = {0x88,
                                                  0x77,
                                                  0x66,
                                                  0x55,
                                                  0x44,
...
static uint8_t m_test_network_key_s2_class_2[] = {0x22,
                                                  0x77,
                                                  0x66,
                                                  0x55,
                                                  0x44,
...
void helper_func_echo_kex_report_frame_expect(uint8_t expected_scheme,
                                              uint8_t expected_curves,
                                              uint8_t expected_keys);
void helper_func_kex_set_frame_expect(uint8_t expected_scheme,
                                      uint8_t expected_curves,
                                      uint8_t expected_keys);
...
void test_controller_build(void)
{
  ...
  TEST_FAIL_MESSAGE("ZW_CONTROLLER is not defined but including node (ZW "
                    "Controller) test cases are being executed.");
#endif
}
...
{
  uint8_t public_key_a[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
  ...
  uint8_t private_key_a[]
    = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0,  // Private key.
  ...
  zwave_event_t *p_expected_challenge_event
    = (zwave_event_t *)m_test_mem_pool[1];
  p_expected_challenge_event->event_type
    = S2_NODE_INCLUSION_PUBLIC_KEY_CHALLENGE_EVENT;
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.length
    = sizeof(m_test_public_key_b);
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.granted_keys
    = 0x82;
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.dsk_length = 0;
  memcpy(
    p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.public_key,
    m_test_public_key_b,
    sizeof(m_test_public_key_b));
  ...
  uint8_t S2_pub_key_A_frame[]
    = {COMMAND_CLASS_SECURITY_2,
       PUBLIC_KEY_REPORT,
       0x01,  // Including node bit set
       0x00,
       0x00,
       0x00,
       0x00,
       ...
       0xFF,
       0x11,
       0x22,  // Public key as being set by upper layer.
       0xAA,
       0xBB,
       0xCC,
       0xDD,
  ...
  uint8_t S2_echo_kex_report_frame[] = {COMMAND_CLASS_SECURITY_2,
                                        KEX_REPORT,
                                        0x07,
                                        0x02,
                                        0x01,
  ...
  p_mock->expect_arg[2].pointer
    = S2_echo_kex_report_frame;  // Ideally, this should be updated to be identically to replayed received KEX Set.
  ...
  uint8_t S2_net_key_report_2_frame[19] = {
    COMMAND_CLASS_SECURITY_2,
    SECURITY_2_NETWORK_KEY_REPORT,
    0x02,
  };
  memcpy(&S2_net_key_report_2_frame[3],
  ...
  uint8_t S2_net_key_report_0_frame[19] = {
    COMMAND_CLASS_SECURITY_2,
    SECURITY_2_NETWORK_KEY_REPORT,
    0x80,
  };
  ...
  uint8_t public_key_frame[3 + sizeof(m_test_public_key_b)] = {
    COMMAND_CLASS_SECURITY_2,
    PUBLIC_KEY_REPORT,
  ...
  uint8_t s2_echo_kex_set_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_SET, 0x03, 0x02, 0x01, 0x82};
...
/** Verification that the normal flow succeeds in inclusion of a new node when using SSA.
  ...
  */
static void
  kex_including_node_state_machine_bad_tansfer_end(uint8_t transfer_end_code)
{
  uint8_t public_key_a[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
                            0x11, 0x22,  // Public key as being set by upper layer.
                            0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF,
  ...
  uint8_t public_key_frame[3 + sizeof(m_test_obfuscated_public_key_b)] = {
    COMMAND_CLASS_SECURITY_2,
    PUBLIC_KEY_REPORT,
    0x00};  // Key exchange received from slave - public key for secure exchange of LTK.
  memcpy(&public_key_frame[3],
  ...
  uint8_t s2_transfer_end_frame[]
    = {COMMAND_CLASS_SECURITY_2, SECURITY_2_TRANSFER_END, transfer_end_code};
  s2_context.buf    = s2_transfer_end_frame;
  s2_context.length = sizeof(s2_transfer_end_frame);
  s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_including_node_state_machine_bad_tansfer_end2(
  uint8_t transfer_end_code)
{
  // S2 Transfer end frame received.
...
void test_kex_including_node_state_machine_bad_tansfer_end3(
  uint8_t transfer_end_code)
{
  // S2 Transfer end frame received.
  // bit0: Key request complete set,
...
void test_kex_including_node_sond_complete(void)
{
  ...
  s2_conn.class_id  = 0xFF;
  s2_inclusion_post_event(&s2_context, &s2_conn);
  TEST_ASSERT_EQUAL_UINT8(S2_AWAITING_KEY_USER_ACCEPT,
                          s2_context.inclusion_state);
  ...
  s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_context, &s2_conn);
  TEST_ASSERT_EQUAL_UINT8(S2_AWAITING_NET_KEY_VERIFY,
                          s2_context.inclusion_state);
...
void test_kex_inclusion_timeout_kex_set_with_retry(void)
{
  ...
  uint8_t s2_kex_report_frame[]
    = {COMMAND_CLASS_SECURITY_2,
       ...
       0x02,
       0x01,
       0x82};  // Scheme 1, curve25519, S2, key 1, and S0 network key.
  s2_context.buf    = s2_kex_report_frame;
  s2_context.length = sizeof(s2_kex_report_frame);
  s2_conn.class_id  = 0xFF;
  s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_inclusion_timeout_all_states(void)
{
  ...
  for (i = 0; i < NO_OF_STATES; i++) {
    ...
    if (i == 0) {
      ...
      zwave_event_t *p_expected_event = (zwave_event_t *)m_test_mem_pool[0];
      p_expected_event->event_type    = S2_NODE_INCLUSION_FAILED_EVENT;
      p_expected_event->evt.s2_event.s2_data.inclusion_fail.kex_fail_type
        = 0x00;
      mock_call_expect(TO_STR(s2_event_handler), &p_mock);
      p_mock->expect_arg[0].pointer = p_expected_event;
    ...
    // When the KEX Report is received, we expect that the event from libs2 contains the key requested in order to present it for the operator.
    // The operator (or including app automatically) can then respond with granted keys.
    zwave_event_t *p_expected_report_event
      = (zwave_event_t *)m_test_mem_pool[0];
    p_expected_report_event->event_type = S2_NODE_INCLUSION_KEX_REPORT_EVENT;
    p_expected_report_event->evt.s2_event.s2_data.kex_report.security_keys
      = 0x82;
    mock_call_expect(TO_STR(s2_event_handler), &p_mock);
    p_mock->expect_arg[0].pointer = p_expected_report_event;
    ...
    // Expect Net Key Set to be sent.
    static uint8_t S2_net_key_set_frame[19]
      = {COMMAND_CLASS_SECURITY_2, SECURITY_2_NETWORK_KEY_REPORT, 0x02};
    memcpy(&S2_net_key_set_frame[3],
           m_test_network_key_s2_class_1,
           sizeof(m_test_network_key_s2_class_1));
    mock_call_expect(TO_STR(S2_send_data), &p_mock);
    p_mock->compare_rule_arg[0]
    ...
    // Expect S2 Transfer End to be sent.
    static uint8_t S2_transfer_end_frame[]
      = {COMMAND_CLASS_SECURITY_2, SECURITY_2_TRANSFER_END, 0x02};
    mock_call_expect(TO_STR(S2_send_data), &p_mock);
    p_mock->compare_rule_arg[0]
      = COMPARE_ANY;  // For the outline, we just expect any/null pointers now.
...
void test_kex_inclusion_invalid_frame_order_all_states(void)
{
  ...
  for (i = 0; i < NO_OF_STATES; i++) {
    ...
    // Expect a S2 KEX Set to be sent.
    static uint8_t S2_kex_set_frame[]
      = {COMMAND_CLASS_SECURITY_2, KEX_SET, 0x00, 0x02, 0x01, 0x82};
    mock_call_expect(TO_STR(S2_send_frame), &p_mock);
    p_mock->compare_rule_arg[0]
      = COMPARE_ANY;  // For the outline, we just expect any/null pointers now.
    ...
    // Expect Echo(KEX Report) to be sent.
    static uint8_t S2_echo_kex_report_frame[]
      = {COMMAND_CLASS_SECURITY_2, KEX_REPORT, 0x01, 0x02, 0x01, 0x82};
    mock_call_expect(TO_STR(S2_send_data), &p_mock);
    p_mock->compare_rule_arg[0]
      = COMPARE_ANY;  // For the outline, we just expect any/null pointers now.
    ...
    if (i == 3) {
      ...
      uint8_t S2_kex_error_frame[]
        = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, 0x07};  // KEX_FAIL_AUTH = 0x05
      mock_call_expect(TO_STR(S2_send_frame), &p_mock);
      p_mock->compare_rule_arg[0]
        = COMPARE_ANY;  // For the outline, we just expect any/null pointers now.
      p_mock->compare_rule_arg[1]
    ...
    if (i == 5) {
      ...
      // Go to transfer end without verifying the key.
      uint8_t s2_transfer_end_frame[]
        = {COMMAND_CLASS_SECURITY_2, SECURITY_2_TRANSFER_END, 0x03};
      ...
      memcpy((uint8_t *)s2_context.buf,
             s2_transfer_end_frame,
             sizeof(s2_transfer_end_frame));
      s2_context.length = sizeof(s2_transfer_end_frame);
      s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
      s2_inclusion_post_event(&s2_context, &s2_conn);
      s2_context.buf = frame_buffer;  // Reset buffer back.
...
void test_kex_inclusion_duplicate_frame_order_all_states(void)
{
  ...
  static uint8_t S2_net_key_set_frame_s0[19]
    = {COMMAND_CLASS_SECURITY_2, SECURITY_2_NETWORK_KEY_REPORT, 0x80};
  memcpy(&S2_net_key_set_frame_s0[3],
         m_test_network_key_s0,
         sizeof(m_test_network_key_s0));
  mock_call_expect(TO_STR(S2_send_data), &p_mock);
  p_mock->compare_rule_arg[0]
...
void test_kex_inclusion_error_schemes(void)
{
  ...
  uint8_t scheme_scheme_fail_test_vector[][2] = {
    {0x00,
     0x00},  // Scheme 2 = false, scheme 0 = false, key request don't care. This should return FAIL_KEY_REPORT.
    {0x01,
     0x80},  // Scheme 2 = false, scheme 0 = true,  key request don't care. This should return FAIL_KEY_REPORT.
  };
...
void test_kex_inclusion_error_keys(void)
{
  ...
  for (i = 0; i < ELEM_COUNT(scheme_key_request_fail_test_vector); i++) {
    ...
    s2_kex_report_frame[4]
      = 0x01,  // Supported ECDH Profiles, bit0=1 is curve 25519 value.
      s2_kex_report_frame[5]
      = scheme_key_request_fail_test_vector[i];  // Requested keys bit.
    s2_context.buf    = s2_kex_report_frame;
    s2_context.length = sizeof(s2_kex_report_frame);
    s2_conn.class_id  = 0xFF;
    s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_inclusion_valid_keys(void)
{
  ...
  uint8_t S2_kex_set_frame[]
    = {COMMAND_CLASS_SECURITY_2,
       ...
       0x02,
       0x01,
       0x82};  // Scheme 0 and 2, curve25519, S2, key 2, and S0 network key.
  ...
  uint8_t scheme_key_request_pass_test_vector[][4] = {
    ...
    {0x02,
     0x04,
     0x02,
     0x04},  // Scheme 1 = true, key request S2 = yes, key request S0 = no.  Valid request.
    {0x02,
     0x05,
     0x02,
     0x05},  // Scheme 1 = true, key request S2 = yes, key request S0 = no.  Valid request.
    {0x02,
     0x06,
     0x02,
     0x06},  // Scheme 1 = true, key request S2 = yes, key request S0 = no.  Valid request.
    {0x02,
     0x07,
     0x02,
     0x07},  // Scheme 1 = true, key request S2 = yes, key request S0 = no.  Valid request.
    {0x02,
     0x80,
     0x02,
    ...
    {0x02,
     0x84,
     0x02,
     0x84},  // Scheme 1 = true, key request S2 = yes, key request S0 = yes. Valid request.
    {0x02,
     0x85,
     0x02,
     0x85},  // Scheme 1 = true, key request S2 = yes, key request S0 = yes. Valid request.
    {0x02,
     0x86,
     0x02,
     0x86},  // Scheme 1 = true, key request S2 = yes, key request S0 = yes. Valid request.
    {0x02,
     0x87,
     0x02,
     0x87},  // Scheme 1 = true, key request S2 = yes, key request S0 = yes. Valid request.
  };
  ...
  for (i = 0; i < ELEM_COUNT(scheme_key_request_pass_test_vector); i++) {
    ...
    s2_kex_report_frame[2] = 0x00,  // bit 0 is echo field, rest are reserved
      s2_kex_report_frame[3]
      = scheme_key_request_pass_test_vector[i][0];  // Supported schemes.
    s2_kex_report_frame[4]
      = 0x01,  // Supported ECDH Profiles, bit0=1 is curve 25519 value.
      s2_kex_report_frame[5]
      = scheme_key_request_pass_test_vector[i][1];  // Requested keys bit.
    s2_context.buf    = s2_kex_report_frame;
    s2_context.length = sizeof(s2_kex_report_frame);
    s2_conn.class_id  = 0xFF;
    s2_inclusion_post_event(&s2_context, &s2_conn);

    s2_inclusion_key_grant(&s2_context,
                           1,
                           scheme_key_request_pass_test_vector[i][3],
                           0);  // Granted keys.
...
void test_kex_inclusion_error_key_get_request(void)
{
  ...
  uint8_t frame_buffer[UNIT_TEST_BUF_SIZE];
  uint8_t s2_kex_report_frame[6]
    = {COMMAND_CLASS_SECURITY_2, KEX_REPORT, 0x00, 0x00, 0x01, 0x00};
  uint8_t s2_echo_kex_set_frame[6]
    = {COMMAND_CLASS_SECURITY_2, KEX_SET, 0x01, 0x00, 0x01, 0x00};
  uint8_t s2_net_key_get_frame[6]
  ...
  uint8_t S2_kex_error_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, 0x08};  // KEX_FAIL_KEY_GET = 0x08
  ...
  uint8_t key_get_request_fail_test_vector[][3] = {
    {0x02,
     0x01,
     0x02},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S2 class 1.
    {0x02,
     0x01,
     0x04},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request S2 class 2.
    {0x02,
     0x01,
     0x80},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S0 Network key.
    {0x02,
     0x02,
     0x01},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S2 class 0.
    {0x02,
     0x02,
     0x04},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S2 class 2.
    {0x02,
     0x02,
     0x08},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S2 class x.
    {0x02,
     0x02,
     0x80},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S0 Network key.
    {0x02,
    ...
    {0x02,
     0x03,
     0x08},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request S2 class x.
    {0x02,
     0x03,
     0x80},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S0 Network key.
    {0x02,
    ...
    {0x02,
     0x06,
     0x01},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S2 class 1.
    {0x02,
     0x06,
     0x08},  // Scheme 2 = true,  scheme 0 = false, key request S2 = yes - Request S2 class x.
    {0x02,
    ...
    {0x02,
     0x81,
     0x02},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request S2 class 1.
    {0x02,
     0x81,
     0x04},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request S2 class 2.
    {0x02,
     0x81,
     0x08},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request unknown/unsupported key.
    {0x02,
     0x82,
     0x01},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request S2 class 0.
    {0x02,
     0x82,
     0x04},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request S2 class 2.
    {0x02,
    ...
    {0x02,
     0x83,
     0x10},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request unknown/unsupported key.
    {0x02,
     0x83,
     0x20},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request unknown/unsupported key.
    {0x02,
     0x83,
     0x40},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request unknown/unsupported key.
  };
  ...
  for (i = 0; i < ELEM_COUNT(key_get_request_fail_test_vector); i++) {
    ...
    helper_func_kex_set_frame_expect(key_get_request_fail_test_vector[i][0],
                                     0x01,
                                     key_get_request_fail_test_vector[i][1]);
    ...
    p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.granted_keys
      = key_get_request_fail_test_vector[i][1];
    p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.dsk_length
      = ((key_get_request_fail_test_vector[i][1] & 0x06) ? 2 : 0);
    memcpy(
      p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.public_key,
      m_test_public_key_b,
      sizeof(m_test_public_key_b));
    ...
    helper_func_echo_kex_report_frame_expect(
      key_get_request_fail_test_vector[i][0],
      0x01,
      key_get_request_fail_test_vector[i][1]);
    ...
    // KEX Report frame received - Valid mix of schemes and keys.
    s2_kex_report_frame[3]
      = key_get_request_fail_test_vector[i][0];  // Supported schemes.
    s2_kex_report_frame[5]
      = key_get_request_fail_test_vector[i][1];  // Requested keys bit.
    s2_context.buf    = s2_kex_report_frame;
    s2_context.length = sizeof(s2_kex_report_frame);
    s2_conn.class_id  = 0xFF;
    s2_inclusion_post_event(&s2_context, &s2_conn);
    ...
    s2_echo_kex_set_frame[3]
      = key_get_request_fail_test_vector[i][0];  // Supported schemes.
    s2_echo_kex_set_frame[5]
      = key_get_request_fail_test_vector[i][1];  // Requested keys bit.
    s2_context.buf    = s2_echo_kex_set_frame;
    s2_context.length = sizeof(s2_echo_kex_set_frame);
    s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
    ...
    s2_net_key_get_frame[2]
      = key_get_request_fail_test_vector[i][2];  // Requested key.
    s2_context.buf    = s2_net_key_get_frame;
    s2_context.length = sizeof(s2_net_key_get_frame);
    s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
    s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_inclusion_key_grant_user_reject(void)
{
  ...
  uint8_t frame_buffer[UNIT_TEST_BUF_SIZE];
  uint8_t s2_kex_report_frame[6]
    = {COMMAND_CLASS_SECURITY_2, KEX_REPORT, 0x00, 0x02, 0x01, 0x83};

  struct S2 s2_context;
  s2_connection_t s2_conn;
  uint8_t S2_kex_error_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, 0x06};  // KEX_FAIL_CANCEL = 0x06
...
void test_kex_inclusion_user_reject(void)
{
  ...
  uint8_t frame_buffer[UNIT_TEST_BUF_SIZE];
  uint8_t s2_kex_report_frame[6]
    = {COMMAND_CLASS_SECURITY_2, KEX_REPORT, 0x00, 0x02, 0x01, 0x81};
...
void test_kex_inclusion_pending_user_accept_echo_kex_set_received(void)
{
  ...
  // KEX Echo Set frame received - Valid mix of schemes and keys.
  memcpy((uint8_t *)s2_context.buf,
         s2_echo_kex_set_frame,
         sizeof(s2_echo_kex_set_frame));
  s2_context.length = sizeof(s2_echo_kex_set_frame);
  s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_inclusion_error_multi_key_get_request(void)
{
  ...
  uint8_t key_get_request_fail_test_vector[][3] = {
    {0x02,
     0x83,
     0x83},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request all keys.
    {0x02,
     0x83,
     0x03},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - Request all S2 keys.
    {0x02,
     0x83,
     0x81},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - two keys.
    {0x02,
     0x83,
     0x82},  // Scheme 2 = true,  scheme 0 = true,  key request S2 = yes - two keys.
  };
...
void test_kex_inclusion_error_public_key_including_bit(void)
{
  ...
  uint8_t frame_buffer[UNIT_TEST_BUF_SIZE];
  uint8_t s2_kex_report_frame[6]
    = {COMMAND_CLASS_SECURITY_2, KEX_REPORT, 0x00, 0x02, 0x01, 0x02};
  struct S2 s2_context;
...
void test_kex_inclusion_public_key_reserved_bits(void)
{
  ...
  frame_buffer[1] = PUBLIC_KEY_REPORT;
  frame_buffer[2]
    = 0xA2;  // Setting a couple of reserved bits which should be ignored on remote side.
  memcpy(&frame_buffer[3], m_test_public_key_b, sizeof(m_test_public_key_b));
  s2_context.buf    = frame_buffer;
  s2_context.length = 3 + sizeof(m_test_public_key_b);
  s2_conn.class_id  = 0xFF;
...
void test_kex_inclusion_error_echo_kex_set(void)
{
  ...
  uint8_t key_echo_fail_test_vector[][4] = {
    {0x02,
     0x01,
     0x02,
     0x02},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x01,
     0x02,
     0x03},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x01,
     0x02,
     0x04},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x01,
     0x02,
     0x80},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x02,
     0x02,
     0x01},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x02,
     0x02,
     0x04},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    ...
    {0x02,
     0x81,
     0x02,
     0x82},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x81,
     0x02,
     0x83},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x81,
     0x02,
     0x84},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x81,
     0x02,
     0x80},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x82,
     0x02,
     0x81},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x82,
     0x02,
     0x83},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    ...
    {0x02,
     0x83,
     0x02,
     0x07},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x83,
     0x02,
     0x78},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x83,
     0x02,
     0xFF},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
    {0x02,
     0x83,
     0x02,
     0x00},  // Scheme 1 = true, key request S2 = yes - Wrong echo frame.
  ...
  for (i = 0; i < ELEM_COUNT(key_echo_fail_test_vector); i++) {
    ...
    helper_func_kex_set_frame_expect(key_echo_fail_test_vector[i][0],
                                     0x01,
                                     key_echo_fail_test_vector[i][1]);
    ...
    p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.granted_keys
      = key_echo_fail_test_vector[i][1];
    p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.dsk_length
      = (key_echo_fail_test_vector[i][1] & 0x06) ? 2 : 0;
    memcpy(
      p_expected_inc_req_event->evt.s2_event.s2_data.challenge_req.public_key,
      m_test_public_key_b,
      sizeof(m_test_public_key_b));
    ...
    // KEX Report frame received - Valid mix of schemes and keys.
    s2_kex_report_frame[3]
      = key_echo_fail_test_vector[i][0];  // Supported schemes.
    s2_kex_report_frame[5]
      = key_echo_fail_test_vector[i][1];  // Requested keys bit.
    s2_context.buf    = s2_kex_report_frame;
    s2_context.length = sizeof(s2_kex_report_frame);
    s2_conn.class_id  = 0xFF;
    s2_inclusion_post_event(&s2_context, &s2_conn);
    ...
    // KEX Echo Set frame received - Valid mix of schemes and keys.
    s2_echo_kex_set_frame[3]
      = key_echo_fail_test_vector[i][2];  // Wrong match supported schemes.
    s2_echo_kex_set_frame[5]
      = key_echo_fail_test_vector[i][3];  // Wrong match requested keys bit.
    s2_context.buf    = s2_echo_kex_set_frame;
    s2_context.length = sizeof(s2_echo_kex_set_frame);
    s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
    s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_including_node_decrypt_failed(void)
{
  ...
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.granted_keys
    = 0x82;
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.dsk_length = 2;
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.public_key[0]
    = 0x00;
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.public_key[1]
    = 0x00;
  memcpy(&p_expected_challenge_event->evt.s2_event.s2_data.challenge_req
            .public_key[2],
         &m_test_public_key_b[2],
         sizeof(m_test_public_key_b) - 2);
  ...
  // When inclusion fails, we expect a KEX Fail to be sent to inclusion_peer.
  uint8_t S2_kex_error_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, 0x05};  // KEX_FAIL_DECRYPT = 0x05
...
void test_kex_including_node_csa_unauthenticated_only(void)
{
  ...
  // Keys to exchange, Security2, class 2 - Security0, network key.
  uint8_t s2_echo_kex_set_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_SET, 0x03, 0x02, 0x01, 0x81};
  s2_context.buf    = s2_echo_kex_set_frame;
  s2_context.length = sizeof(s2_echo_kex_set_frame);
  s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_context, &s2_conn);
...
void test_kex_inclusion_kex_fail_frame_all_states(void)
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (i == 0) {
      // The KEX Fail frame.
      uint8_t s2_error_frame[]
        = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, KEX_FAIL_KEX_KEY};
    ...
    if (i == 1) {
      // The KEX Fail frame.
      uint8_t s2_error_frame[]
        = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, KEX_FAIL_CANCEL};
    ...
    if (i == 2) {
      // The KEX Fail frame.
      uint8_t s2_error_frame[]
        = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, KEX_FAIL_AUTH};
    ...
    if (i == 3) {
      // The KEX Fail frame.
      uint8_t s2_error_frame[]
        = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, KEX_FAIL_KEY_REPORT};
...
void test_kex_inclusion_net_key_verify_temp_key_encrypted_error(void)
{
  ...
  uint8_t S2_kex_error_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_FAIL, 0x07};  // KEX_FAIL_KEY_GET = 0x08
...
{
  mock_t *p_mock;
  static uint8_t s2_kex_set_frame[6]
    = {COMMAND_CLASS_SECURITY_2, KEX_SET, 0x00, 0x00, 0x00, 0x00};
                                              ...
                                              uint8_t expected_keys)
{
  ...
  // Expect Echo(KEX Report) to be sent.
  static uint8_t S2_echo_kex_report_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_REPORT, 0x01, 0x00, 0x00, 0x00};
...
void helper_func_echo_kex_set_frame(struct S2 *p_context)
{
  uint8_t s2_frame[] = {
    ...
    0x02,
    0x01,
    0x82};  // This data must be compared to earlier sent data. As long as frames are not defined, this data is unknown.
  memcpy((uint8_t *)p_context->buf, s2_frame, sizeof(s2_frame));
  p_context->length = sizeof(s2_frame);
  s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
}
...
{
  uint8_t s2_frame[]
    = {COMMAND_CLASS_SECURITY_2, SECURITY_2_NETWORK_KEY_GET, key};
  memcpy((uint8_t *)p_context->buf, s2_frame, sizeof(s2_frame));
  p_context->length = sizeof(s2_frame);
  s2_conn.class_id  = UNIT_TEST_TEMP_KEY_SECURE;
}
...
void kex_including_node_state_machine_no_keys(int positive_test)
{
  ...
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.length
    = sizeof(m_test_obfuscated_public_key_b);
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.granted_keys
    = positive_test ? 0x00 : 0x82;
  p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.dsk_length
    = positive_test ? 0 : 2;
  memcpy(
    p_expected_challenge_event->evt.s2_event.s2_data.challenge_req.public_key,
    m_test_obfuscated_public_key_b,
...
void test_kex_including_node_LR_unauth_keys(void)
{
  ...
  p_expected_report_event->event_type    = S2_NODE_INCLUSION_KEX_REPORT_EVENT;
  p_expected_report_event->evt.s2_event.s2_data.kex_report.security_keys
    = 0x02;  // 0x01 bit must be filtered out by libs2
  p_expected_report_event->evt.s2_event.s2_data.kex_report.csa = 0x00;
  ...
  mock_call_expect(TO_STR(keystore_network_key_read), &p_mock);
  p_mock->expect_arg[0].value = 0x08;
  p_mock->output_arg[1].pointer
    = m_test_network_key_s2_class_0;  // Just using a random key here, arg0 with the index is what matters
  ...
  // Expect S2 Transfer End to be sent.
  uint8_t S2_transfer_end_frame[] = {COMMAND_CLASS_SECURITY_2,
                                     SECURITY_2_TRANSFER_END,
                                     0x02};  // 0x02 = Key Verified
  mock_call_expect(TO_STR(S2_send_data), &p_mock);
  p_mock->compare_rule_arg[0]
    = COMPARE_ANY;  // For the outline, we just expect any/null pointers now.
  p_mock->compare_rule_arg[1]
  ...
  uint8_t s2_echo_kex_set_frame[]
    = {COMMAND_CLASS_SECURITY_2, KEX_SET, 0x01, 0x02, 0x01, 0x02};
