...
zwave_controller_encapsulation_scheme_t zwave_controller_get_highest_encapsulation(zwave_keyset_t keyset)
{
    ...
    if (keyset & ZWAVE_CONTROLLER_S2_AUTHENTICATED_KEY) {
        return ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_AUTHENTICATED;
    }
    if (keyset & ZWAVE_CONTROLLER_S2_UNAUTHENTICATED_KEY) {
        return ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_UNAUTHENTICATED;
    }
    if (keyset & ZWAVE_CONTROLLER_S0_KEY) {
...
zwave_keyset_t zwave_controller_get_key_from_encapsulation(zwave_controller_encapsulation_scheme_t encapsulation)
{
    ...
    if (encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_AUTHENTICATED) {
        return ZWAVE_CONTROLLER_S2_AUTHENTICATED_KEY;
    }
    if (encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_UNAUTHENTICATED) {
        return ZWAVE_CONTROLLER_S2_UNAUTHENTICATED_KEY;
    }
    if (encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_0) {
