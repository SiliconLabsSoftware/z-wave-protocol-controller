...
typedef enum { DATASTORE_VALUE_TYPE_INT, DATASTORE_VALUE_TYPE_BLOB } datastore_value_type_t;

////////////////////////////////////////////////////////////////////////////////
// Private functions
////////////////////////////////////////////////////////////////////////////////
static bool datastore_table_exists(const char *table_name)
{
    ...
    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "Failed to check table presence: %s\n", sqlite3_errmsg(db));
        return false;
    }
...
static sl_status_t datastore_migrate_legacy_table(const char *legacy_table_name, const char *new_table_name)
{
    if (datastore_table_exists(new_table_name) || !datastore_table_exists(legacy_table_name)) {
        return SL_STATUS_OK;
    }

    char sql[200] = {0};
    snprintf(sql, sizeof(sql), "ALTER TABLE %s RENAME TO %s;", legacy_table_name, new_table_name);
    if (datastore_exec_sql(sql) != SQLITE_OK) {
        return SL_STATUS_FAIL;
    }

    sl_log_info(LOG_TAG, "Migrated legacy datastore table %s -> %s\n", legacy_table_name, new_table_name);
    return SL_STATUS_OK;
}

sl_status_t datastore_table_init()
{
    if (datastore_migrate_legacy_table(LEGACY_DATASTORE_TABLE_INT, DATASTORE_TABLE_INT) != SL_STATUS_OK || datastore_migrate_legacy_table(LEGACY_DATASTORE_TABLE_BLOB, DATASTORE_TABLE_BLOB) != SL_STATUS_OK
        || datastore_migrate_legacy_table(LEGACY_DATASTORE_TABLE_ATTRIBUTES, DATASTORE_TABLE_ATTRIBUTES) != SL_STATUS_OK) {
        sqlite3_close(db);
        return SL_STATUS_FAIL;
    }

    // The integer table:
    int rc = datastore_exec_sql("CREATE TABLE IF NOT EXISTS " DATASTORE_TABLE_INT " ("
...
static sl_status_t datastore_fetch_internal(const datastore_key_t key, void *value, unsigned int *size, datastore_value_type_t value_type)
{
    ...
    const char *sql;
    if (db == NULL) {
        sl_log_error(LOG_TAG, "Datastore is not initialized. Fetching data failed.\n");
        return SL_STATUS_FAIL;
    }
...
static sl_status_t datastore_store_internal(const datastore_key_t key, const void *value, unsigned int size, datastore_value_type_t value_type)
{
    ...
    const char *sql;
    if (db == NULL) {
        sl_log_error(LOG_TAG, "Datastore is not initialized. Writing data failed.\n");
        return SL_STATUS_FAIL;
    }
    if (value_type == DATASTORE_VALUE_TYPE_INT) {
