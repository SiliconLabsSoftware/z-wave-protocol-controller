...
/**
 * @brief Check if a node wants supervision encapsulation for outgoing frames.
 *
 * Invokes COMMAND_CLASS_SUPERVISION_CHECK_SUPERVISION_SUPPORT event.
 *
 * @param node_id     The NodeID to check.
 * @param endpoint_id The Endpoint ID to check.
 *
 * @returns true if the node/endpoint supports supervision, false otherwise.
 */
bool zwave_command_class_supervision_want_supervision_frame(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id);
