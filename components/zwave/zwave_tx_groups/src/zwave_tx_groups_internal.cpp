...
zwave_multicast_group_id_t zwave_tx_groups::get_next_group_to_reuse()
{
    ...
    while (this->next_group_id_to_reuse != start_group_id && this->is_group_locked(this->next_group_id_to_reuse)) {
        this->increment_next_group_id();
    }
...
bool zwave_tx_groups::is_node_list_empty(const zwave_nodemask_t nodes)
{
    for (zwave_node_id_t n = ZW_MIN_NODE_ID; n <= ZW_LR_MAX_NODE_ID; n++) {
        if (ZW_IS_NODE_IN_MASK(n, nodes)) {
            return false;
        }
    }
    return true;
...
sl_status_t zwave_tx_groups::analyze_group_suitability(const zwave_nodemask_t list, const zwave_nodemask_t group_nodes, uint16_t *common_nodes)
{
...
zwave_multicast_group_id_t zwave_tx_groups::get_most_similar_subgroup(const zwave_nodemask_t nodes) const
{
    ...
    for (auto it = this->groups.begin(); it != this->groups.end(); ++it) {
        ...
        group_suitability             = zwave_tx_groups::analyze_group_suitability(nodes, it->node_list, &common_nodes);
...
sl_status_t zwave_tx_groups::get_group_data(zwave_tx_group_t *group_data, zwave_multicast_group_id_t group_id) const
{
...
zwave_tx_groups::zwave_tx_groups(void) : next_group_id_to_reuse(ZWAVE_TX_INVALID_GROUP) {}

// Nothing to do on destruction.
zwave_tx_groups::~zwave_tx_groups(void) = default;

sl_status_t zwave_tx_groups::add_node_to_group(zwave_node_id_t node_id, zwave_multicast_group_id_t group_id)
{
    sl_log_debug(LOG_TAG, "Manually adding NodeID %d to Group ID %d", node_id, group_id);

    // Bail out if the Group is locked.
    if (this->is_group_locked(group_id)) {
        sl_log_warning(LOG_TAG, "GroupID %d is locked. NodeID %d will not be added.", group_id, node_id);
...
sl_status_t zwave_tx_groups::assign_group(const zwave_nodemask_t nodes, zwave_multicast_group_id_t *group_id)
{
    if (zwave_tx_groups::is_node_list_empty(nodes)) {
        sl_log_warning(LOG_TAG, "Invalid TX group. You need at least 2 nodes to form a Tx group");
...
sl_status_t zwave_tx_groups::get_nodes(zwave_nodemask_t nodes, zwave_multicast_group_id_t group_id)
{
    for (auto it = this->groups.begin(); it != this->groups.end(); ++it) {
        if (it->group_id == group_id) {
