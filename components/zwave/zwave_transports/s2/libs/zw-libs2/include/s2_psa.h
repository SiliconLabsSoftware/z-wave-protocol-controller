 ...
 */
typedef enum { ZWAVE_KEY_TYPE_SINGLE_CAST = 0x01, ZWAVE_KEY_TYPE_MULTI_CAST, ZWAVE_KEY_TYPE_NONCE } zwave_derived_key_type_t;
 ...
 */
extern zw_status_t zw_compute_ecdh_shared_secret(const uint8_t *remote_public_key, uint32_t keyid, uint8_t *result);
 ...
 */
extern zw_status_t zw_psa_aead_encrypt_ccm(uint32_t key_id, const uint8_t *nonce, size_t nonce_len, const uint8_t *ad, size_t ad_len, const uint8_t *plain_text, size_t plain_text_len, uint8_t *cipher_text, size_t cipher_text_len, size_t *cipher_len);
/**
 * \brief PSA wrapper for  Authenticated Decryption with Associated Data (AEAD) - PSA_ALG_CCM.
 */
extern zw_status_t zw_psa_aead_decrypt_ccm(uint32_t key_id, const uint8_t *nonce, size_t nonce_len, const uint8_t *ad, size_t ad_len, const uint8_t *plain_text, size_t plain_text_len, uint8_t *cipher_text, size_t cipher_text_len, size_t *cipher_len);
