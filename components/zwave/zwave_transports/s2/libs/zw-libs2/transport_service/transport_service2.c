...
/* Note: ZW_ctimer.h was replaced with timer.h */
/* For clock_time_t, we rely on ZW_ctimer.h's definition or timer.h's fallback */
#include "timer.hpp"
#include "zgw_crc.h"
...
#ifdef ZIPGW
bool ZW_TransportService_SendData(ts_param_t *p, const uint8_t *pData, uint16_t dataLength, void (*completedFunc)(uint8_t txStatus, TX_STATUS_TYPE *t))
#else
#if defined(EFR32ZG) || defined(ZWAVE_ON_LINUX)
bool ZW_TransportService_SendData(ts_param_t *p, const uint8_t *pData, uint16_t dataLength, void (*completedFunc)(uint8_t txStatus, void *t))
#else
bool ZW_TransportService_SendData(ts_param_t *p, uint8_t *pData, uint16_t dataLength, ZW_TransportService_SendData_Callback_t completedFunc)
#endif
#endif
{
#if defined(ZIPGW)
...
#endif
    timer_set(&scb.reset_timer, RESET_TIME, FUNC(reset_transport_service), 0);
...
static void send_last_frag(void)
{
    ...
    uint8_t ret = 0;
retry:
    timer_stop(&rcb.fc_timer); /* FIXME this is called twice. First in send_subseq_frag() ? */
    timer_set(&scb.reset_timer, RESET_TIME, FUNC(reset_transport_service), 0);
    scb.sending = false;
    ...
    if (scb.does_not_fit_in_first_frag) {
        ret = TS_SEND_RAW(scb.cmn.p.snode, scb.cmn.p.dnode, t2_txBuf, sizeof(*subseq_frag) + scb.datalen_to_send - 1, scb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, ZCB_temp_callback_last_frag);
    } else {
        ret = TS_SEND_RAW(scb.cmn.p.snode, scb.cmn.p.dnode, t2_txBuf, sizeof(*first_frag) + scb.datalen_to_send - 1, scb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, ZCB_temp_callback_last_frag);
    }
...
#ifdef TIMER
    if ((scb.cmn.p.tx_flags == RECEIVE_STATUS_TYPE_BROAD) || (scb.cmn.p.dnode == 0xff)) {
        t2_sm_post_event(EV_MISSING_FRAG_BCAST);
    } else {
        timer_set(&rcb.fc_timer, FRAGMENT_FC_TIMEOUT, FUNC(fc_timer_expired), 0);
...
#endif
{
    ...
    */
    if (scb.transmission_aborted == scb.cmn.session_id) {
        ...
        timer_set(&scb.timer_btwn_2_frags, 15 + (scb.round_trip_first_frag), FUNC(send_subseq_frag), 0);
#ifdef NEW_TEST_T2
...
static void send_subseq_frag(__attribute__((unused)) void *nthing)
{
    ...
    timer_set(&scb.reset_timer, RESET_TIME, FUNC(reset_transport_service), 0);
    ret = TS_SEND_RAW(scb.cmn.p.snode, scb.cmn.p.dnode, t2_txBuf, sizeof(*subseq_frag) + scb.datalen_to_send - 1, scb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, ZCB_ts_senddata_cb);
    if (0 != ret) {
        if (scb.remaining_data_len >= FragmentMaxPayload) {
...
/* Only 4 bits for session id, so max session id can be 0xf */
    ...
    timer_set(&scb.reset_timer, RESET_TIME, FUNC(reset_transport_service), 0);
    ret = TS_SEND_RAW(scb.cmn.p.snode, scb.cmn.p.dnode, t2_txBuf, sizeof(*first_frag) + scb.datalen_to_send - 1, scb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, ZCB_ts_senddata_cb);
    if (ret == 0) {
        return;
    }
...
static void reply_frag_req(__attribute__((unused)) void *nthing)
{
    ...
    memcpy((uint8_t *)&subseq_frag->payload1, (scb.datagram + scb.missing_offset), scb.datalen_to_send);
    ...
    if (scb.sending) {
        ...
        ret = TS_SEND_RAW(scb.cmn.p.snode, scb.cmn.p.dnode, t2_txBuf, sizeof(*subseq_frag) + scb.datalen_to_send - 1, scb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, ZCB_temp_callback_reply_frag_req);
    }
    ...
    timer_set(&rcb.fc_timer, (FRAGMENT_FC_TIMEOUT + 500), FUNC(fc_timer_expired), 0);
}
...
static void rx_timer_expired(void *ss)
{
    ...
    /*
        timer_set(&rcb.rx_timer, FRAGMENT_RX_TIMEOUT, ZCB_rx_timer_expired, 0);
    */
}
#endif
...
#ifdef TIMER
            timer_set(&rcb.rx_timer, FRAGMENT_RX_TIMEOUT, FUNC(rx_timer_expired), &rcb.rx_data);
#endif
...
#define SUBSEQ_HDR_LEN 5 /* Cmd class, cmd, size, seqno + offset 1, offset 2*/
        ...
        case COMMAND_SEGMENT_WAIT_V2:
            ...
            timer_set(&scb.wait_restart_timer, (100 + 100 * rcb.cmn.pending_segments), FUNC(wait_restart_from_first), NULL);
            break;
        default:
...
static uint8_t send_frag_wait_cmd(void)
{
    ...
    if (scb.sending) { /* If there is a sending session going on FRAG_WAIT will be queed for next callback*/
        ...
        ret                        = TS_SEND_RAW(scb.frag_wait_p.dnode, scb.frag_wait_p.snode, (uint8_t *)&frag_wait, sizeof(frag_wait), scb.frag_wait_p.tx_flags | TRANSMIT_OPTION_ACK, ZCB_ts_senddata_cb);
        ...
        ret                        = TS_SEND_RAW(scb.frag_wait_p.dnode, scb.frag_wait_p.snode, (uint8_t *)&frag_wait, sizeof(frag_wait), scb.frag_wait_p.tx_flags | TRANSMIT_OPTION_ACK, NULL);
...
static uint8_t send_frag_complete_cmd(void)
{
    ...
    timer_set(&scb.reset_timer, RESET_TIME, FUNC(reset_transport_service), 0);
    ret = TS_SEND_RAW(rcb.cmn.p.dnode, rcb.cmn.p.snode, (uint8_t *)&frag_compl, sizeof(frag_compl), rcb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, NULL);
    if (ret == 0) {
    }
...
retry:
    ...
    timer_set(&scb.reset_timer, RESET_TIME, FUNC(reset_transport_service), 0);
    ret1 = TS_SEND_RAW(rcb.cmn.p.dnode, rcb.cmn.p.snode, t2_txBuf, sizeof(*frag_req), rcb.cmn.p.tx_flags | TRANSMIT_OPTION_ACK, NULL);
    if (ret1 == false) {
