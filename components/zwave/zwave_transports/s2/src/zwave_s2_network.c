    ...
    switch (event->event_type) {
        ...
        case S2_NODE_INCLUSION_PUBLIC_KEY_CHALLENGE_EVENT:
            if (callbacks.on_dsk_challenge) {
                assert(event->evt.s2_event.s2_data.challenge_req.length >= sizeof(zwave_dsk_t));
                callbacks.on_dsk_challenge(event->evt.s2_event.s2_data.challenge_req.granted_keys, event->evt.s2_event.s2_data.challenge_req.dsk_length, event->evt.s2_event.s2_data.challenge_req.public_key);
            }
            break;
        case S2_NODE_INCLUSION_KEX_REPORT_EVENT:
            if (callbacks.on_keys_request) {
                callbacks.on_keys_request(event->evt.s2_event.s2_data.kex_report.security_keys, event->evt.s2_event.s2_data.kex_report.csa);
            }
            break;
        ...
        case S2_JOINING_COMPLETE_NEVER_STARTED_EVENT:
            ...
            // completed.
            zwave_s2_transport_set_inclusion_in_progress(false);
            if (callbacks.on_inclusion_complete) {
                callbacks.on_inclusion_complete(event->evt.s2_event.s2_data.inclusion_complete.exchanged_keys, ZWAVE_NETWORK_MANAGEMENT_KEX_FAIL_NONE);
            }
        ...
        case S2_NODE_INCLUSION_FAILED_EVENT:
            ...
            zwave_s2_transport_set_inclusion_in_progress(false);
            if (callbacks.on_inclusion_complete) {
                zwave_kex_fail_type_t kex_fail_type = event->evt.s2_event.s2_data.inclusion_fail.kex_fail_type;
...
{
    zwave_s2_transport_lock();
    // NM may call this without zwave_s2_transport_init (e.g. set-default after
    // exclusion). libS2 is recreated here, so drop any stale inclusion flag.
    zwave_s2_transport_set_inclusion_in_progress(false);
    ...
    s2_inclusion_init(SECURITY_2_SCHEME_1_SUPPORT, KEX_REPORT_CURVE_25519, SECURITY_2_SECURITY_2_CLASS_0 | SECURITY_2_SECURITY_2_CLASS_1 | SECURITY_2_SECURITY_2_CLASS_2 | SECURITY_2_SECURITY_0_NETWORK_KEY);
