...
const char select_all_sql[]                       = "SELECT id, type, parent_id, reported_value, "
...
sl_status_t datastore_attribute_table_init()
{
    ...
    rc = datastore_exec_sql("CREATE TABLE IF NOT EXISTS " DATASTORE_TABLE_ATTRIBUTES "("
...
sl_status_t datastore_attribute_statement_init()
{
    ...
    // Upsert:
    const char *upsert_sql = "INSERT INTO " DATASTORE_TABLE_ATTRIBUTES " (id, type, parent_id, reported_value, desired_value) "
                             " VALUES( ?, ?, ?, ?, ? ) "
    ...
    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "Prepare Upsert statement failed: %s\n", sqlite3_errmsg(db));
        return SL_STATUS_FAIL;
    }

    // Select statement:
    const char *select_sql = "SELECT id, type, parent_id, reported_value, "
                             "desired_value FROM " DATASTORE_TABLE_ATTRIBUTES " WHERE id = ?";

    rc = sqlite3_prepare_v2(db, select_sql, -1, &select_statement, NULL);
    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "Prepare Select statement failed: %s\n", sqlite3_errmsg(db));
        return SL_STATUS_FAIL;
    }
    ...
    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "Prepare All Select statement failed: %s\n", sqlite3_errmsg(db));
        return SL_STATUS_FAIL;
    }
    // Select child index statement:
    // LIMIT ?,1 is to fetch a particular row
    // (child_index, 0 for the first, 1 for the second, etc.)
    const char *select_child_index_sql = "SELECT id, type, reported_value, "
                                         "desired_value FROM " DATASTORE_TABLE_ATTRIBUTES " WHERE parent_id = ? ORDER BY id LIMIT ?,1";

    rc = sqlite3_prepare_v2(db, select_child_index_sql, -1, &select_child_index_statement, NULL);
    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "Prepare Select Child Index statement failed: %s\n", sqlite3_errmsg(db));
        return SL_STATUS_FAIL;
    }

    // Delete statement:
    const char *delete_sql = "DELETE FROM " DATASTORE_TABLE_ATTRIBUTES " WHERE id = ?;";

    rc = sqlite3_prepare_v2(db, delete_sql, -1, &delete_statement, NULL);

    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "Prepare Delete statement failed: %s\n", sqlite3_errmsg(db));
        return SL_STATUS_FAIL;
    }
...
sl_status_t datastore_store_attribute(datastore_attribute_id_t id, uint32_t type, uint32_t parent_id, const uint8_t *reported_value, uint8_t reported_value_size, const uint8_t *desired_value, uint8_t desired_value_size)
{
    ...
    rc = sqlite3_bind_blob(upsert_statement, 4, reported_value, reported_value_size, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        log_binding_failed();
        result = SL_STATUS_FAIL;
    }
    ...
    // Bind the reported_value (blob object)
    rc = sqlite3_bind_blob(upsert_statement, 5, desired_value, desired_value_size, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        log_binding_failed();
        result = SL_STATUS_FAIL;
    }
    ...
    if (result == SL_STATUS_OK) {
        ...
        if (rc != SQLITE_DONE && rc != SQLITE_ROW) {
            ...
            sl_log_error(LOG_TAG, "Step operation failed: %s, Query: %s", sqlite3_errmsg(db), expanded_sql);
...
sl_status_t datastore_fetch_attribute(datastore_attribute_id_t id, uint32_t *type, datastore_attribute_id_t *parent_id, uint8_t *reported_value, uint8_t *reported_value_size, uint8_t *desired_value, uint8_t *desired_value_size)
{
    ...
    if (step == SQLITE_ROW) {
        ...
        *parent_id = (uint32_t)sqlite3_column_int64(select_statement, 2);
        ...
        // Reported value
        const uint8_t *value_buffer = (const uint8_t *)sqlite3_column_blob(select_statement, 3);
...
sl_status_t datastore_fetch_all_attributes(datastore_attribute_t *attribute)
{
    ...
    if (step == SQLITE_ROW) {
        ...
        attribute->parent_id = (uint32_t)sqlite3_column_int64(select_all_statement, 2);

        // Get a pointer from sqlite and copy the data to the user pointer
        // Reported value
        const uint8_t *value_buffer    = (const uint8_t *)sqlite3_column_blob(select_all_statement, 3);
        attribute->reported_value_size = (uint8_t)sqlite3_column_bytes(select_all_statement, 3);
        memcpy(attribute->reported_value, value_buffer, attribute->reported_value_size);

        // Desired value
        value_buffer                  = (const uint8_t *)sqlite3_column_blob(select_all_statement, 4);
        attribute->desired_value_size = (uint8_t)sqlite3_column_bytes(select_all_statement, 4);
        memcpy(attribute->desired_value, value_buffer, attribute->desired_value_size);
...
sl_status_t datastore_fetch_attribute_child(datastore_attribute_id_t parent_id, uint32_t child_index, datastore_attribute_id_t *child_id, uint32_t *type, uint8_t *reported_value, uint8_t *reported_value_size, uint8_t *desired_value, uint8_t *desired_value_size)
{
    ...
    if (step == SQLITE_ROW) {
        ...
        *type     = (uint32_t)sqlite3_column_int64(select_child_index_statement, 1);
        ...
        // Get a pointer from sqlite and copy the data to the user pointer
        const uint8_t *value_buffer = (const uint8_t *)sqlite3_column_blob(select_child_index_statement, 2);
        *reported_value_size        = (uint8_t)sqlite3_column_bytes(select_child_index_statement, 2);
        memcpy(reported_value, value_buffer, *reported_value_size);

        // Same thing for the desired value
        value_buffer        = sqlite3_column_blob(select_child_index_statement, 3);
        *desired_value_size = (uint8_t)sqlite3_column_bytes(select_child_index_statement, 3);
        memcpy(desired_value, value_buffer, *desired_value_size);
        status = SL_STATUS_OK;
    } else {
...
sl_status_t datastore_delete_all_attributes()
{
    ...
    if (rc != SQLITE_OK) {
        sl_log_error(LOG_TAG, "SQL Error: Failed to execute statement: %s\n", sqlite3_errmsg(db));
        return SL_STATUS_FAIL;
    }
