...
void on_multicast_message_send_complete(uint8_t status, const zwapi_tx_report_t *tx_info, void *user)
{
    ...
    options.number_of_responses = zwave_tx_get_number_of_responses(state.parent_session_id);
    ...
    if (SL_STATUS_OK != zwave_tx_get_nodes(node_list, state.group_id)) {
        ...
        multicast_follow_ups_terminate_current_session(TRANSMIT_COMPLETE_FAIL, NULL, state.user);
        return;
    }

    for (zwave_node_id_t n = state.last_singlecast_node_id; n <= ZW_LR_MAX_NODE_ID; n++) {
        ...
        sl_status_t send_status = zwave_tx_send_data(&info, zwave_tx_get_frame_length(state.parent_session_id), zwave_tx_get_frame(state.parent_session_id), &options, &on_multicast_message_send_complete, NULL, NULL);
    ...
    sl_log_debug(LOG_TAG, "Multicast session completed for frame id=%p.", state.parent_session_id);
...
static sl_status_t multicast_follow_ups_send_data(const zwave_controller_connection_info_t *info, uint16_t data_length, const uint8_t *data, const zwave_tx_options_t *tx_options, const on_zwave_tx_send_data_complete_t on_send_data_complete, void *user, zwave_tx_session_id_t parent_session_id)
{
    ...
    if ((info->remote.is_multicast == false) || (tx_options->send_follow_ups == false)) {
    ...
    sl_status_t send_status = zwave_tx_send_data(info, data_length, data, &new_options, &on_multicast_message_send_complete, NULL, NULL);
