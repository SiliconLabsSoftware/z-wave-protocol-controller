...
const mock_t default_entry = {.p_func_name = "default", .expect_arg = {{0}, {0}, {0}, {0}}, .actual_arg = {{0}, {0}, {0}, {0}}, .compare_rule_arg = {COMPARE_STRICT, COMPARE_STRICT, COMPARE_STRICT, COMPARE_STRICT}, .error_code = {0}, .return_code = {0}, .executed = false};
...
bool mock_call_find(char const *const p_function_name, mock_t **pp_mock)
{
    uint32_t i;
    for (i = 0; i < mock_db_idx; i++) {
        if ((false == mock_call_db[i].executed) && strcmp(mock_call_db[i].p_func_name, p_function_name) == 0) {
    ...
    for (i = 0; i < mock_db_idx; i++) {
        if ((true == mock_call_db[i].multiple) && strcmp(mock_call_db[i].p_func_name, p_function_name) == 0) {
            *pp_mock     = &mock_call_db[i];
            g_mock_index = i;
            return true;
        }
...
bool mock_call_used_as_stub(char const *const p_file_name, char const *const p_function_name)
{
    ...
    for (i = 0; i < stub_db_idx; i++) {
        if ((strcmp(stub_call_db[i].p_name, p_function_name) == 0) || (strcmp(stub_call_db[i].p_name, p_file_name) == 0)) {
            return true;
        }
    }
    return false;
...
bool mock_call_used_as_fake(char const *const p_file_name, char const *const p_function_name)
{
    ...
    for (i = 0; i < fake_db_idx; i++) {
        if ((strcmp(fake_call_db[i].p_name, p_function_name) == 0) || (strcmp(fake_call_db[i].p_name, p_file_name) == 0)) {
            return true;
        }
    }
    return false;
