...
struct Args {
    ...
    #[clap(long, forbid_empty_values = true)]
    /// Input device type for eed to be supported. Supported device types: dimmablelight, onofflight, doorlock, onofflightswitch, lightsensor, occupancysensor, onoffpluginunit, dimmablepluginunit, extendedcolorlight, windowcovering, thermostat, flowsensor, allclustersdevice
    eed_device_type: Option<String>,
    #[clap(subcommand)]
...
fn main() {
    ...
    if parsed_args.eed_device_type != None {
        let device_type_string =
            format!("--eed.device_type \"{}\"", &parsed_args.eed_device_type.unwrap());
        set_eed_device_type_arg(&device_type_string);
    }    
    ...
    if !prot_sel_list_string.contains("Z-Wave") {
        app_sel_list_string = app_sel_list_string.clone() + "," + "EED";
    }
    ...
    if unify_zip_exists.is_err() {
        ...
        let example_package = "unify_1.6.0_x86_64.zip".to_string();
        eprintln!(
