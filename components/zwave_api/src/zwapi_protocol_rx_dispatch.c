...
void zwave_api_protocol_rx_dispatch(uint8_t *pData, uint16_t len)
{
    ...
    sl_log_debug(LOG_TAG, "Incoming Z-Wave API frame (hex): %s\n", zwapi_frame_to_string(pData, len));
    ...
    switch (pData[IDX_CMD]) {
        case FUNC_ID_ZW_APPLICATION_CONTROLLER_UPDATE: {
            ...
            /* */
            if ((bStatus == UPDATE_STATE_NODE_INFO_FOREIGN_HOMEID_RECEIVED) || (bStatus == UPDATE_STATE_NODE_INFO_SMARTSTART_HOMEID_RECEIVED_LR)) {
                /* ZW->HOST: 0x49 | 0x85 | Source NodeID (1/2 bytes) | length
             ...
             */
                ...
                node_info_length = pData[index++];
                if (node_info_length > sizeof(zwapi_command_buffer)) {
                    node_info_length = sizeof(zwapi_command_buffer);
                }
                memcpy(zwapi_command_buffer, &pData[index], node_info_length);

                zwave_api_get_callbacks()->application_controller_update(bStatus, source_node_id, zwapi_command_buffer, node_info_length, nwi_home_id);
             ...
             */
                ...
                zwave_api_get_callbacks()->application_controller_update(bStatus, source_node_id, NULL, 0, nwi_home_id);
                ...
                zwave_api_get_callbacks()->application_controller_update(learnNodeInfo.bStatus, learnNodeInfo.bSource, learnNodeInfo.pCmd, learnNodeInfo.bLen, 0);
            }
        } break;
        ...
        case FUNC_ID_APPLICATION_COMMAND_HANDLER: {
            if (zwave_api_get_callbacks()->application_command_handler != NULL) {
                ...
                zwave_api_get_callbacks()->application_command_handler(rx_status, destination_node_id, source_node_id, zwapi_command_buffer, command_length, rssi_value);
            }
        } break;
        ...
        case FUNC_ID_APPLICATION_COMMAND_HANDLER_BRIDGE: {
            if (zwave_api_get_callbacks()->application_command_handler_bridge) {
                ...
                zwave_api_get_callbacks()->application_command_handler_bridge(rx_status, destination_node_id, source_node_id, zwapi_command_buffer, command_length, rssi_value);
            }
        } break;
        ...
        case FUNC_ID_SEND_NOP: {
            ...
            if (pData[IDX_CMD] == FUNC_ID_ZW_SEND_DATA || pData[IDX_CMD] == FUNC_ID_ZW_SEND_PROTOCOL_DATA) {
        ...
        case FUNC_ID_ZW_SET_SLAVE_LEARN_MODE:
            if (zwapi_set_virtual_node_to_learn_mode_callback != NULL) {
                ...
                zwapi_set_virtual_node_to_learn_mode_callback(pData[IDX_DATA + 1], original_node_id, new_node_id);
            }
            break;
        ...
        case FUNC_ID_SERIAL_API_STARTED:
            ...
            if (zwave_api_get_callbacks()->zwapi_started != NULL) {
                uint8_t data_length = len - IDX_DATA - 1;  //-1 to strip off the checksum field
                ...
                zwave_api_get_callbacks()->zwapi_started(zwapi_command_buffer, data_length);
            }
            break;
        ...
        case FUNC_ID_ZW_REQUEST_PROTOCOL_CC_ENCRYPTION:
            if (zwave_api_get_callbacks()->protocol_cc_encryption_request != NULL) {
                ...
                uint8_t current_index                     = IDX_DATA;
                const zwave_node_id_t destination_node_id = zwapi_read_node_id(pData, &current_index);
