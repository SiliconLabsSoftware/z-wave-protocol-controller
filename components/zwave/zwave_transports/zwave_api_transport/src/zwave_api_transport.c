...
static void zwave_api_transport_start_timer(void);
static bool zwave_api_transport_is_busy(void);
...
static void zwave_api_reset_state()
{
    ...
    timer_stop(&state.emergency_timer);
}
...
static sl_status_t zwave_api_transport_intercept_payload(const zwave_controller_connection_info_t *info, uint16_t data_length, const uint8_t *data)
{
    ...
    if ((data[COMMAND_CLASS_INDEX] == ZWAVE_PROTOCOL_COMMAND_CLASS) && (data[COMMAND_INDEX] == ZWAVE_NODE_INFO_REQUEST_COMMAND)) {
...
void zwave_api_send_data_callback(uint8_t status, zwapi_tx_report_t *tx_status)
{
    ...
    if (state.remote_node_id != INVALID_NODE_ID) {
        if (IS_TRANSMISSION_SUCCESSFUL(status)) {
            zwave_controller_on_frame_transmission(true, tx_status, state.remote_node_id);
        } else if (state.fast_track == false) {
            // Inform of transmission failures only if we sent without fast track.
            zwave_controller_on_frame_transmission(false, tx_status, state.remote_node_id);
        }
    }
...
static const zwave_controller_callbacks_t zwave_api_transport_callbacks = {.on_node_information = &zwave_api_transport_on_node_information, .on_node_info_req_failed = &zwave_api_transport_on_node_info_req_failed};
...
static sl_status_t zwave_api_send_data(const zwave_controller_connection_info_t *info, uint16_t data_length, const uint8_t *data, const zwave_tx_options_t *tx_options, const on_zwave_tx_send_data_complete_t on_send_data_complete, void *user, zwave_tx_session_id_t parent_session_id)
{
    ...
    if (data_length > ZWAVE_API_MAXIMUM_FRAME_LENGTH) {
        sl_log_error(LOG_TAG, "Z-Wave API frame too large (size=%d) to be send.", data_length);
        return SL_STATUS_WOULD_OVERFLOW;
    }

    // Are we already sending?
    if (state.transmission_ongoing == true) {
        // TX state machine handles SL_STATUS_BUSY gracefully.
        // It keeps the frame in the queue and retries once our
        // send-data callback fires.
        sl_log_debug(LOG_TAG,
    ...
    if (SL_STATUS_OK == zwave_api_transport_intercept_payload(info, data_length, data)) {
        // Mark that we are busy transmitting and return
        state.remote_node_id = info->remote.node_id;
        zwave_api_transport_start_transmission(on_send_data_complete, user, parent_session_id);
        return SL_STATUS_OK;
    }
    ...
    if (info->remote.is_multicast == false) {
        ...
        if (tx_options->transport.is_test_frame == true) {
            ...
            state.remote_node_id = INVALID_NODE_ID;
            transmit_status      = zwapi_send_test_frame(info->remote.node_id, tx_options->transport.rf_power, &zwave_api_send_test_frame_callback);
        } else if (tx_options->transport.is_protocol_frame == true) {
            transmit_status = zwapi_send_protocol_data(info->remote.node_id, data, (uint8_t)data_length, user, &zwave_api_send_data_callback);
        } else {
            transmit_status = zwapi_send_data(info->remote.node_id, data, (uint8_t)data_length, zwapi_tx_options, &zwave_api_send_data_callback);
        }
    ...
    if (info->remote.is_multicast == true) {
        ...
        if (SL_STATUS_OK != zwave_tx_get_nodes(node_list, info->remote.multicast_group)) {
            sl_log_warning(LOG_TAG,
        ...
        // Try to send.
        if (SL_STATUS_OK == zwapi_send_data_multi(node_list, data, (uint8_t)data_length, zwapi_tx_options, &zwave_api_send_data_multi_callback)) {
            // Mark that we are busy transmitting and return
            zwave_api_transport_start_transmission(on_send_data_complete, user, parent_session_id);
            return SL_STATUS_OK;
        }
        sl_log_warning(LOG_TAG, "Z-Wave API zwapi_send_data_multi refused frame (parent_id = %p)\n", parent_session_id);
        return SL_STATUS_FAIL;
...
static sl_status_t zwave_api_abort_send_data(const zwave_tx_session_id_t session_id)
{
    ...
    if (state.transmission_ongoing != true || state.parent_session_id != session_id) {
...
sl_status_t zwave_api_transport_init()
{
    ...
    transport.is_busy                      = &zwave_api_transport_is_busy;
...
static bool zwave_api_transport_is_busy(void)
{
    return state.transmission_ongoing;
}
...
{
    if (state.transmission_ongoing == true) {
        timer_set(&state.emergency_timer, SEND_DATA_EMERGENCY_TIMEOUT * TIMER_SECOND, zwave_api_transport_emergency_timeout_step, 0);
    }
}
