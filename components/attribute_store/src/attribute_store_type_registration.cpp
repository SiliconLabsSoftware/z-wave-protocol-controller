...
namespace
{
    std::map<attribute_store_type_t, attribute_type_information_t> attribute_information_map;
}
...
///////////////////////////////////////////////////////////////////////////////
sl_status_t attribute_store_register_type(attribute_store_type_t type, const char *name, attribute_store_type_t parent_type, attribute_store_storage_type_t storage_type)
{
    // if such type ID has been already registered - return an error
    if (attribute_information_map.contains(type)) {
        sl_log_error(LOG_TAG, "Error: attribute type [0x%08X:'%s'] is already registered.", type, name);
    ...
    attribute_type_information_t info = {.type_name = std::string(name), .parent_type = parent_type, .storage_type = storage_type};
...
const char *attribute_store_get_type_name(attribute_store_type_t type)
{
    try {
        ...
        uint16_t index             = snprintf(type_name, sizeof(type_name), "Unknown 0x%08X", type);
        type_name[index]           = '\0';
        return type_name;
    }
}
...
{
    return (attribute_information_map.contains(type));
}
 ...
 */
sl_status_t attribute_store_set_type_name(attribute_store_type_t type, const char *name)
{
    if (!attribute_store_is_type_registered(type)) {
...
attribute_store_type_t attribute_store_get_type_by_name(const char *name)
{
    ...
    for (auto it = attribute_information_map.begin(); it != attribute_information_map.end(); ++it) {
...
attribute_store_type_t attribute_store_get_registered_parent_type(attribute_store_type_t type)
{
    if (!attribute_store_is_type_registered(type)) {
        return ATTRIBUTE_STORE_INVALID_ATTRIBUTE_TYPE;
    }
