...
void compare_any_all_args(mock_t *p_mock)
{
  p_mock->compare_rule_arg[0]
    = COMPARE_ANY;  // For this call we just expect any, as we will feed the actual output into next event.
...
void test_kex_joining_node_state_machine(void)
{
  ...
  mock_call_expect(TO_STR(keystore_dynamic_public_key_read),
                   &p_keystore_read_pub_b);
  ...
  mock_call_expect(TO_STR(keystore_dynamic_private_key_read),
                   &p_keystore_read_priv_b);
  p_keystore_read_priv_b->output_arg[0].pointer = m_test_private_key_b;
  mock_call_expect(TO_STR(keystore_dynamic_public_key_read),
                   &p_keystore_read_pub_b);
  p_keystore_read_pub_b->output_arg[0].pointer = m_test_public_key_b;
  mock_call_expect(TO_STR(S2_network_key_update),
                   &p_network_key_joining_update_temp_key_mock);
  ...
  mock_call_expect(TO_STR(keystore_network_key_read),
                   &p_keystore_read_network_key_s2_class_0);
  p_keystore_read_network_key_s2_class_0->expect_arg[0].value
    = KEY_CLASS_S2_UNAUTHENTICATED;
  p_keystore_read_network_key_s2_class_0->output_arg[1].pointer
  ...
  mock_call_expect(TO_STR(S2_network_key_update),
                   &p_network_key_update_round1_mock);
  ...
  s2_inclusion_challenge_response(&s2_including_context,
                                  1,
                                  m_test_public_key_b,
                                  2);
  s2_inclusion_challenge_response(&s2_joining_context,
                                  1,
                                  m_test_public_key_a,
                                  0);
  ...
  s2_including_context.buf
    = p_network_key_get_round1_mock->actual_arg[2].pointer;
  s2_including_context.length
    = p_network_key_get_round1_mock->actual_arg[3].value;
  s2_including_conn.class_id = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_including_context, &s2_including_conn);

  s2_joining_context.buf
    = p_network_key_report_round1_mock->actual_arg[2].pointer;
  s2_joining_context.length
    = p_network_key_report_round1_mock->actual_arg[3].value;
  s2_joining_conn.class_id = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_joining_context, &s2_joining_conn);

  s2_including_context.buf
    = p_network_key_verify_round1_mock->actual_arg[2].pointer;
  s2_including_context.length
    = p_network_key_verify_round1_mock->actual_arg[3].value;
  ...
  s2_including_context.buf
    = p_network_key_get_round2_mock->actual_arg[2].pointer;
  s2_including_context.length
    = p_network_key_get_round2_mock->actual_arg[3].value;
  s2_including_conn.class_id = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_including_context, &s2_including_conn);

  s2_joining_context.buf
    = p_network_key_report_round2_mock->actual_arg[2].pointer;
  s2_joining_context.length
    = p_network_key_report_round2_mock->actual_arg[3].value;
  s2_joining_conn.class_id = UNIT_TEST_TEMP_KEY_SECURE;
  s2_inclusion_post_event(&s2_joining_context, &s2_joining_conn);

  s2_including_context.buf
    = p_network_key_verify_round2_mock->actual_arg[2].pointer;
  s2_including_context.length
    = p_network_key_verify_round2_mock->actual_arg[3].value;
  ...
  TEST_ASSERT_EQUAL_UINT8(
    0x0C,
    ((uint8_t *)p_transfer_end_complete_mock->actual_arg[2].pointer)[1]);
  // 0x01 in third byte denotes the if key exchange is complete.
  TEST_ASSERT_EQUAL_UINT8(
    0x01,
    ((uint8_t *)p_transfer_end_complete_mock->actual_arg[2].pointer)[2]);
