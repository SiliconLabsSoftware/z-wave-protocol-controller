...
#define IS_BROADCAST_NODE_ID(node_id) ((node_id == ZWAVE_BROADCAST_NODE_ID) || (node_id == ZWAVE_LR_BROADCAST_NODE_ID))
...
#define ZW_IS_NODE_IN_MASK(node_id, nodemask) (IS_ZWAVE_NODE_ID_VALID(node_id) ? (IS_ZWAVE_NODE_ID(node_id) ? ((nodemask[(node_id - 1) >> 3] >> ((node_id - 1) & 7)) & 0x1) : ((nodemask[(node_id) >> 3] >> ((node_id) & 7)) & 0x1)) : 0)

#define ZW_ADD_NODE_TO_MASK(node_id, nodemask) (IS_ZWAVE_NODE_ID_VALID(node_id) ? (IS_ZWAVE_NODE_ID(node_id) ? ((nodemask[(node_id - 1) / 8]) |= (1 << ((node_id - 1) % 8))) : ((nodemask[(node_id) / 8]) |= (1 << ((node_id) % 8)))) : 0)

#define ZW_REMOVE_NODE_FROM_MASK(node_id, nodemask) (IS_ZWAVE_NODE_ID_VALID(node_id) ? (IS_ZWAVE_NODE_ID(node_id) ? ((nodemask[(node_id - 1) / 8]) &= (0xFF - (1 << ((node_id - 1) % 8)))) : ((nodemask[(node_id) / 8]) &= (0xFF - (1 << ((node_id) % 8))))) : 0)
...
typedef struct {
        uint8_t basic;     ///< Basic Device Type, is it a Controller, Controller_Static, End_Node or a End_Node_Routing Device Type
