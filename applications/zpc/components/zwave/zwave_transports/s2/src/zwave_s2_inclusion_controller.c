...
const size_t s2_transition_table_controller_length
  = ELEM_COUNT(s2_transition_table_controller);
...
#ifdef ZWAVE_PSA_SECURE_VAULT
  zw_status_t status
    = zw_compute_inclusion_ecdh_shared_secret(mp_context->public_key,
                                              zwave_shared_secret);
...
{
  mp_context->scheme_support = (mp_context->buf[SECURITY_2_KEX_REP_SCHEME_POS]
                                & SECURITY_2_SCHEME_SUPPORT_MASK);
...
static uint8_t validate_reported_curves(void)
{
  mp_context->curve_support
    = (mp_context->buf[SECURITY_2_KEX_REP_CURVE_POS] & KEX_REPORT_CURVE_MASK);
...
static uint8_t validate_reported_keys(void)
{
  ...
  mp_context->key_granted
    = mp_context->key_exchange
      & SECURITY_2_KEY_MASK; /* Why not mask ->key_exchange? */
...
static uint8_t validate_requested_key(void)
{
  ...
  if (
    ...
    && (req_key
        & mp_context
            ->key_granted))  // key_granted is already in LR format because it was converted in s2_inclusion_key_grant()
  {
...
{
  // Test that only one bit is set and that the bit corresponds to a granted key.
  if ((mp_context->scheme_support
       == mp_context->buf[SECURITY_2_KEX_SET_SCHEME_POS])
      && (mp_context->curve_support
          == mp_context->buf[SECURITY_2_KEX_SET_CURVE_POS])
      && (mp_context->key_granted
          == mp_context->buf[SECURITY_2_KEX_SET_KEYS_POS])
      && (mp_context->inclusion_mode
              == ((mp_context->buf[SECURITY_2_KEX_SET_ECHO_POS]
                   & SECURITY_2_CSA_ON)
                  == SECURITY_2_CSA_ON)
            ? INCLUSION_MODE_CSA
            : INCLUSION_MODE_SSA)) {
...
static void s2_kex_rep_recv(void)
{
  ...
  mp_context->csa_support = (mp_context->buf[SECURITY_2_KEX_REP_ECHO_POS]
                             & SECURITY_2_KEX_REP_CSA_MASK)
                              ? 1
                              : 0;
  mp_context->nls_state   = (mp_context->buf[SECURITY_2_KEX_REP_ECHO_POS]
                           & SECURITY_2_KEX_REP_NLS_MASK)
                              ? 1
                              : 0;
  ...
  s2_event->evt.s2_event.s2_data.kex_report.security_keys
    = mp_context->key_granted;
  s2_event->evt.s2_event.s2_data.kex_report.csa
    = (mp_context->buf[SECURITY_2_KEX_REP_ECHO_POS]
       & SECURITY_2_KEX_REP_CSA_MASK)
      != 0;
  s2_event->evt.s2_event.s2_data.kex_report.nls_available
    = (mp_context->buf[SECURITY_2_KEX_REP_ECHO_POS]
       & SECURITY_2_KEX_REP_NLS_MASK);
...
static void s2_send_pub_key_a(void)
{
  ...
  keystore_public_key_read(
    &mp_context->u.inclusion_buf[SECURITY_2_PUB_KEY_KEY_POS]);
...
static void s2_send_echo_kex_report(void)
{
  ...
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_ECHO_POS]
    = (uint8_t)((mp_context->nls_state << 2) | (mp_context->csa_support << 1)
                | SECURITY_2_ECHO_ON);
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_SCHEME_POS]
    = mp_context->scheme_support;
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_CURVE_POS]
    = mp_context->curve_support;
  mp_context->u.inclusion_buf[SECURITY_2_KEX_REP_KEYS_POS]
    = mp_context->kex_report_keys;
...
static void s2_send_net_key_report(void)
{
  ...
  mp_context->key_exchange
    |= mp_context->buf[SECURITY_2_NET_KEY_GET_REQ_KEY_POS];
  ...
  if (mp_context->u.inclusion_buf[SECURITY_2_NET_KEY_REP_GRANT_KEY_POS]
