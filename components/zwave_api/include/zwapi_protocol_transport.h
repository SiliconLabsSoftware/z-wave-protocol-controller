...
#define TRANSMIT_COMPLETE_NO_ACK  0x01  ///< The destination has not acknowledge the frame
...
#define PROTOCOL_METADATA_LENGTH (sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint32_t) + sizeof(uint8_t))  ///< rf_speed | tx_power | tx_options | callback_id
...
typedef enum _E_ROUTING_SCHEME_ {
    ...
    ROUTINGSCHEME_CACHED_ROUTE_NLWR = 4,  ///< Next to LastWorkingRoute(controller)
