...
namespace attribute_store
{
    ...
    template<typename T> struct is_iterable<T, std::void_t<decltype(std::declval<T>().begin()), decltype(std::declval<T>().end()), decltype(std::declval<T>().push_back(*(std::declval<T>().begin())))>> : std::true_type {};
    ...
    class attribute
    {
        public:
            ...
            inline bool get(attribute_store_node_value_state_t value_type, uint8_t *buffer, uint8_t *size) const
            {
                return (SL_STATUS_OK == attribute_store_get_node_attribute_value(_n, value_type, buffer, size));
            }
             ...
             */
            inline attribute first_parent_or_self(attribute_store_type_t parent_type) const
            {
                if (type() == parent_type) {
                    return *this;
                } else {
            ...
            template<typename T> inline T get(attribute_store_node_value_state_t value_type) const
            {
                ...
                if (get(value_type, (uint8_t *)buffer, &size)) {
                    ...
                    if (size == sizeof(T)) {
                        ...
                        throw std::invalid_argument("Unexpected attribute size for node " + name_and_id() + ".Expected: " + std::to_string(sizeof(T)) + ", actual: " + std::to_string(size) + " value state: " + std::to_string(value_type) + ".");
                    }
                } else {
            ...
            inline attribute &set(attribute_store_node_value_state_t value_type, const uint8_t *value, size_t size)
            {
                if (SL_STATUS_OK != attribute_store_set_node_attribute_value(_n, value_type, value, size)) {
            ...
            template<typename T> inline attribute &set(attribute_store_node_value_state_t value_type, const T &value)
            {
                if constexpr (std::is_same<T, char const *>::value) {
                    ...
                    // We are storing strings as C Strings with the null termination character in the attribute store.
                    return set(value_type, reinterpret_cast<const uint8_t *>(value.c_str()), value.size() + 1);
                    ...
                    return set(value_type, reinterpret_cast<const uint8_t *>(v.data()), v.size() * sizeof(v_t));
                } else {
                    static_assert(std::is_trivially_copyable<T>::value);
            ...
            {
                if (SL_STATUS_OK != attribute_store_set_node_attribute_value(_n, value_type, 0, 0)) {
            ...
            /**
             * @brief Clears the reported value then sets the desired value in
             *        two separate attribute store operations, used to trigger
             *        resolver GET rules on a group node.
             */
            template<typename T> attribute &set_desired_and_undefine_reported(T v)
            {
                clear_reported();
                return set_desired<T>(v);
            }
            ...
            attribute first_child(attribute_store_type_t type) const
            {
                ...
                if (!child_node.is_valid()) {
                    throw std::invalid_argument("No child of type " + std::string(attribute_store_get_type_name(type)) + " found under " + name_and_id());
                }
             ...
             * @return attribute
             */
            template<typename T> attribute child_by_type_and_value(attribute_store_type_t type, T value) const
            {
             ...
             * @return attribute
             */
            template<typename T> attribute child_by_type_and_value(attribute_store_type_t type, T value, attribute_store_node_value_state_t value_type) const
            {
                for (auto c: children(type)) {
             ...
             */
            template<typename T> attribute emplace_node(attribute_store_type_t type, T val, attribute_store_node_value_state_t value_state = REPORTED_ATTRIBUTE)
            {
             ...
             */
            inline sl_status_t visit(const std::function<sl_status_t(attribute_store::attribute &, int)> &func, int depth = 0)
            {
        ...
        private:
            ...
            template<typename T> std::string _get_str_value() const
            {
            ...
            template<typename T> std::string _get_str_value_with_state(attribute_store_node_value_state_t state) const
            {
                if (!exists(state)) {
                    return "<>";
                }
                ...
                if constexpr (std::is_same<T, std::vector<uint8_t>>::value) {
                    ...
                    str_value.pop_back();
                    str_value += "]";
                } else if constexpr (std::is_same<T, int64_t>::value || std::is_same<T, uint64_t>::value || std::is_same<T, double>::value) {
