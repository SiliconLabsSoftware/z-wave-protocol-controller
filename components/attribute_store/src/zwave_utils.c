...
bool zwave_get_nls_support(zwave_node_id_t node_id, attribute_store_node_value_state_t value_state)
{
    ...
    attribute_store_node_t node = attribute_store_get_first_child_by_type(node_id_node, ATTRIBUTE_ZWAVE_NLS_SUPPORT);
    ...
    if (SL_STATUS_OK != attribute_store_read_value(node, value_state, &nls_supported, sizeof(nls_supported))) {
        return false;
    }
...
bool zwave_get_nls_state(zwave_node_id_t node_id, attribute_store_node_value_state_t value_state)
{
    ...
    attribute_store_node_t node = attribute_store_get_first_child_by_type(node_id_node, ATTRIBUTE_ZWAVE_NLS_STATE);
    ...
    if (SL_STATUS_OK != attribute_store_read_value(node, value_state, &nls_enabled, sizeof(nls_enabled))) {
        return false;
    }
...
zwave_operating_mode_t zwave_get_operating_mode(zwave_node_id_t node_id)
{
    ...
    attribute_store_node_t listening_protocol_node = attribute_store_get_first_child_by_type(node_id_node, ATTRIBUTE_ZWAVE_PROTOCOL_LISTENING);
    ...
    if (SL_STATUS_OK != attribute_store_read_value(listening_protocol_node, REPORTED_ATTRIBUTE, &listening_protocol, sizeof(listening_protocol))) {
        return OPERATING_MODE_UNKNOWN;
    }
    ...
    attribute_store_node_t optional_protocol_node = attribute_store_get_first_child_by_type(node_id_node, ATTRIBUTE_ZWAVE_OPTIONAL_PROTOCOL);
    ...
    if (SL_STATUS_OK != attribute_store_read_value(optional_protocol_node, REPORTED_ATTRIBUTE, &optional_protocol, sizeof(optional_protocol))) {
        return OPERATING_MODE_UNKNOWN;
    }

    if (optional_protocol & ZWAVE_NODE_INFO_OPTIONAL_PROTOCOL_SENSOR_1000MS_MASK) {
        return OPERATING_MODE_FL;
    }
    if (optional_protocol & ZWAVE_NODE_INFO_OPTIONAL_PROTOCOL_SENSOR_250MS_MASK) {
        return OPERATING_MODE_FL;
    }
...
zwave_protocol_t zwave_get_inclusion_protocol(zwave_node_id_t node_id)
{
    ...
    // Log a warning on failure: the attribute is expected to exist for any included
    // node, so a read failure here typically points at a transient store issue and
    // surfaces downstream as "Unknown keyset" S2 send aborts.
    sl_status_t status = attribute_store_get_desired_else_reported(inclusion_protocol_node, &inclusion_protocol, sizeof(zwave_protocol_t));
    if (status != SL_STATUS_OK) {
        sl_log_warning(LOG_TAG, "Failed to read inclusion protocol for NodeID %d (status=0x%x). Returning PROTOCOL_UNKNOWN.", node_id, status);
        inclusion_protocol = PROTOCOL_UNKNOWN;
    }
...
sl_status_t zwave_store_inclusion_protocol(zwave_node_id_t node_id, zwave_protocol_t inclusion_protocol)
{
    ...
    if (inclusion_protocol_node == ATTRIBUTE_STORE_INVALID_NODE) {
        inclusion_protocol_node = attribute_store_add_node(ATTRIBUTE_ZWAVE_INCLUSION_PROTOCOL, node_id_node);
    }

    return attribute_store_set_reported(inclusion_protocol_node, &inclusion_protocol, sizeof(zwave_protocol_t));
}

// Read the granted keys for a node from the attribute store
sl_status_t zwave_get_node_granted_keys(zwave_node_id_t node_id, zwave_keyset_t *keyset)
{
    attribute_store_node_t node_id_node = attribute_store_network_helper_get_zwave_node_id_node(node_id);
    ...
    return attribute_store_get_reported(granted_keys_node, keyset, sizeof(zwave_keyset_t));
}
...
sl_status_t zwave_set_node_granted_keys(zwave_node_id_t node_id, zwave_keyset_t *keyset)
{
    ...
    attribute_store_node_t node_id_node = attribute_store_network_helper_create_node_id_node(home_id, node_id);

    // Write down the granted keys for that node
    return attribute_store_set_child_reported(node_id_node, ATTRIBUTE_GRANTED_SECURITY_KEYS, keyset, sizeof(zwave_keyset_t));
}
...
bool zwave_node_supports_command_class(zwave_command_class_t command_class, zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    ...
    attribute_store_node_t non_secure_nif_node = attribute_store_get_first_child_by_type(endpoint_node, ATTRIBUTE_ZWAVE_NIF);
    ...
    if ((non_secure_nif_node != ATTRIBUTE_STORE_INVALID_NODE) && (SL_STATUS_OK == attribute_store_get_node_attribute_value(non_secure_nif_node, REPORTED_ATTRIBUTE, nif, &nif_length))) {
        command_class_found = is_command_class_in_supported_list(command_class, nif, nif_length);
    }
    ...
    if (SL_STATUS_OK == attribute_store_get_node_attribute_value(secure_nif_node, REPORTED_ATTRIBUTE, nif, &nif_length)) {
        command_class_found = is_command_class_in_supported_list(command_class, nif, nif_length);
    }
    return command_class_found;
}

uint8_t zwave_node_get_command_class_version(zwave_command_class_t command_class, zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    attribute_store_node_t endpoint_node = zwave_get_endpoint_node(node_id, endpoint_id);

    if (endpoint_node == ATTRIBUTE_STORE_INVALID_NODE) {
        return 0;
    }

    attribute_store_node_t version_node = attribute_store_get_first_child_by_type(endpoint_node, ZWAVE_CC_VERSION_ATTRIBUTE(command_class));
...
{
    zwave_home_id_t home_id = zwave_network_management_get_home_id();
    return attribute_store_network_helper_get_endpoint_node(home_id, node_id, endpoint_id);
}
...
bool zwave_security_validation_is_node_s2_capable(zwave_node_id_t node_id)
{
    ...
    attribute_store_node_t s2_capable_node = attribute_store_get_first_child_by_type(node_id_node, ATTRIBUTE_NODE_IS_S2_CAPABLE);
...
wake_up_interval_t zwave_get_wake_up_interval(zwave_node_id_t node_id)
{
    ...
    attribute_store_node_t endpoint_id_node = attribute_store_get_node_child_by_type(node_id_node, ATTRIBUTE_ENDPOINT_ID, endpoint_id_child_index);

    wake_up_interval_t wake_up_interval = 0;

    while (endpoint_id_node != ATTRIBUTE_STORE_INVALID_NODE && wake_up_interval == 0) {
        // Find if there is a Wake Up Setting:
        // Set up the Wake Up Interval
        attribute_store_node_t wake_up_setting_node  = attribute_store_get_first_child_by_type(endpoint_id_node, ATTRIBUTE_COMMAND_CLASS_WAKE_UP_SETTING);
        attribute_store_node_t wake_up_interval_node = attribute_store_get_first_child_by_type(wake_up_setting_node, ATTRIBUTE_COMMAND_CLASS_WAKE_UP_INTERVAL);

        attribute_store_get_reported(wake_up_interval_node, &wake_up_interval, sizeof(wake_up_interval));

        // Move to the next Endpoint
        endpoint_id_child_index += 1;
        endpoint_id_node = attribute_store_get_node_child_by_type(node_id_node, ATTRIBUTE_ENDPOINT_ID, endpoint_id_child_index);
    }
...
bool is_command_in_array(zwave_command_class_t command_class, zwave_command_t command, const uint8_t *command_list, uint8_t command_list_length)
{
    ...
    while (i < command_list_length) {
        if (command_list[i] >= EXTENDED_COMMAND_CLASS_IDENTIFIER_START) {
            ...
            zwave_command_class_t current_class = (command_list[i] << 8) | (command_list[i + 1]);
...
bool endpoint_exists(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    ...
    // Find the granted keys under the NodeID
    return (ATTRIBUTE_STORE_INVALID_NODE != attribute_store_get_node_child_by_value(node_id_node, ATTRIBUTE_ENDPOINT_ID, REPORTED_ATTRIBUTE, &endpoint_id, sizeof(endpoint_id), 0));
}
