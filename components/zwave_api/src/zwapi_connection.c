...
static zwapi_connection_interface_t zwapi_connection_interface = {};
...
static const char *zwapi_frame_to_string(const uint8_t *buffer, uint16_t buffer_length)
{
    ...
    for (uint16_t i = 0; i < buffer_length; i++) {
        ...
        written = snprintf(message + index, sizeof(message) - index, "%02X ", buffer[i]);
        if (written < 0 || written >= sizeof(message) - index) {
            sl_log_error(LOG_TAG, "Overflow in zwapi_frame_to_string\n");
            assert(false);
            return NULL;
...
static int zwapi_init_serial_connection(const zwapi_connection_params_t *connection_params)
{
    int rc = zwapi_serial_init(connection_params->serial_port);

    if (rc == -1) {
        sl_log_warning(LOG_TAG, "Failed to initialize serial connection");
        return rc;
    }

    zwapi_connection_interface.close             = zwapi_serial_close;
    zwapi_connection_interface.restart           = zwapi_serial_restart;
    zwapi_connection_interface.get_byte          = zwapi_serial_get_byte;
    zwapi_connection_interface.put_byte          = zwapi_serial_put_byte;
    zwapi_connection_interface.get_buffer        = zwapi_serial_get_buffer;
    zwapi_connection_interface.put_buffer        = zwapi_serial_put_buffer;
    zwapi_connection_interface.is_file_available = zwapi_serial_is_file_available;
    zwapi_connection_interface.drain_buffer      = zwapi_serial_drain_buffer;

    return rc;
}

static int zwapi_init_ip_connection(const zwapi_connection_params_t *connection_params)
{

    int rc = zwapi_ip_init(connection_params->ip_address, connection_params->ip_port);

    if (rc == -1) {
        sl_log_warning(LOG_TAG, "Failed to initialize IP connection");
        return rc;
    }

    zwapi_connection_interface.close             = zwapi_ip_close;
    zwapi_connection_interface.restart           = zwapi_ip_restart;
    zwapi_connection_interface.get_byte          = zwapi_ip_get_byte;
    zwapi_connection_interface.put_byte          = zwapi_ip_put_byte;
    zwapi_connection_interface.get_buffer        = zwapi_ip_get_buffer;
    zwapi_connection_interface.put_buffer        = zwapi_ip_put_buffer;
    zwapi_connection_interface.is_file_available = zwapi_ip_is_file_available;
    zwapi_connection_interface.drain_buffer      = zwapi_ip_drain_buffer;

    return rc;
}

static void zwapi_connection_feed(int rc)
{
    if (rc > 0) {
        // Send ACK in case that the target expects one.
        zwapi_connection_interface.put_byte(ACK);
...
int zwapi_connection_init(const zwapi_connection_params_t *connection_params)
{
    ...
    rc = zwapi_init_serial_connection(connection_params);
    if (rc != -1) {
        sl_log_info(LOG_TAG, "Successfully initialized serial connection %s", connection_params->serial_port);
        zwapi_connection_feed(rc);
        return rc;
    }

    sl_log_info(LOG_TAG, "Trying to initialize IP connection");

    rc = zwapi_init_ip_connection(connection_params);
    if (rc != -1) {
        sl_log_info(LOG_TAG, "Successfully initialized IP connection %s:%d", connection_params->ip_address, connection_params->ip_port);
        zwapi_connection_feed(rc);
        return rc;
    }

    sl_log_error(LOG_TAG, "Failed to initialize connection");
    return -1;
}

void zwapi_connection_shutdown()
{
    zwapi_connection_interface.close();
}

int zwapi_connection_restart()
{
    sl_log_critical(LOG_TAG, "Restarting connection");
    rx_buffer_length       = 0;
    zwapi_connection_state = STATE_SOF_HUNT;
    ack_nak_needed         = false;
...
zwapi_connection_status_t zwapi_connection_refresh()
{
    ...
    while (zwapi_connection_interface.is_file_available() && retVal == ZWAPI_CONNECTION_STATUS_IDLE) {
        if (zwapi_connection_interface.get_byte(&c) == 0) {
            ...
            // Check ACK timeout even when no byte is available, so upper layers
            // receive TX_TIMEOUT for retry/reopen logic instead of IDLE.
        ...
        switch (zwapi_connection_state) {
                ...
                if (c == SOF) {
                    ...
                    if (ack_nak_needed) {
                        ...
                        if (c == ACK) {
                            ...
                            sl_log_warning(LOG_TAG, "Frame parsing received unexpected value (0x%02X). Ignoring\n", c);
                            retVal = ZWAPI_CONNECTION_STATUS_TX_TIMEOUT;
                        }
                    }
                }
            ...
            case STATE_PARSE_FRAME:
                ...
                uint8_t sof = SOF;
                zwapi_log_rx_start(&sof, 1);
                zwapi_log_rx_continue(rx_buffer, rx_buffer_length);
                ...
                if (rx_checksum == 0) {
                    ...
                    zwapi_connection_interface.put_byte(NAK);  // Tell them something is wrong...
                    zwapi_connection_interface.drain_buffer();
