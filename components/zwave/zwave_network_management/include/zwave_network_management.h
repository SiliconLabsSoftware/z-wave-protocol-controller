...
/**
 * @brief Get the high-level activity of the network management module.
 *
 * Returns a coarse classification of what the network management module is
 * doing, suitable for reporting to users without exposing internal details.
 *
 * @return zwave_network_management_activity_t
 */
...
/**
 * Maximum time to wait for protocol-layer inclusion callbacks while in
 * NM_WAIT_FOR_PROTOCOL before recovering (e.g. after dropped Serial API frames).
 */
#define NM_WAIT_FOR_PROTOCOL_CALLBACK_TIMEOUT_MS 60000
