...
[`Network/FactoryReset/Report`](#network_factory_reset_report) is published immediately with `"status": "fail"` and `"reason": 6401`.
...
| Field | Type | Description |
|-------|------|-------------|
| `reason` | number | `6401` (`NETWORK_MANAGEMENT_BUSY`) when network management is not idle. |
