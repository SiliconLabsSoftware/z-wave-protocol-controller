...
{
    if (!attribute_store_is_type_validation_enabled()) {
        return true;
    }
    ...
    attribute_store_storage_type_t storage_type = attribute_store_get_storage_type(node_type);
...
bool is_node_addition_valid(attribute_store_type_t node_type, attribute_store_type_t parent_type)
{
    ...
    attribute_store_type_t mandatory_parent_type = attribute_store_get_registered_parent_type(node_type);
    if ((mandatory_parent_type != ATTRIBUTE_STORE_INVALID_ATTRIBUTE_TYPE) && (parent_type != mandatory_parent_type)) {
