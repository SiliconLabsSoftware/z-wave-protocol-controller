...
static uint8_t send_data(ts_node_id_t source, ts_node_id_t dest, const uint8_t *payload, const uint16_t payload_len, uint8_t no_of_expected_responses, const on_lower_layer_send_data_complete_t cb)
{
    ...
    if (transport_service_busy) {
        ...
        // other side.
        options.qos_priority = ZWAVE_TX_QOS_RECOMMENDED_GET_ANSWER_PRIORITY + (ZWAVE_TX_RECOMMENDED_QOS_GAP * 2);
    }

    if (zwave_tx_send_data(&conn, payload_len, payload, &options, on_zwave_tx_send_data_complete, 0, 0) == SL_STATUS_OK) {
        return 0;
    }
    return 1;
}
...
static void on_transport_service_send_data_complete(uint8_t status, void *user)
{
    ...
    if (status == 0) {
        intercepted_on_zwave_tx_send_data_complete(TRANSMIT_COMPLETE_OK, 0, zwave_tx_parent_session_id);
    } else {
        intercepted_on_zwave_tx_send_data_complete(TRANSMIT_COMPLETE_FAIL, 0, zwave_tx_parent_session_id);
    }
}

sl_status_t
  zwave_transport_service_send_data(const zwave_controller_connection_info_t *conn_info, uint16_t data_length, const uint8_t *cmd_data, const zwave_tx_options_t *tx_options, const on_zwave_tx_send_data_complete_t on_zwave_tx_send_data_complete, void *user, zwave_tx_session_id_t parent_session_id)
{
    (void)user;
    (void)tx_options;
    ...
    uint16_t maximum_payload = zwave_tx_scheme_get_max_payload(conn_info->remote.node_id);
    ...
    if (!zwave_controller_storage_is_node_s2_capable(conn_info->remote.node_id) && (zwave_controller_storage_get_cc_version(COMMAND_CLASS_TRANSPORT_SERVICE_V2, conn_info->remote.node_id, 0) < TRANSPORT_SERVICE_VERSION_V2)) {
        sl_log_info(LOG_TAG,
    ...
    if (transport_service_busy) {
        sl_log_warning(LOG_TAG, "Transport service is busy. Cannot service frame (parent_id=%p)", parent_session_id);
    ...
    ret = transport_service_send_data(conn_info->local.node_id, conn_info->remote.node_id, cmd_data, data_length, maximum_payload, on_transport_service_send_data_complete);

    if (ret == TRANSPORT_SERVICE_SEND_SUCCESS) {
        transport_service_busy = true;
        return SL_STATUS_OK;
    }
    if (ret == TRANSPORT_SERVICE_WILL_OVERFLOW) {
...
sl_status_t zwave_transport_service_on_frame_received(const zwave_controller_connection_info_t *connection_info, const zwave_rx_receive_options_t *rx_options, const uint8_t *frame_data, uint16_t frame_length)
{
    ...
    if ((cmd_type != COMMAND_FIRST_SEGMENT_V2) && (cmd_type != COMMAND_SEGMENT_COMPLETE_V2) && (cmd_type != COMMAND_SEGMENT_REQUEST_V2) && (cmd_type != COMMAND_SEGMENT_WAIT_V2) && (cmd_type != COMMAND_SUBSEQUENT_SEGMENT_V2)) {
        sl_log_info(LOG_TAG,
    ...
    if (transport_service_on_frame_received(connection_info->remote.node_id, connection_info->local.node_id, rx_type, frame_data, frame_length) == true) {
        return SL_STATUS_OK;
    }
    return SL_STATUS_FAIL;
}
