...
static void
  convert_to_octet_string(uint16_t text_to_encrypt_len, uint8_t *Q, uint8_t q)
{
  //int i;
  uint8_t i;
...
static int format_b0(const uint8_t t,
...
static int format_aad(uint8_t blocks[2][BLOCK_SIZE],
                      const uint8_t *aad,
                      ...
                      const uint8_t *key)
{
  ...
  memcpy(
    ...
    MIN(
      aad_len,
      aad_bytes_in_first_block)); /* Fill the first block's 14 octets with first 14 octets of aad*/
  /* Copy first 14 octets of AAD to blocks[1] */
  if (
    aad_len
    < aad_bytes_in_first_block) { /* need to pad with zeroes if aad_len is less than 14*/
    memset(&blocks[1][aad_start] + aad_len,
           0,
           aad_bytes_in_first_block - aad_len);
    bit_xor(blocks[1], blocks[0], BLOCK_SIZE);
    ciph_block(blocks[0], key);
  } else if (
    aad_len
    > aad_bytes_in_first_block) { /* If aad_len is more than 14 we need more blocks */
    bit_xor(blocks[1], blocks[0], BLOCK_SIZE);
    ciph_block(blocks[0], key);
    new_aad_len = aad_len - aad_bytes_in_first_block;
    no_of_blocks_for_aad
      = new_aad_len / BLOCK_SIZE; /* Find the number of more blocks needed */
    if (new_aad_len % BLOCK_SIZE)
      no_of_blocks_for_aad++;

    offset
      = aad_bytes_in_first_block; /* Start copying the octets from aad_bytes_in_first_block'th offset*/
    for (i = 0; i < no_of_blocks_for_aad; i++) {
      ...
      if ((i == (no_of_blocks_for_aad - 1)) && (new_aad_len % BLOCK_SIZE)) {
        ...
        /* pad with zeroes */
        memset(&blocks[1][0] + (new_aad_len % BLOCK_SIZE),
               0,
               BLOCK_SIZE - (new_aad_len % BLOCK_SIZE));
        bit_xor(blocks[1], blocks[0], BLOCK_SIZE);
        ciph_block(blocks[0], key);
        break;
      }
...
static void format_payload_block(uint8_t blocks[2][BLOCK_SIZE],
                                 const uint8_t *P,
                                 const uint16_t text_to_encrypt_len,
                                 const uint8_t *key)
{
  ...
  for (i = 0; i < no_blocks_payload; i++) {
    if (
      (i == (no_blocks_payload - 1))
      && (text_to_encrypt_len
          % BLOCK_SIZE)) { /* If its a last block and not aligned to BLOCK_SIZE */
      memcpy(&blocks[1][0], &P[offset], (text_to_encrypt_len % BLOCK_SIZE));
      /* pad with zeroes */
      memset(&blocks[1][0] + (text_to_encrypt_len % BLOCK_SIZE),
             0,
             BLOCK_SIZE - (text_to_encrypt_len % BLOCK_SIZE));
      bit_xor(blocks[1], blocks[0], BLOCK_SIZE);
      ciph_block(blocks[0], key);
      break;
    }
...
static void encrypt_or_decrypt(uint8_t *data_array,
                               uint16_t data_len,
...

{
  ...
  for (i = 0; i < num_ctr_blks; i++) {
    ...
    if ((i == 0) && (mode == ENCRYPT)) {
      ...
      memcpy(block,
             ctr,
             BLOCK_SIZE); /* We need this first block to decrypt MAC */
    } else {
      if (length >= BLOCK_SIZE) {
        bit_xor(ctr,
                &data_array[(i - 1) * BLOCK_SIZE],
                BLOCK_SIZE); /* step 7 in NIST pdf*/
      } else {
...
uint32_t CCM_encrypt_and_auth(const uint8_t *key,
                              const uint8_t *nonce,
                              const uint8_t *aad,
                              const uint32_t aad_len,
                              ...
                              const uint16_t text_to_encrypt_len)
{
  ...
  encrypt_or_decrypt(plain_ciphertext,
                     text_to_encrypt_len,
                     counter_blocks,
                     blocks[0],
                     nonce,
...
uint16_t CCM_decrypt_and_auth(const uint8_t *key,
                              const uint8_t *nonce,
                              const uint8_t *aad,
                              const uint32_t aad_len,
                              ...
                              const uint32_t ciphertext_len)
{
  ...
  encrypt_or_decrypt(cipher_plaintext,
                     ciphertext_len - t,
                     num_counter_blocks,
                     blocks[0],
                     nonce,
