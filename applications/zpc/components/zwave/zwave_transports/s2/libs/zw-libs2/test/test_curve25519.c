...
static void bigint_mul(unsigned char *r,
...
void test_alice_calculation_of_public_key(void)
{
  ...
  UNITY_TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_alice_public_key,
                                      alice_public_key,
                                      KEY_SIZE,
                                      __LINE__,
                                      "");
...
//void test_bob_calculation_of_public_key(void)
{
  ...
  UNITY_TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_bob_public_key,
                                      bob_public_key,
                                      KEY_SIZE,
                                      __LINE__,
                                      "");
...
//void test_alice_bob_shared_secret(void)
{
  ...
  UNITY_TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_shared_secret,
                                      k,
                                      KEY_SIZE,
                                      __LINE__,
                                      "");
...
//void test_10_shared_secrets(void)
{
  ...
  for (count = 0; count < 10; count++) {
    ...
    crypto_scalarmult_curve25519(person1_shared_key_calc,
                                 person1_secret_key,
                                 person2_public_key);

    crypto_scalarmult_curve25519(person2_shared_key_calc,
                                 person2_secret_key,
                                 person1_public_key);

    UNITY_TEST_ASSERT_EQUAL_UINT8_ARRAY(person1_shared_key_calc,
