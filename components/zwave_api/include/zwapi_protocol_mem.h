...
typedef struct _NVR_FLASH_STRUCT_ {
        ...
        uint8_t idVendorUsb[NVR_USBID_SIZE];  /*idVendor if 0xff -> use sigma Vendor ID (Assigned by USB Org)*/
        uint8_t idProductUsb[NVR_USBID_SIZE]; /*idProduct  if 0xff -> use sigma Product ID (Assigned by Manufacturer)*/
 ...
 * aka ZW_NVRGetValue
 */
sl_status_t zwapi_nvr_get_value(uint8_t bOffset, uint8_t bLength, uint8_t *pNVRValue);
