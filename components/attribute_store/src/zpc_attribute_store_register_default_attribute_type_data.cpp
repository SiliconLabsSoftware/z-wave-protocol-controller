...
#include "zpc_attribute_store_all_ids.h"
    ...
    for (const auto &a: attributes) {
        if (!attribute_store_is_type_registered(a.first)) {
            status |= attribute_store_register_type(a.first, a.second, ATTRIBUTE_STORE_INVALID_ATTRIBUTE_TYPE, UNKNOWN_STORAGE_TYPE);
        }
    }
