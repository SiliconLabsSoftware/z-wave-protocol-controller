...
/**
 * @defgroup datastore_fixt ZPC Datastore Fixture
 * @ingroup zpc_datastore
 * @brief Persistent Datastore fixture
 *
 * Fixture to initialize the datastore from the ZPC main process,
 * in order to enable the datastore in a ZPC application,
 * add \ref datastore_fixt_setup to the fixt_setup array,
 * when calling \ref main
 *
 * @{
 */
 ...
 */
#define ZPC_VERSION_MAX_LEN 100
 ...
 *
 * If the versioning is good then writes the current ZPC version in generic
 ...
 * and writes the current ZPC version in generic table
