...
{
    sl_status_t ret = datastore_open(database_path);
    if (ret != SL_STATUS_OK) {
        return ret;
    }
...
sl_status_t datastore_fixt_setup_and_handle_version(const char *datastore_file, int64_t db_version)
{
    ...
    uint8_t fetched_zpc_version[ZPC_VERSION_MAX_LEN] = {0};
    unsigned int fetched_zpc_version_size            = ZPC_VERSION_MAX_LEN;
    if (datastore_contains_int("version") == true) {
        datastore_fetch_int("version", &version_fetched);
        sl_log_info(LOG_TAG, "Datastore version reported from the datastore file: %" PRId64 "\n", version_fetched);

        datastore_fetch_arr("zpc_version", fetched_zpc_version, &fetched_zpc_version_size);
        sl_log_info(LOG_TAG, "Last ZPC version written in the datastore file: %s", fetched_zpc_version);
        ...
        sl_log_info(LOG_TAG, "Writing Datastore version: %" PRId64 " for first time", db_version);
        datastore_store_int("version", db_version);
    }
    sl_log_info(LOG_TAG, "Writing last ZPC version: %s to the datastore file\n", ZPC_VERSION);

    datastore_store_arr("zpc_version", (const uint8_t *)ZPC_VERSION, sizeof(ZPC_VERSION));

    return datastore_attribute_init();
}

////////////////////////////////////////////////////////////////////////////////
// Fixtures for ZPC main integration
////////////////////////////////////////////////////////////////////////////////
