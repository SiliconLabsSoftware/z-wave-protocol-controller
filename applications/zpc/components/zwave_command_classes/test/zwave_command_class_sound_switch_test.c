...
void set_volume_and_tone(uint8_t volume, uint8_t tone)
{
  ...
  attribute_store_node_t volume_node = attribute_store_add_node(
    ATTRIBUTE_COMMAND_CLASS_SOUND_SWITCH_CONFIGURED_DEFAULT_VOLUME,
    endpoint_id_node);
  attribute_store_node_t tone_node = attribute_store_add_node(
  ...
  const uint8_t expected_frame[] = {COMMAND_CLASS_SOUND_SWITCH,
...
void set_play(uint8_t tone_id, uint8_t volume)
{
  ...
  attribute_store_node_t play_node = attribute_store_get_node_child_by_type(
    endpoint_id_node,
    ATTRIBUTE_COMMAND_CLASS_SOUND_SWITCH_TONE_PLAY,
    0);
  attribute_store_node_t volume_node;
  if (version == 2) {
    volume_node = attribute_store_get_node_child_by_type(
      endpoint_id_node,
      ATTRIBUTE_COMMAND_CLASS_SOUND_SWITCH_CONFIGURED_DEFAULT_VOLUME,
      0);
  }
  ...
  if (tone_id > 0 && tone_id < 255) {
    ...
    attribute_store_node_t ontime_node
      = attribute_store_add_node(DOTDOT_ATTRIBUTE_ID_ON_OFF_ON_TIME,
                                 endpoint_id_node);
    sl_status_t result = attribute_store_set_desired(ontime_node,
                                                     &tone_id_16,
                                                     sizeof(tone_id_16));
    ...
    result = attribute_store_get_desired(play_node,
                                         &expected_tone,
                                         sizeof(expected_tone));
    TEST_ASSERT_EQUAL(SL_STATUS_OK, result);

    // Result should be undefined as we only use this as a buffer
    result = attribute_store_get_desired(ontime_node,
                                         &expected_tone,
                                         sizeof(expected_tone));
...
{
  const uint8_t frame[]
    = {COMMAND_CLASS_SOUND_SWITCH, SOUND_SWITCH_TONE_PLAY_REPORT, tone_id};
...
void test_sound_switch_tone_number_report_happy_case()
{
  ...
  for (uint8_t i = 1; i <= EXPECTED_TONE_NUMBER; i++) {
    attribute_store_node_t current_tone_id_node
      = attribute_store_get_node_child(tone_node, i - 1);
    uint8_t expected_value;
    sl_status_t status = attribute_store_get_desired(current_tone_id_node,
                                                     &expected_value,
  ...
  for (uint8_t i = 1; i < EXPECTED_TONE_NUMBER; i++) {
    ...
    // Name test
    char name[3];
    attribute_store_node_t name_node = attribute_store_get_node_child_by_type(
      current_tone_id_node,
...
void test_no_update_if_no_sound_switch()
{
  ...
  attribute_store_node_t ontime_node
    = attribute_store_add_node(DOTDOT_ATTRIBUTE_ID_ON_OFF_ON_TIME,
                               endpoint_id_node);
  sl_status_t result
    = attribute_store_set_desired(ontime_node, &tone_id_16, sizeof(tone_id_16));
  TEST_ASSERT_EQUAL(SL_STATUS_OK, result);
