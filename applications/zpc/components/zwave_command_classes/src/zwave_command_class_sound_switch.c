  ...
  uint16_t frame_length)
{
  ...
  attribute_store_set_reported(tone_number_node,
                               &tone_number,
                               sizeof(tone_number));

  for (uint8_t i = 1; i <= tone_number; i++) {
    attribute_store_node_t node_id = attribute_store_add_node(
  ...
  attribute_store_node_t node, uint8_t *frame, uint16_t *frame_length)
{
  sl_log_debug(LOG_TAG,
               "zwave_command_class_sound_switch_tone_info_get : %d",
               node);

  sound_switch_tone_id_t tone_id = 0;
  sl_status_t status
    = attribute_store_get_desired(node, &tone_id, sizeof(tone_id));
  ...
  if (status != SL_STATUS_OK) {
    ...
    attribute_store_node_t tone_id_node
      = attribute_store_get_first_child_by_type(
        parent_node,
        ATTRIBUTE_COMMAND_CLASS_SOUND_SWITCH_CONFIGURED_DEFAULT_TONE_IDENTIFIER);

    status
      = attribute_store_get_reported(tone_id_node, &tone_id, sizeof(tone_id));
  ...
  uint16_t frame_length)
{
  ...
  const uint8_t SIZE_MESSAGE_WITHOUT_NAME
    = SIZE_FRAME_INFO + SIZE_TONE_ID + SIZE_TONE_DURATION + SIZE_TONE_NAME_SIZE;
  ...
  const uint8_t ADDRESS_NAME_SIZE
    = SIZE_FRAME_INFO + SIZE_TONE_ID + SIZE_TONE_DURATION;      // 5
  ...
  // 01                             05
  // Result 0105
  sound_switch_tone_duration_t tone_duration
    = (frame_data[ADDRESS_DURATION_1] << 8) | frame_data[ADDRESS_DURATION_2];
  // Size of name array
  ...
  attribute_store_set_reported(id_node,
                               &tone_identifier,
                               sizeof(tone_identifier));
...
{
  attribute_store_node_t volume_node = attribute_store_get_first_child_by_type(
    state_node,
    ATTRIBUTE_COMMAND_CLASS_SOUND_SWITCH_CONFIGURED_DEFAULT_VOLUME);

  sl_status_t status
    = attribute_store_get_desired_else_reported(volume_node,
                                                &configuration->volume,
                                                sizeof(configuration->volume));
  if (status != SL_STATUS_OK) {
    configuration->volume = 0;
    sl_log_warning(LOG_TAG,
                   "Can't get CONFIGURED_DEFAULT_VOLUME from attribute store. "
                   "Value set to 0.");
  }
  ...
  status
    = attribute_store_get_desired_else_reported(tone_node,
                                                &configuration->tone,
  ...
  if (status != SL_STATUS_OK) {
    ...
    sl_log_warning(LOG_TAG,
                   "Can't get CONFIGURED_DEFAULT_TONE_IDENTIFIER from "
                   "attribute store. Value set to 0.");
  }
}
  ...
  attribute_store_node_t updated_node, attribute_store_change_t change)
{
  ...
  uint16_t on_time = 0;
  sl_status_t status
    = attribute_store_get_desired(updated_node, &on_time, sizeof(on_time));
  ...
  attribute_store_set_desired(play_node,
                              &tone_id,
                              sizeof(sound_switch_tone_id_t));
  ...
  attribute_store_node_t node, uint8_t *frame, uint16_t *frame_length)
{
  ...
  attribute_store_get_desired_else_reported(version_node,
                                            &current_supported_version,
                                            sizeof(current_supported_version));
...
sl_status_t zwave_command_class_sound_switch_init()
{
  ...
  handler.comments
    = "You can control the specific tone play with the ON_OFF_ON_TIME ZCL "
      "attribute. See code or python CTT script for details.";
