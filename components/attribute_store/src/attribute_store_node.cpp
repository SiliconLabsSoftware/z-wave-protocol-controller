...
///////////////////////////////////////////////////////////////////////////////
attribute_store_node::attribute_store_node(attribute_store_node *_parent_node, attribute_store_type_t _type, attribute_store_node_t _id)
{
...
attribute_store_node *attribute_store_node::add_child(attribute_store_type_t _type, attribute_store_node_t _id)
{
...
void attribute_store_node::log(uint8_t indent)
{
    ...
    for (uint8_t i = 0; i < indent; i++) {
        log_index += snprintf(log_message + log_index, sizeof(log_message) - log_index, " ");
    }

    log_index += snprintf(log_message + log_index, sizeof(log_message) - log_index, "ID: %d - %s - ", id, attribute_store_get_type_name(type));

    // Desired value
    char desired_value_data[MAXIMUM_MESSAGE_SIZE] = {0};
    uint16_t index                                = 0;
    for (uint8_t i = 0; i < this->desired_value.size(); i++) {
        index += snprintf(desired_value_data + index, sizeof(desired_value_data) - index, "%02X ", this->desired_value.at(i));
    }
    if (index > 1) {
    ...
    for (uint8_t i = 0; i < this->reported_value.size(); i++) {
        index += snprintf(reported_value_data + index, sizeof(reported_value_data) - index, "%02X ", this->reported_value.at(i));
    }
    if (index > 1) {
    ...
    log_index += snprintf(log_message + log_index, sizeof(log_message) - log_index, "Reported (hex): [%s] ", reported_value_data);

    if (!this->desired_value.empty()) {
        snprintf(log_message + log_index, sizeof(log_message) - log_index, "(Desired (hex): [%s])", desired_value_data);
    }
...
{
    this->child_nodes.erase(std::remove(this->child_nodes.begin(), this->child_nodes.end(), child_node), this->child_nodes.end());
}