...
class zwave_frame_parser
{
    public:
         ...
         */
        bool is_frame_size_valid(uint16_t min_size, uint16_t max_size = 0) const;
       ...
       * @return The byte read from the frame
       */
        zwave_parser_bitmask_result read_byte_with_bitmask(const std::vector<bitmask_data> &bitmask_data);
         ...
         */
        zwave_parser_bitmask_result read_byte_with_bitmask(const bitmask_data &bitmask_data);
         ...
         * @return The bitmask read from the frame
         */
        zwave_report_bitmask_t read_bitmask(attribute_store_node_t node, uint8_t bitmask_length = 0);
        ...
        /**
         * @brief Get the current frame length
         */
        uint8_t get_frame_length() const;

        /**
         * @brief Check if checksum associated to the frame is valid
         *
         * Read the next two bytes of the frame as the checksum and compute the
        ...
        /**
         * @brief Check if the expected checksum apply to the frame
         *
         * @param expected_checksum The expected checksum. Default to 0.
         * @param checksum_in_frame If true, the checksum is in the frame. The checksum calculation will not take the last two bytes. Otherwise take the whole frame
         *
         * @return true if the checksum is valid
         * @return false if the checksum is invalid
         */
        bool is_checksum_valid(zwave_checksum_t expected_checksum, bool checksum_in_frame = false) const;
        ...
        {
            static_assert(std::is_integral<T>::value || std::is_same<T, std::string>::value || std::is_same<T, std::vector<uint8_t>>::value, "Unsupported type");
         ...
         * @return The value read from the frame
         */
        template<typename T> T read_sequential(uint8_t bytes_to_read, attribute_store_node_t node)
        {
    ...
    private:
         ...
         */
        template<typename T> void helper_store_value(attribute_store_node_t node, T value)
        {
            attribute_store::attribute current_attribute(node);

            if (current_attribute.is_valid()) {
                current_attribute.set_reported<T>(value);
                sl_log_debug("zwave_frame_parser", "Attribute updated : %s", current_attribute.value_to_string().c_str());
            } else {
                sl_log_warning("zwave_frame_parser", "Node does not exist in the attribute store. Not updating value");
            }
        }
         ...
         */
        static uint8_t get_trailing_zeroes(const std::bitset<8> &bitset);
