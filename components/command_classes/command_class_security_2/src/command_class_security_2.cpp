...
namespace zwave_command_class
{
    ...
    command_class_security_2::command_class_security_2()
    {
        ...
        attribute_resolver_set_resolution_give_up_listener(static_cast<attribute_store_type_t>(security_2_commands_supported_get_group_attributes_t::SECURITY_2_COMMANDS_SUPPORTED_GET_GROUP), &command_class_security_2::on_s2_supported_get_resolution_give_up);
    ...
    {
        attribute_store::attribute endpoint_node(payload_struct.endpoint_node);
        auto group_node = endpoint_node.emplace_node(static_cast<attribute_store_type_t>(security_2_commands_supported_get_group_attributes_t::SECURITY_2_COMMANDS_SUPPORTED_GET_GROUP));
        command_class_security_2_core::start_group_resolution(group_node);
        return SL_STATUS_OK;
    }

    void command_class_security_2::on_s2_supported_get_resolution_give_up(attribute_store_node_t node)
    {
        zwave_node_id_t node_id         = 0;
        zwave_endpoint_id_t endpoint_id = 0;
        attribute_store_network_helper_get_zwave_ids_from_node(node, &node_id, &endpoint_id);
        fire_s2_supported_get_tx_failed(node_id, endpoint_id, 0);
    }

    void command_class_security_2::fire_s2_supported_get_tx_failed(zwave_node_id_t node_id, uint8_t endpoint_id, uint8_t status)
    {
        sl_log_warning(LOG_TAG.data(), "S2 Commands Supported Get resolution given up for NodeID %d endpoint %d (status=%u)", node_id, endpoint_id, status);
