 ...
 * incoming messages from the Z-Wave module using a C++ threading implementation.
 *
...
/**
 * @brief Waits for the Z-Wave API to have restarted/be ready after a soft reset.
 *
 * Polls for FUNC_ID_SERIAL_API_STARTED with a per-attempt timeout. On timeout,
 * reconnects the serial connection and retries the soft reset. Returns
 * SL_STATUS_FAIL if the module does not respond after all attempts.
 *
 * @return SL_STATUS_OK if the Z-Wave API started callback was received.
 * @return SL_STATUS_FAIL if the module did not respond within the retry limit.
 */
