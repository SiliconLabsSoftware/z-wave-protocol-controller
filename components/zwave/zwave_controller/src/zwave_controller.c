...
{
    node_type_t my_node_type = {.basic = zpc_get_config()->zpc_basic_device_type, .generic = zpc_get_config()->zpc_generic_device_type, .specific = zpc_get_config()->zpc_specific_device_type};
    // Tell the Z-Wave module what our NIF should be:
    zwapi_set_application_node_information(APPLICATION_NODEINFO_LISTENING, my_node_type, command_classes, command_classes_length);
}
