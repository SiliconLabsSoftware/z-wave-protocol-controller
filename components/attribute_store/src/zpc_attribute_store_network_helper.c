...
attribute_store_node_t attribute_store_network_helper_create_home_id_node(zwave_home_id_t home_id)
{
    ...
    attribute_store_node_t home_id_node = attribute_store_get_node_child_by_value(root_node, ATTRIBUTE_HOME_ID, REPORTED_ATTRIBUTE, (uint8_t *)&home_id, sizeof(home_id), 0);
    ...
    attribute_store_set_node_attribute_value(home_id_node, REPORTED_ATTRIBUTE, (uint8_t *)&home_id, sizeof(home_id));
    return home_id_node;
}
...
attribute_store_node_t attribute_store_network_helper_create_node_id_node(zwave_home_id_t home_id, zwave_node_id_t node_id)
{
    ...
    home_id_node = attribute_store_network_helper_create_home_id_node(home_id);
    if (ATTRIBUTE_STORE_INVALID_NODE == home_id_node) {
        return ATTRIBUTE_STORE_INVALID_NODE;
    }

    attribute_store_node_t node_id_node = attribute_store_get_node_child_by_value(home_id_node, ATTRIBUTE_NODE_ID, REPORTED_ATTRIBUTE, (uint8_t *)&node_id, sizeof(node_id), 0);
    ...
    attribute_store_set_node_attribute_value(node_id_node, REPORTED_ATTRIBUTE, (uint8_t *)&node_id, sizeof(node_id));
    return node_id_node;
}
...
attribute_store_node_t attribute_store_network_helper_create_endpoint_node(zwave_home_id_t home_id, zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    ...
    node_id_identifier = attribute_store_network_helper_create_node_id_node(home_id, node_id);
    if (node_id_identifier == ATTRIBUTE_STORE_INVALID_NODE) {
        return ATTRIBUTE_STORE_INVALID_NODE;
    }

    // Look for an endpoint under the node_id node:
    attribute_store_node_t endpoint_node = attribute_store_get_node_child_by_value(node_id_identifier, ATTRIBUTE_ENDPOINT_ID, REPORTED_ATTRIBUTE, &endpoint_id, sizeof(endpoint_id), 0);
    ...
    endpoint_node = attribute_store_add_node(ATTRIBUTE_ENDPOINT_ID, node_id_identifier);
    attribute_store_set_node_attribute_value(endpoint_node, REPORTED_ATTRIBUTE, &endpoint_id, sizeof(endpoint_id));
    return endpoint_node;
}
...
attribute_store_node_t attribute_store_network_helper_ensure_zwave_node_placeholder(zwave_node_id_t node_id)
{
    ...
    attribute_store_set_child_reported(node_id_node, ATTRIBUTE_ENDPOINT_ID, &endpoint_id, sizeof(endpoint_id));
...
attribute_store_node_t attribute_store_network_helper_get_home_id_node(zwave_home_id_t home_id)
{
    ...
    return attribute_store_get_node_child_by_value(root_node, ATTRIBUTE_HOME_ID, REPORTED_ATTRIBUTE, (uint8_t *)&home_id, sizeof(home_id), 0);
}

attribute_store_node_t attribute_store_network_helper_get_node_id_node(zwave_home_id_t home_id, zwave_node_id_t node_id)
{
    attribute_store_node_t home_id_node;
    home_id_node = attribute_store_network_helper_get_home_id_node(home_id);
    if (ATTRIBUTE_STORE_INVALID_NODE == home_id_node) {
        return ATTRIBUTE_STORE_INVALID_NODE;
    }

    return attribute_store_get_node_child_by_value(home_id_node, ATTRIBUTE_NODE_ID, REPORTED_ATTRIBUTE, (uint8_t *)&node_id, sizeof(node_id), 0);
}

attribute_store_node_t attribute_store_network_helper_get_zwave_node_id_node(zwave_node_id_t zwave_node_id)
{
    zwave_home_id_t home_id = zwave_network_management_get_home_id();
    return attribute_store_network_helper_get_node_id_node(home_id, zwave_node_id);
}
...
{
    attribute_store_node_t node_id_identifier = attribute_store_network_helper_get_node_id_node(home_id, node_id);
    if (node_id_identifier == ATTRIBUTE_STORE_INVALID_NODE) {
        return ATTRIBUTE_STORE_INVALID_NODE;
    }

    // Look for an endpoint under the node_id node:
    return attribute_store_get_node_child_by_value(node_id_identifier, ATTRIBUTE_ENDPOINT_ID, REPORTED_ATTRIBUTE, &endpoint_id, sizeof(endpoint_id), 0);
}

attribute_store_node_t attribute_store_get_endpoint_0_node(attribute_store_node_t node_id_node)
{
    const zwave_endpoint_id_t endpoint_id = 0;
    return attribute_store_get_node_child_by_value(node_id_node, ATTRIBUTE_ENDPOINT_ID, REPORTED_ATTRIBUTE, &endpoint_id, sizeof(endpoint_id), 0);
}
...
sl_status_t attribute_store_network_helper_get_home_node_endpoint_from_node(attribute_store_node_t node, zwave_home_id_t *home_id, zwave_node_id_t *node_id, zwave_endpoint_id_t *endpoint_id)
{
    ...
    if (SL_STATUS_OK != attribute_store_get_reported(node, node_id, sizeof(zwave_node_id_t))) {
        // Abort if no NodeID data was retrieved.
        return SL_STATUS_FAIL;
    }

    // Continue climbing up until we find the HomeID type of node
    node = attribute_store_get_first_parent_with_type(node, ATTRIBUTE_HOME_ID);
    if (node == ATTRIBUTE_STORE_INVALID_NODE) {
        return SL_STATUS_NOT_FOUND;
    }
    if (node != get_zpc_network_node()) {
        sl_log_debug(LOG_TAG, "Note: Retrieving Home/Node/endpoint values from a foreign HomeID.");
    }
    if (SL_STATUS_OK != attribute_store_get_reported(node, home_id, sizeof(zwave_home_id_t))) {
        return SL_STATUS_FAIL;
    }
...
{
    attribute_store_type_t current_node_type = attribute_store_get_node_type(node);
    // Start climbing up until we find an NodeID type of node:
    while (node != ATTRIBUTE_STORE_INVALID_NODE && current_node_type != ATTRIBUTE_NODE_ID) {
    ...
    node = attribute_store_get_first_parent_with_type(node, ATTRIBUTE_HOME_ID);
    if (node != get_zpc_network_node()) {
        sl_log_debug(LOG_TAG, "Note: Retrieving Home/Node values from a foreign HomeID.");
    }
...
sl_status_t attribute_store_network_helper_get_node_id_from_node(attribute_store_node_t node, zwave_node_id_t *zwave_node_id)
{
    ...
    if (SL_STATUS_OK != attribute_store_get_reported(node_id_node, zwave_node_id, sizeof(zwave_node_id_t))) {
        // Abort if no NodeID data was retrieved.
        return SL_STATUS_FAIL;
    }
    return SL_STATUS_OK;
...
{
    attribute_store_node_t endpoint_id_node = attribute_store_get_first_parent_with_type(node, ATTRIBUTE_ENDPOINT_ID);

    if (endpoint_id_node == ATTRIBUTE_STORE_INVALID_NODE) {
        return SL_STATUS_NOT_FOUND;
    }

    if (SL_STATUS_OK != attribute_store_get_reported(endpoint_id_node, zwave_endpoint_id, sizeof(zwave_endpoint_id_t))) {
...
sl_status_t attribute_store_network_helper_get_zwave_ids_from_node(attribute_store_node_t node, zwave_node_id_t *zwave_node_id, zwave_endpoint_id_t *zwave_endpoint_id)
{
    ...
    attribute_store_type_t parent_node_type = attribute_store_get_node_type(parent_node);

    // Start climbing up until we find a consecutive EndpoinID -> NodeID
    // types of node:
    while (parent_node != ATTRIBUTE_STORE_INVALID_NODE && node_type != ATTRIBUTE_ENDPOINT_ID && parent_node_type != ATTRIBUTE_NODE_ID) {
        node             = attribute_store_get_node_parent(node);
    ...
    if (SL_STATUS_OK != attribute_store_get_reported(parent_node, zwave_node_id, sizeof(zwave_node_id_t))) {
        // Abort if no NodeID data was retrieved.
        return SL_STATUS_FAIL;
    }
    if (SL_STATUS_OK != attribute_store_get_reported(node, zwave_endpoint_id, sizeof(zwave_endpoint_id_t))) {
        // Abort if no Endpoint ID data was retrieved.
        return SL_STATUS_FAIL;
    }
...
{
    return attribute_store_get_node_child_by_value(attribute_store_get_node_child_by_value(get_zpc_network_node(), ATTRIBUTE_NODE_ID, REPORTED_ATTRIBUTE, (uint8_t *)&node_id, sizeof(node_id), 0), ATTRIBUTE_ENDPOINT_ID, REPORTED_ATTRIBUTE, &endpoint_id, sizeof(endpoint_id), 0);
}
