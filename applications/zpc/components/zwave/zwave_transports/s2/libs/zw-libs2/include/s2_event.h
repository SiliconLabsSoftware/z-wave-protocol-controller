...
typedef struct {
  struct S2 *
    s2_handle;  ///< Handle reference to the S2 structure to which this event relates.
...
typedef struct {
  ...
  uint8_t
    dsk_length;  ///< Number of missing bytes in the DSK which is requested by the protocol and must be returned with \ref s2_inclusion_challenge_response.
  uint16_t length;  ///< Length of public key following below.
  uint8_t
    public_key[1];  ///< Public key, length of key is describe in length field.
