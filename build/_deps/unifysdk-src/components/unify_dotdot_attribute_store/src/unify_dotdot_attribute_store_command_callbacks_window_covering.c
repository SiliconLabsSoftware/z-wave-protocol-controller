                     ...
                     uic_mqtt_dotdot_callback_call_type_t call_type)
{
  ...
  // operational check
  uint8_t config_status = dotdot_get_window_covering_config_or_status(unid, endpoint, DESIRED_OR_REPORTED_ATTRIBUTE);
  if (!(config_status & WINDOW_COVERING_CONFIG_OR_STATUS_OPERATIONAL))
  {
    sl_log_debug(LOG_TAG, "config status indication not operational, hence ignore command");
    return SL_STATUS_FAIL;
  }
  ...
  if (dotdot_is_supported_window_covering_lift(unid, endpoint)) {
  ...
  if (dotdot_is_supported_window_covering_tilt(unid, endpoint)) {
                           ...
                           uint16_t lift_value)
{
  ...
  // Normal case
  if (is_desired_value_update_on_commands_enabled()
      && dotdot_is_supported_window_covering_lift(unid, endpoint)) {
    sl_log_debug(LOG_TAG,
                 "Updating ZCL desired values after "
                           ...
                           uint16_t tilt_value)
{
  ...
  // Normal case
  if (is_desired_value_update_on_commands_enabled()
      && dotdot_is_supported_window_covering_tilt(unid,
                                                                   endpoint)) {
    sl_log_debug(LOG_TAG,
                 "Updating ZCL desired values after "
