...
static _Atomic bool reset_ongoing;
...
sl_status_t zwave_controller_register_reset_step(zwave_controller_reset_step_t step_function, int32_t priority)
{
    ...
    if ((priority < ZWAVE_CONTROLLER_RESET_STEP_MIN_PRIORITY) || (priority >= ZWAVE_CONTROLLER_MAXIMUM_CALLBACK_SUBSCRIBERS)) {
        return SL_STATUS_FAIL;
    }
...
void zwave_controller_reset()
{
    if (reset_ongoing == false) {
        reset_ongoing = true;
        zwave_controller_on_reset_step_complete(ZWAVE_CONTROLLER_RESET_STEP_MIN_PRIORITY - 1);
    }
}
...
void zwave_controller_on_reset_step_complete(int32_t priority)
{
    ...
    for (uint8_t i = priority + 1; i < ZWAVE_CONTROLLER_MAXIMUM_CALLBACK_SUBSCRIBERS; i++) {
...
#define ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(callback_function, ...)                                                          \
    for (uint8_t i = 0; i < ZWAVE_CONTROLLER_MAXIMUM_CALLBACK_SUBSCRIBERS; i++) {                                            \
        if (zwave_controller_callbacks_array[i] != NULL && zwave_controller_callbacks_array[i]->callback_function != NULL) { \
...
{
    sl_log_info(LOG_TAG, "Network Management error has occurred. Error: %d", error);
...
void zwave_controller_on_node_id_assigned(zwave_node_id_t node_id, zwave_protocol_t inclusion_protocol)
{
    ...
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_node_id_assigned, node_id, included_by_us, inclusion_protocol)
}
...
void zwave_controller_on_frame_transmission(bool transmission_successful, const zwapi_tx_report_t *tx_status, zwave_node_id_t node_id)
{
    ...
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_frame_transmission, transmission_successful, tx_status, node_id)
}
...
void zwave_controller_on_node_added(sl_status_t status, const zwave_node_info_t *nif, zwave_node_id_t node_id, zwave_dsk_t dsk, zwave_keyset_t granted_keys, zwave_kex_fail_type_t kex_fail_type, zwave_protocol_t inclusion_protocol)
{
    ...
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_node_added, status, nif, node_id, dsk, granted_keys, kex_fail_type, inclusion_protocol)
}
...
void zwave_controller_on_network_address_update(zwave_home_id_t home_id, zwave_node_id_t node_id)
{
    ...
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_network_address_update, home_id, node_id)
}
...
void zwave_controller_on_new_network_entered(zwave_home_id_t home_id, zwave_node_id_t node_id, zwave_keyset_t granted_keys, zwave_kex_fail_type_t kex_fail_type)
{
    sl_log_info(LOG_TAG,
                ...
                node_id,
                granted_keys);
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_new_network_entered, home_id, node_id, granted_keys, kex_fail_type)
}
...
{  // Inform listeners that we just had an Rx with a node
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_rx_frame_received, node_id, frame_data, frame_length)
}
...
{
    sl_status_t status = zwave_controller_transport_on_frame_received(connection_info, rx_options, frame_data, frame_length);
    // If no transport plugins needs the frame parse it on to upper layers
    if (status == SL_STATUS_NOT_FOUND) {
        ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_application_frame_received, connection_info, rx_options, frame_data, frame_length);
    } else if (status == SL_STATUS_INVALID_PARAMETER) {
        sl_log_warning(LOG_TAG, "zwave_controller_on_frame_received: Invalid params");
    }
}
...
{
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_protocol_cc_encryption_request, destination_node_id, payload_length, payload, protocol_metadata_length, protocol_metadata, use_supervision, session_id);
}
...
{
    ZWAVE_CONTROLLER_DISPATCH_CALLBACKS(on_smart_start_inclusion_request, home_id, already_included, node_info, inclusion_protocol)
}
