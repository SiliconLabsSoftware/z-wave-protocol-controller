...
// Base class
#include "command_class_protocol_core.hpp"
...
{
    // Log tag
    [[maybe_unused]] static constexpr std::string_view LOG_TAG = "command_class_protocol_core";

    const attribute_list_registration_t attributes = {};

    const command_class_properties cc_properties = {
      .command_class_id   = 1,
      .command_class_name = "Protocol",
      .comments           = "",
      .supported_version  = 1,
    };

    command_class_protocol_core::command_class_protocol_core() : zwave_command_class_base(cc_properties, attributes, "Protocol") {}

    sl_status_t command_class_protocol_core::support_handler(const zwave_controller_connection_info_t *connection_info, const uint8_t *frame_data, uint16_t frame_length)
    {
