...
static void zw_expand_aes_cmac(uint32_t key_id,
...
void generic_expand(uint32_t key_id,
                    const uint8_t *network_key,
...
void networkkey_expand(uint32_t key_id,
                       const uint8_t *network_key,
                       uint8_t *ccm_key,
                       uint8_t *nonce_pstring,
                       uint8_t *mpan_key)
{
  uint8_t constant_nk[15] = {0x55,
                             0x55,
                             0x55,
                             0x55,
                             0x55,
...
void tempkey_extract(const uint8_t *const ecdh_share_secret,
                     const uint8_t *const auth_tag,
                     uint8_t *pseudo_random_keymat_output)
{
  uint8_t temp[96];
  uint8_t constant_prk[16] = {0x33,
                              0x33,
                              0x33,
                              0x33,
                              0x33,
  ...
  memcpy(temp,
         ecdh_share_secret,
         32);                      /*length of ecdh_share_secret is 32 byte */
