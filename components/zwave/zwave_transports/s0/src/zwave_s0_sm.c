...
#include <stddef.h>
#include <string.h>
// Common component (for clock_time_t)
...
typedef enum { EV_S0_BOOTSTRAPPING_START, EV_S0_SCHEME_REPORT_RECEIVED, EV_S0_SEND_KEY_SET, EV_S0_NET_KEY_VERIFY_RECEIVED, EV_S0_SEND_SCHEME_INHERIT, EV_S0_WAS_NOT_CONTROLLER, EV_S0_WRONG_SCHEME_REPORT, EV_S0_BOOTSTRAP_ABORT, EV_S0_TIMEOUT } s0_bootstrap_event_t;

typedef enum { S0_SEND_SCHEME_GET_ACTION, S0_SCHEME_REPORT_RECV_ACTION, S0_SEND_KEY_SET_ACTION, S0_NET_KEY_VERIFY_RECEIVED_ACTION, S0_SEND_SCHEME_INHERIT_ACTION, S0_2SCHEME_REPORT_RECEIVED_ACTION, S0_BOOTSTRAP_ABORT_ACTION, S0_NO_ACTION } s0_action_t;
...
typedef struct {
        ...
        struct timer_handle_t bootstrap_timer;
...
static void reset_s0_sm()
{
    ...
    timer_stop(&s0_sm.bootstrap_timer);
...
static void s0_abort_bootstrap()
{
    ...
    if (s0_on_bootstrapping_complete_callback) {
        s0_on_bootstrapping_complete_callback(0, ZWAVE_NETWORK_MANAGEMENT_KEX_FAIL_KEX_SCHEME);
    }
    timer_stop(&s0_sm.bootstrap_timer);
    memset(&s0_sm, 0, sizeof(s0_sm));
}
...
{
    (void)data;
    sl_log_debug(LOG_TAG, "Timeout in response from NodeID: %d in state: %s\n", s0_sm.bootstrap_conn.remote.node_id, s0_incl_state_name(s0_sm.bootstrap_state));
...
static void s0_send_data_callback(uint8_t status, const zwapi_tx_report_t *tx_info, void *user)
{
    (void)tx_info;
    (void)user;
    sl_log_debug(LOG_TAG, "S0 Send data callback invoked with status = %d\n", status);
    if (status == TRANSMIT_COMPLETE_OK) {
        timer_set(&s0_sm.bootstrap_timer, S0_INCLUSION_TIMER, s0_timeout, 0);
...
static sl_status_t s0_bootstrap_frames_generic_send(zwave_controller_encapsulation_scheme_t en, const uint8_t *frame, uint8_t frame_len)
{
    ...
    sl_status_t ret = zwave_tx_send_data(&s0_sm.bootstrap_conn, frame_len, frame, &options, s0_send_data_callback, 0, 0);
    return ret;
}

static sl_status_t s0_send_scheme_inherit()
{
    ZW_SECURITY_SCHEME_INHERIT_FRAME inherit_frame = {.cmdClass = COMMAND_CLASS_SECURITY, .cmd = SECURITY_SCHEME_INHERIT, .supportedSecuritySchemes = 0};

    return s0_bootstrap_frames_generic_send(ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_0, (uint8_t *)&inherit_frame, sizeof(inherit_frame));
}
...
static sl_status_t s0_send_key_set()
{
    ...
    ZW_NETWORK_KEY_SET_1BYTE_FRAME *net_key_set_frame                       = (ZW_NETWORK_KEY_SET_1BYTE_FRAME *)buf;

    net_key_set_frame->cmdClass = COMMAND_CLASS_SECURITY;
    net_key_set_frame->cmd      = NETWORK_KEY_SET;
    memcpy(&buf[2], key, S0_KEY_LENGTH);

    return s0_bootstrap_frames_generic_send(ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_0, buf, sizeof(buf));
}

static sl_status_t s0_send_scheme_get()
{
    ZW_SECURITY_SCHEME_GET_FRAME scheme_get_frame = {.cmdClass = COMMAND_CLASS_SECURITY, .cmd = SECURITY_SCHEME_GET, .supportedSecuritySchemes = 0};

    return s0_bootstrap_frames_generic_send(ZWAVE_CONTROLLER_ENCAPSULATION_NONE, (uint8_t *)&scheme_get_frame, sizeof(scheme_get_frame));
}

static const s0_transition_t s0_transition_table[] = {
  {S0_INC_IDLE, EV_S0_BOOTSTRAPPING_START, S0_SEND_SCHEME_GET_ACTION, S0_AWAITING_SCHEME_REPORT},

  /* If the Bootstrapping needs to be aborted while scheme get is being sent?
   * for e.g. if scheme get sending failed
   */
  {S0_AWAITING_SCHEME_REPORT, EV_S0_BOOTSTRAP_ABORT, S0_BOOTSTRAP_ABORT_ACTION, S0_INC_IDLE},

  /* If the Bootstrapping needs to be aborted while timing out in getting scheme
   * report as response to scheme get
   */
  {S0_AWAITING_SCHEME_REPORT, EV_S0_TIMEOUT, S0_BOOTSTRAP_ABORT_ACTION, S0_INC_IDLE},

  {S0_AWAITING_SCHEME_REPORT, EV_S0_SCHEME_REPORT_RECEIVED, S0_SCHEME_REPORT_RECV_ACTION, S0_HANDLE_SCHEME_REPORT},

  {S0_HANDLE_SCHEME_REPORT, EV_S0_SEND_KEY_SET, S0_SEND_KEY_SET_ACTION, S0_AWAITING_KEY_VERIFY},

  /* If the Bootstrapping needs to be aborted while timing out in getting net
   * key verify as response to net key set
   */
  {S0_AWAITING_KEY_VERIFY, EV_S0_TIMEOUT, S0_BOOTSTRAP_ABORT_ACTION, S0_INC_IDLE},

  {S0_AWAITING_KEY_VERIFY, EV_S0_NET_KEY_VERIFY_RECEIVED, S0_NET_KEY_VERIFY_RECEIVED_ACTION, S0_HANDLE_NET_KEY_VERIFY},

  {S0_AWAITING_KEY_VERIFY, EV_S0_BOOTSTRAP_ABORT, S0_BOOTSTRAP_ABORT_ACTION, S0_INC_IDLE},
   ...
   */
  {S0_HANDLE_NET_KEY_VERIFY, EV_S0_WAS_NOT_CONTROLLER, S0_NO_ACTION, S0_INC_IDLE},

  {S0_HANDLE_NET_KEY_VERIFY, EV_S0_SEND_SCHEME_INHERIT, S0_SEND_SCHEME_INHERIT_ACTION, S0_AWAITING_2SCHEME_REPORT},

  {S0_AWAITING_2SCHEME_REPORT, EV_S0_SCHEME_REPORT_RECEIVED, S0_2SCHEME_REPORT_RECEIVED_ACTION, S0_INC_IDLE},

  /* If the Bootstrapping needs to be aborted while timing out in getting scheme
   * report as response to scheme inherit (only if node being boostrapped is
   * controller)
   */
  {S0_AWAITING_2SCHEME_REPORT, EV_S0_TIMEOUT, S0_BOOTSTRAP_ABORT_ACTION, S0_INC_IDLE},

  {S0_AWAITING_2SCHEME_REPORT, EV_S0_BOOTSTRAP_ABORT, S0_BOOTSTRAP_ABORT_ACTION, S0_INC_IDLE},
};
...
static void s0_sm_process_event(uint16_t evt)
{
    ...
    sl_log_debug(LOG_TAG, "s0_sm_process_event state: %s: event: %s\n", s0_incl_state_name(s0_sm.bootstrap_state), s0_event_name(event));
    for (int i = 0; i < ELEM_COUNT(s0_transition_table); i++) {
        if ((s0_sm.bootstrap_state == s0_transition_table[i].state) && (event == s0_transition_table[i].event)) {
            if (s0_sm.bootstrap_state != s0_transition_table[i].new_state) {
                s0_sm.bootstrap_state = s0_transition_table[i].new_state;
            }

            sl_log_debug(LOG_TAG, "action: %s new state: %s\n", s0_action_name(s0_transition_table[i].action), s0_incl_state_name(s0_sm.bootstrap_state));
            switch (s0_transition_table[i].action) {
                ...
                case S0_2SCHEME_REPORT_RECEIVED_ACTION:
                    if (s0_on_bootstrapping_complete_callback && s0_sm.is_controller) {
                        s0_on_bootstrapping_complete_callback(KEY_CLASS_S0, ZWAVE_NETWORK_MANAGEMENT_KEX_FAIL_NONE);
                    } else {
                        sl_log_warning(LOG_TAG, "No s0_on_bootstrapping_complete_callback set to be notified?\n");
                    }
                    break;
...
void s0_scheme_report_received(const uint8_t scheme, zwave_node_id_t from_node)
{
    ...
    if ((from_node == s0_sm.bootstrap_conn.remote.node_id) && ((s0_sm.bootstrap_state == S0_AWAITING_SCHEME_REPORT) || (s0_sm.bootstrap_state == S0_AWAITING_2SCHEME_REPORT))) {
        timer_stop(&s0_sm.bootstrap_timer);
