 ...
 */
typedef uint8_t (*zwave_controller_storage_cc_version_t)(zwave_command_class_t command_class, zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id);
...
{
    if (callbacks == NULL) {
        sl_log_warning(LOG_TAG, "zwave_controller_storage callbacks are not registered.");
...
bool zwave_controller_storage_is_node_s2_capable(zwave_node_id_t node_id)
{
    if (verify_node_is_s2_cb == NULL) {
        sl_log_warning(LOG_TAG, "Callback that checks if a node is supporting S2 is not registered.");
        return true;
    }
...
sl_status_t zwave_controller_storage_as_set_node_s2_capable(zwave_node_id_t node_id)
{
    if (set_node_as_s2_capable_cb == NULL) {
        sl_log_warning(LOG_TAG, "Callback that sets a node as S2 capable is not registered.");
        return SL_STATUS_FAIL;
    }
    return (set_node_as_s2_capable_cb(node_id));
}

uint8_t zwave_controller_storage_get_cc_version(zwave_command_class_t command_class, zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    if (zwave_node_cc_version_cb == NULL) {
        sl_log_warning(LOG_TAG, "Callback that helps to get the node cc version is not registered.");
        return 1;
    }
...
sl_status_t zwave_controller_storage_get_node_granted_keys(zwave_node_id_t node_id, zwave_keyset_t *keys)
{
    if (granted_keys_cb == NULL) {
        sl_log_warning(LOG_TAG, "Callback that helps to get granted keys is not registered.");
        return SL_STATUS_FAIL;
    }
