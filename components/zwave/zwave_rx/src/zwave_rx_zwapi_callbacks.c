...
{
    zwave_controller_on_protocol_cc_encryption_request_received(destination_node_id, payload_length, payload, protocol_metadata_length, protocol_metadata, use_supervision, session_id);
}
...
{
    if (zwave_command_payload_length == 0) {
        sl_log_warning(LOG_TAG, "Dropping zero-length application frame from NodeID %d\n", source_node_id);
        return;
    }
    ...
    if ((rx_status & RECEIVE_STATUS_TYPE_BROAD) || (rx_status & RECEIVE_STATUS_TYPE_MULTI)) {
        connection_info.local.is_multicast = true;
    }

    zwave_rx_receive_options_t rx_options = {.status_flags = rx_status, .rssi = rssi_value, .nodes_in_multicast = {0}};

    // Tell the controller about the Rx event, forwarding the Z-Wave frame
    // so subscribers can make payload-aware decisions synchronously
    // on the Rx thread (before transport dispatch).
    zwave_controller_on_frame_reception(source_node_id, zwave_command_payload, (uint16_t)zwave_command_payload_length);

    // Pass the frame on to the Z-Wave controller module
    zwave_controller_on_frame_received(&connection_info, &rx_options, zwave_command_payload, (uint16_t)zwave_command_payload_length);
}
...
void zwave_rx_application_command_handler_bridge(uint8_t rx_status, zwave_node_id_t destination_node_id, zwave_node_id_t source_node_id, const uint8_t *zwave_command_payload, uint8_t zwave_command_payload_length, int8_t rssi_value)
{
    ...
    zwave_rx_application_command_handler(rx_status, destination_node_id, source_node_id, zwave_command_payload, zwave_command_payload_length, rssi_value);
}
...
void zwave_rx_application_controller_update(uint8_t status, zwave_node_id_t node_id, const uint8_t *zwave_nif, uint8_t zwave_nif_length, zwave_home_id_t nwi_home_id)
{
    ...
    switch (status) {
        ...
        case UPDATE_STATE_NODE_INFO_RECEIVED:  // NIF has been received
            // NIF means we received a frame from that node. No Z-Wave frame
            // is available here, so pass NULL/0 for frame_data/length.
            zwave_controller_on_frame_reception(node_id, NULL, 0);
            // Assemble the NIF data first
            zwave_rx_map_node_information(node_id, zwave_nif, zwave_nif_length, &received_nif);
        ...
        case UPDATE_STATE_NODE_INFO_SMARTSTART_HOMEID_RECEIVED_LR:  // SmartStart Prime received on Long Range
            ...
            zwave_rx_map_node_information(0, zwave_nif, zwave_nif_length, &received_nif);

            // Notify the Z-Wave controller, which will dispatch the event further.
            zwave_controller_on_smart_start_inclusion_request(nwi_home_id, already_included, &received_nif, status == UPDATE_STATE_NODE_INFO_SMARTSTART_HOMEID_RECEIVED_LR);
            break;
...
static void zwave_rx_map_node_information(zwave_node_id_t node_id, const uint8_t *zwapi_zwave_nif, uint8_t zwapi_zwave_nif_length, zwave_node_info_t *destination_node_info)
{
    if (zwapi_zwave_nif == NULL || destination_node_info == NULL || zwapi_zwave_nif_length == 0) {
        return;
    }
    ...
    if (node_id > 0) {
        ...
        sl_status_t command_status                         = zwapi_get_protocol_info(node_id, &zwave_rx_node_info_header);
        if (command_status == SL_STATUS_OK) {
            // Copy our retrieved data into the NIF, else it will just be 0.
            destination_node_info->listening_protocol = zwave_rx_node_info_header.capability;
            destination_node_info->optional_protocol  = zwave_rx_node_info_header.security;
        }
    }
    ...
    zwave_command_class_list_unpack(destination_node_info, &zwapi_zwave_nif[index], zwapi_zwave_nif_length - 3);
}

void zwave_rx_poll_request()
{
    zwave_rx_process_request_poll();
}
