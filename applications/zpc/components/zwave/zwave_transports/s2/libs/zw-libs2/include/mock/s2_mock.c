...
#define MOCK_CALL_COMPARE_INPUT_STRUCT_S2(P_MOCK, ARGUMENT, P_RECV_S2) \
  do {                                                                 \
    MOCK_CALL_COMPARE_STRUCT_MEMBER_UINT32(P_MOCK,                     \
                                           ARGUMENT,                   \
                                           P_RECV_S2,                  \
                                           struct S2,                  \
                                           my_home_id);                \
    MOCK_CALL_COMPARE_STRUCT_ARRAY_LENGTH_UINT8(P_MOCK,                \
                                                ARGUMENT,              \
                                                P_RECV_S2,             \
                                                struct S2,             \
                                                sg[0].enc_key,         \
...
#define MOCK_CALL_COMPARE_INPUT_STRUCT_S2_CONNECTION(P_MOCK,   \
                                                     ARGUMENT, \
                                                     P_ACTUAL) \
  do {                                                         \
...
uint8_t S2_send_data(struct S2 *ctx,
                     s2_connection_t *dst,
                     const uint8_t *buf,
                     uint16_t len)
{
...
void S2_application_command_handler(struct S2 *ctx,
                                    s2_connection_t *src,
                                    uint8_t *buf,
                                    uint16_t len)
{
