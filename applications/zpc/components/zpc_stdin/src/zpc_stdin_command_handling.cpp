...
const std::map<std::string, std::pair<std::string, handler_func>> commands = {
  ...
  {"zwave_set_priority_route",
   {" :Set the priority route to a destination node", &handle_set_priority_route}},
...
static sl_status_t handle_set_priority_route(const handle_args_t &arg)
{
  ...
  try {
    dst_node_id = static_cast<zwave_node_id_t>(std::stoi(arg[1].c_str(), nullptr, 16));
    for (size_t i = 0; i < 5; ++i) {
      route[i] = static_cast<uint8_t>(std::stoi(arg[i + 2].c_str(), nullptr, 16));
    }
  } catch (const std::invalid_argument &e) {
