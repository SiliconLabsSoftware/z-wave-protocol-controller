...
sl_status_t zwapi_send_data_multi_bridge(zwave_node_id_t source_node_id, const zwave_nodemask_t destination_node_mask, const uint8_t *data, uint8_t data_length, uint8_t tx_options, void (*callback_function)(uint8_t tx_status))
{
    uint8_t number_of_nodes                     = 0;
    uint8_t number_of_nodes_index               = 0;
    uint8_t func_id                             = 0;
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_SEND_DATA_MULTI_BRIDGE, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_send_data_bridge(zwave_node_id_t source_node_id, zwave_node_id_t destination_node_id, const uint8_t *data, uint8_t data_length, uint8_t tx_options, void (*callback_function)(uint8_t, zwapi_tx_report_t *))
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_SEND_DATA_BRIDGE, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_send_data_multi(zwave_nodemask_t destination_node_mask, const uint8_t *data, uint8_t data_length, uint8_t tx_options, void (*callback_function)(uint8_t tx_status))
{
    ...
    if (zwapi_get_library_type() == ZWAVE_LIBRARY_TYPE_CONTROLLER_BRIDGE) {
        return zwapi_send_data_multi_bridge(0xFF, destination_node_mask, data, data_length, tx_options, callback_function);
    }
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_SEND_DATA_MULTI, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_send_data(zwave_node_id_t destination_node_id, const uint8_t *data, uint8_t data_length, uint8_t tx_options, void (*callback_function)(uint8_t, zwapi_tx_report_t *))
{
    // If the current API is a Bridge, just call the bridge version.
    if (zwapi_get_library_type() == ZWAVE_LIBRARY_TYPE_CONTROLLER_BRIDGE) {
        return zwapi_send_data_bridge(0xFF, destination_node_id, data, data_length, tx_options, callback_function);
    }
    ...
    if ((data_length + 2) > sizeof(request_buffer)) {
        sl_log_error(LOG_TAG, "The frame is too long, therefore, the frame is discarded\n");
        return SL_STATUS_FAIL;
    }
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_SEND_DATA, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_send_protocol_data(zwave_node_id_t destination_node_id, const uint8_t *data, uint8_t data_length, void *metadata, void (*callback_function)(uint8_t, zwapi_tx_report_t *))
{
    ...
    // Check for too long payloads
    if ((data_length + protocol_metadata->data_length + 2) > sizeof(request_buffer)) {
        sl_log_error(LOG_TAG, "The frame is too long, therefore, the frame is discarded\n");
        return SL_STATUS_FAIL;
    }
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_ZW_SEND_PROTOCOL_DATA, request_buffer, index, response_buffer, &response_length);
...
sl_status_t zwapi_send_nop(zwave_node_id_t destination_node_id, uint8_t tx_options, void (*callback_function)(uint8_t, zwapi_tx_report_t *))
{
    ...
    sl_status_t send_command_status = zwapi_send_command_with_response(FUNC_ID_SEND_NOP, request_buffer, index, response_buffer, &response_length);
    ...
    if (send_command_status == SL_STATUS_NOT_SUPPORTED) {
