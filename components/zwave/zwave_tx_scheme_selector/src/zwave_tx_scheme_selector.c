...
{
    zwave_controller_storage_get_node_granted_keys(zwave_network_management_get_node_id(), keys);
}
...
void zwave_tx_scheme_get_node_connection_info(zwave_node_id_t node_id, uint8_t endpoint_id, zwave_controller_connection_info_t *connection_info)
{
    ...
    connection_info->local.node_id       = 0;  // 0 will make the tx component to use ZPC NodeID
    connection_info->local.endpoint_id   = 0;

    // Find out the highest granted key of the remote node
    zwave_keyset_t remote_node_keys;
    zwave_controller_storage_get_node_granted_keys(connection_info->remote.node_id, &remote_node_keys);

    connection_info->encapsulation = zwave_controller_get_highest_encapsulation(remote_node_keys);
}
...
void zwave_tx_scheme_get_node_tx_options(uint32_t expected_qos_priority, uint8_t expected_number_of_response, uint32_t expected_discard_timeout_ms, zwave_tx_options_t *tx_options)
{
    ...
    tx_options->skip_s2_verify_delivery = false;
...
{
    zwave_protocol_t protocol = zwave_controller_storage_inclusion_protocol(node_id);
...
uint16_t zwave_tx_scheme_get_max_application_payload(zwave_node_id_t node_id, zwave_endpoint_id_t endpoint_id)
{
    ...
    } else if ((encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_ACCESS) || (encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_AUTHENTICATED) || (encapsulation == ZWAVE_CONTROLLER_ENCAPSULATION_SECURITY_2_UNAUTHENTICATED)) {
